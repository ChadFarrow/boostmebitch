...
/**
 * Nothing was paid, so the caller may still pay this another way.
 *
 * The distinction is the same one `NwcNotAttemptedError` draws on the boost
 * rails. No caller falls back today — the note card's zap reports any failure —
 * but the class is what would license one, and widening it to cover a wallet
 * failure would re-pay a zap that already went out.
 */
...
export async function sendZap(args: {
  ...
  try {
    prepared = await prepareZap(args);
  } catch (e) {
    ...
    // nothing moved — which is what would license a caller to pay another way.
    // Classifying throw sites one at a time is how the opposite mistake gets
    // made: one unwrapped `throw` reads as a payment failure. Same discipline
    // as NwcNotAttemptedError; see boost invariant 11 in CLAUDE.md.
    throw e instanceof ZapNotAttemptedError
      ? e
      : new ZapNotAttemptedError(e instanceof Error ? e.message : String(e));
  }
 ...
 */
async function prepareZap(args: Parameters<typeof sendZap>[0]): Promise<PreparedZap> {
  ...
  // The tag list is a pinned pure function — see lib/nostr/zap-request.ts for
  // the two real Fountain requests it is checked against. `p` is the zapped
  // author's pubkey and `e` the zapped note.
  //
  // No `client` tag. The recipient's LNURL server reads this event before any
  // relay does, so an app-identity claim here is a money-path change.
  ...
  // The LUD-21 comment is the typed message, fitted to `commentAllowed`. No
  // BoostBox descriptor: a zap is never a boost payment (boost invariant 12),
  // so there is no `rss::payment` record to point at.
  const commentArgs = { message: args.comment };
  ...
  // otherwise kills the zap with the sats still in the wallet, and nothing
  // moved before the refusal, so asking again for an invoice is safe. The
  // retry costs the reader nothing: the typed message also rides in the
  // kind:9734 `content`, which is the copy every Nostr client renders.
  if (out.failed && comment) {
    const retry = lnurlCommentRetry(commentArgs, out.why, comment);
    if (retry) {
      console.info(
        retry.comment === undefined
          ? `[zap] comment of ${comment.length} refused for length; retrying with no comment ` +
              `(${out.why ?? 'no reason given'}) — the message still rides in the kind:9734 content`
