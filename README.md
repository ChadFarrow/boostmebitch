...
- **NIP-46 bunker / `nostrconnect://`** remote signer (nsec.app, Clave, Amber-as-bunker, Primal). One-tap hand-off on both mobiles — `nostrconnect://` into Amber on Android, `clave://` into Clave on iOS, straight from the header dropdown so it costs one tap rather than three. The custom scheme rather than Clave's Universal Link because a universal link only reaches the app from a real tap on a real anchor — dispatched from script it loads clave.casa and takes the pending pairing with it. clave.casa stays as the labelled fallback, rendered as an anchor so it can still open the app, and `bmb:nc_pending` makes a lost pairing resumable either way. A signer that queues a request for its user (Clave answers `permission denied` first and the real result after the tap) is asked again rather than reported as having refused.
