...
- **NIP-46 bunker / `nostrconnect://`** remote signer (nsec.app, Clave, Amber-as-bunker, Primal). One-tap hand-off on both mobiles — `nostrconnect://` into Amber on Android, Clave's Universal Link on iOS, straight from the header dropdown so it costs one tap rather than three (`clave://` stays as the recovery when Safari has been told to open clave.casa as a web page). A signer that queues a request for its user (Clave answers `permission denied` first and the real result after the tap) is asked again rather than reported as having refused.
