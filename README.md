...
**Every boost payment carries Podcasting 2.0 metadata — none is a zap.** A leg pays by keysend with the boostagram in TLV `7629169`, or over LNURL with the BoostBox `rss::payment` descriptor in the comment. That includes a boost to a Nostr live stream, which pays its host the same way and, when you share under your own key, also posts a kind:1311 "⚡ Boosted N sats" line into the stream's chat. The Nostr side is the boost note, which quotes a site-signed summary receipt for the sats that settled.
