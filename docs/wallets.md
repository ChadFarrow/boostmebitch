...
**Not pinnable by a `check:*` script:** `lib/v4v/nwc.ts` imports `../storage`, which touches `localStorage`, so it can't load under `node --experimental-strip-types`. Verify in DevTools → Network → WS: sockets to the wallet relay should return to baseline after a boost, and the real test is four or five boosts **without reloading**, since one clean boost proves nothing about a leak.
