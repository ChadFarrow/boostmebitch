...
> **STATUS (2026-09-06).** This document is carried forward as the written
> requirement for work that has **not** shipped. PR #132 was rewritten against a
> `main` 532 commits younger than the branch it was drafted on, and its scope was
> cut to the **listen queue alone**: the cross-show queue, its persistence and its
> `+ queue` controls are in the tree; the **Inbox** (per-show "N NEW" badges over
> favorites, and the `seen` set this document syncs) and the **kind:30078 sync
> described below** are both deferred.
>
> Two things below are already out of date and are kept as written on purpose,
> because rewriting them would lose the reasoning they record:
>
> - The Inbox it names has no host component any more. `FavoritesList` was
>   deleted when favorites moved to `/favorites`; a re-scope targets
>   `components/favorites-page.tsx`, not the browse panel.
> - Favorites no longer sync over NIP-51. They are one kind:10333 replaceable
>   event — see [`docs/nostr.md`](../../nostr.md).
>
> The `queue` half of the design still describes the shape that shipped: each
> item carries its own podcast, and the local timestamp it compares against is
> the one a future publisher would still need.
