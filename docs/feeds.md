...
- **NPUB** — never reaches the server at all. It is a mode of the *input*: the box already recognises a pasted key and skips Podcast Index for one, and this app has no Nostr name search to offer. That also makes it the strongest form of the `nsec` guard — no request is issued. A name typed under it renders an explainer saying what the mode accepts, because a mode that silently does nothing is the dead control this repo keeps paying for.
