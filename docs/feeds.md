...
- **The roster is cached hard and NEVER blocks a search.** List feeds are rare and slow-moving, so one refresh serves six hours (servable for 24). A cold roster returns no lane and warms behind the request, because ten PI round trips is not a thing to make somebody wait for. That is the read-index rule again: this lane may be absent, and its absence must never be reported as "there are no playlists".
