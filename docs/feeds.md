...
- **The Upcoming half is a by-product, and the page must not overclaim it.** PI indexes currently-broadcasting rows only (`lib/pi.ts`), while `parseRssLiveItems` returns `pending` and `live` together — so every feed read for verification hands back its schedule for free, and **no feed that is not already live is ever read.** There is no global source for a pending item and no endpoint that would provide one. `<LivePage>`'s Upcoming caption says so in the section rather than in a tooltip.
