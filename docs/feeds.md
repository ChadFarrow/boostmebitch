...
- **Out of scope for the POLL:** a live item published *after* page load won't appear (that needs replacing the live section, not patching fields), and Nostr kind:30311 streams are untouched — `nostr-live-streams.tsx` polls itself. Finding a broadcast you had not already opened the show for is `/api/live-shows`, below.

## `/api/live-shows` — every show on air, not just the one you opened

Backs `/live`. Until it existed a `<podcast:liveItem>` was reachable only by opening the show first, so a listener had to guess.

**Podcast Index answers WHICH FEEDS to look at; each publisher's RSS answers WHAT IS LIVE.** `getGlobalLiveItems` (`lib/pi.ts`) is one call to `/episodes/live?max=1000`, which `/api/feed` was already making and discarding all but one feed of. It is a good roster and a poor authority — the same lag recorded above, in both directions: it under-reports a feed that has just gone live, and keeps rows for broadcasts that have ended. So every candidate is re-read from its own feed.

- **The roster fetch is deliberately UNCAUGHT, and the line next to it in `/api/feed` is the trap.** That route writes `getLiveItemsForFeed(id).catch(() => [])`, correctly — a live hiccup must not blank a page holding 500 real episodes. Here the live items *are* the page, so swallowing the throw converts "PI would not answer us" into "nobody is broadcasting". Letting it throw is what routes a 429/408 through `withErrorHandling` → `piCouldNotAskStatus` verbatim, keeping the client breaker shut.
- **Two fan-outs, SEQUENTIAL, each with its own ceiling.** PI's live record carries `feedId` but no feed URL, so stage one resolves `getPodcast` per candidate at `PI_FANOUT` and stage two reads each RSS at `FEED_FANOUT`. Nesting the second inside the first is still "bounded" and is wrong: it makes PI's courtesy limit double as the heap limit, and the two constants' own doc comments say they answer different questions. `check:livemerge` pins that they stay separate.
- **`MAX_LIVE_FEEDS` is sized against the clock, not against taste.** `PI_TIMEOUT_MS` is 8 s and no route here sets `maxDuration`, so the worst case is `ceil(N/PI_FANOUT)×8s + ceil(N/FEED_FANOUT)×8s`. At 24 that is seven rounds. Going over is reported as `truncated`, never silently trimmed.
- **An unreadable feed does not end a show — it flags one.** `mergeLiveOverPi` (`lib/util.ts`, pinned by `check:livemerge`) takes the `ok` flag, not a bare array: `ok: true` makes RSS authoritative about which items exist *including the ones it omits*, and `ok: false` makes it authoritative about nothing, so PI's rows survive as `verified: false`. One publisher's slow host must not take a running broadcast off the only page that lists broadcasts; the page says the list may be stale instead.
- **A `status="live"` flag is not evidence a show is on air.** Measured 2026-09-06 on production: Behind the Sch3m3s was serving `status="live"` for `S02E49` with `end="2026-09-01T05:30:00.000Z"` — its own declared finish five days earlier — and `/live` rendered a pulsing badge and a working PLAY button over it. The status is a flag a human flips and humans forget; nothing downstream recovers, because the item never becomes `ended` and never leaves the feed. `liveBroadcastIsOver` (`lib/util.ts`, pinned by `check:liveover`) drops it, in the RSS parser AND in `getGlobalLiveItems` — both, because a `pi-only` row survives an unreadable feed by design, so filtering one half leaves the failure reachable whenever a publisher 503s. **`end` in the past is authoritative** and needs no heuristic; the 24 h ceiling is only for feeds that declare no `end`, and it is deliberately far longer than the Nostr side's `LIVE_FRESH_SECS` of 2 h because a podcast broadcast runs for hours (Satellite Skirmish's own items are 3.5 h) where a stream runs for minutes. `pending` is never over — a scheduled item whose start has passed is a host running late.
- **The client's roster is favorites UNION the sent-boost log, music filtered out, rotated.** Three rules, each earning its place. **Music is dropped** (`isMusicMedium`/`isPlaylistMedium`) because an album never publishes a `<podcast:liveItem>` and a library here is mostly albums — it removes the large majority before anything is asked; an *absent* medium is KEPT, since absent means "not resolved yet", never "not a podcast". **The boost log joins it** (`bmb:boosts:<npub>`, `StoredBoost.podcastId`): somebody who sent sats was listening, which predicts caring about the next broadcast better than a heart pressed once, and boosting does not favorite so the lists overlap less than expected. **The window rotates** `MAX_FAVORITE_FEEDS` at a time, because a library larger than one request would otherwise have everything past the first slice permanently invisible; verified covering 30 feeds through a 20-wide window. Upcoming is therefore ACCUMULATED across polls and live is not — a schedule is stable for days, an on-air state is not, and a remembered live row is the forgotten-flag failure above arriving by another door.
- **The global version of that roster is public data we already index, and it is the named next step.** Boost notes carry `['i', 'podcast:guid:<uuid>']` and `['k', 'podcast:guid']` (`lib/nostr/boost-notes.ts`), and `services/nostr-index` stores kind:1 and indexes exactly those tags (`queries.ts`) — so every show anyone has boosted is already in Postgres, with no privacy constraint at all, which is what makes it a better roster than either local list. What is missing is a "distinct recently-referenced podcast guids" query; `notesByIdentifier` answers the opposite question. That is a new query plus endpoint in a service that **does not deploy when you merge**, which is why it is written down here rather than built.
- **Three rosters feed the RSS pass, and they are bounded separately because they carry different risk.** `MAX_LIVE_FEEDS` (24) bounds what PI reported live. `MAX_FAVORITE_FEEDS` (12) bounds the `?feeds=` list — the caller's own favorited feed ids, which is **untrusted input**: anyone can name any ids, so they are validated as positive integers, deduped and capped exactly as `/api/by-guid/batch` treats its guid list. `MAX_REMEMBERED_FEEDS` (10) bounds the recently-live memory below.
- **The Upcoming half is a by-product, and the page must not overclaim it.** PI indexes currently-broadcasting rows only (`lib/pi.ts`), while `parseRssLiveItems` returns `pending` and `live` together — so every feed read hands back its schedule for free. **There is no global source for a pending item**: `/podcasts/bytag` supports only `podcast-value` and `podcast-valueTimeSplit`, so no endpoint anywhere lists "feeds that publish live items". Measured 2026-09-06 with nothing live worldwide: Homegrown Hits was publishing `status="pending"` for Episode 150 and PI's roster named nobody, so that item was reachable only through the two rosters below. `<LivePage>`'s Upcoming caption says which shows it can see, in the section rather than in a tooltip.
- **The `?feeds=` answer is `private, max-age=30`, never `s-maxage`.** A response that includes rows read from the caller's own favorites is not the same document for everybody and must not sit in a shared cache. The CDN keys by URL and the ids ride in the query, so a cross-user hit is unlikely anyway — but "unlikely" is not the test for personal data in a shared cache, and the browser still caches it for the same 30 s.
- **The recently-live memory is built from PI's roster ONLY, and that restriction is a privacy rule rather than an optimisation.** `seenLiveCache` remembers each feed this server watched go live, with its URL, for 24 h, so a show that broadcast yesterday keeps surfacing its NEXT episode — the roster nobody publishes, assembled from observation. It is server-wide state that shapes what every visitor sees, so it may only be seeded from PI's public answer: seeding it from a caller's favorites would let one person's private list change another person's page, which is an inference about that list however weak. It remembers a `live` item and never a `pending` one, because remembering a feed off its own schedule is a loop rather than an observation. **It is per server instance** — module state does not survive a cold start or cross instances — so coverage is patchy and resets. That is tolerable only because it is strictly additive: every row is still read from the publisher's own feed and merged by `mergeLiveOverPi`, so the failure mode is a shorter list, never a wrong one. A remembered feed keeps its URL and therefore skips the PI stage entirely, which is what makes it the cheapest of the three. The version without the caveat is a background crawler over the `podcast-value` roster with durable storage in `services/nostr-index`; that is the named next step, not this.
- **The limitation that will arrive as a bug report:** RSS verification fixes false *positives* only. A show that is genuinely live but absent from PI's roster is invisible here and always will be — which is the direction the 2026-08-07 observation actually measured. The honest fix is a durable roster of feeds known to publish live items, swept on a schedule; that needs storage this app does not have server-side (`services/nostr-index` is the only place that does). A client-side sweep of the user's own favorites would also work and is **not** to be built as a server call — favorites are local-or-NIP-44-private by design.
- **Cache-Control `max-age=30, s-maxage=30` on the 200 only**, with `no-store` whenever PI could not be fully asked, so the thin answer is not served for the rest of the window. `maxAgeMs: 10_000` on the RSS reads matches the two sibling live routes; the route's own 30 s window is what bounds upstream load, not that number.
...
**The Greatest Hits playlist is "organized by play count", and it says so with a second marker kind.** The feed is the tracks that appeared on two or more of ChadF's playlists, and above each run it writes `<podcast:txt purpose="playcount">24 plays</podcast:txt>`, then `21 plays`, and so on down to `2 plays` — the same marker-before-run shape as the episode captions, read in the same one pass of `parsePlaylistRemoteItems`. It carries as `PlaylistItemRef.plays` and `Episode.playlistPlays`, through both resolution paths (`withMarkers` in `/api/playlist` and the `opts` of `dbRowToEpisode`), and the list heads each run the way StableKraft does: `24 PLAYS · 1 TRACK`, `21 PLAYS · 1 TRACK`, `17 PLAYS · 3 TRACKS`. The heading is the same comparison-with-the-previous-row that `playlistGroup` uses, so it survives "load more" with no extra state.

Four things that are not derivable from the feed's shape:

- **It is a NUMBER, never the marker's text.** `parsePlayCount` takes a leading run of digits and nothing else, so `24 plays` and a bare `24` both count, `plays: 24` does not, and `0 plays` clears rather than prints — a zero on a most-played list is a data error, and printing it states the error as a fact. An episode caption is a heading the curator wrote, and printing that text is the point; a count is a claim about the track, and free text there would print whatever a feed puts under the purpose.
- **The track total in the heading is the SERVER's, over the whole list (`PlaylistResponse.playGroups`), never a count of the rows loaded so far.** A page is 100 rows and the Greatest Hits list's `2 plays` run is ~800 tracks, so a client-side count would read `2 PLAYS · 100 TRACKS` and then grow on every "load more" — a number that changes as you scroll is one the reader cannot trust. `playGroupsOf` walks the deduped refs once per request, in wire order, so it agrees with `total`; it is undefined when the list carries no marker, so ordinary playlists do not grow an empty array. Read from page 0 like `sourceShow`, and the e2e harness pins it with a total deliberately larger than the rows on the page.
- **The two marker kinds are INDEPENDENT states.** A play-count marker never clears an episode caption and an episode marker never clears a count, so a playlist that one day publishes both keeps each across changes in the other, and the list renders both headings. Pinned by `check:musicl`, with the naive replay reading every `<podcast:txt>` as a caption — which would head each Greatest Hits run with "24 plays" as an episode and count nothing.
...
**The runs are not collapsible, deliberately for now.** StableKraft folds each run behind its heading. Here a run can span pages that are not loaded yet, so a fold would have to hide rows it does not hold and "expand" would have to fetch — a paging change, not a heading. The heading alone gives the reader the structure the feed was built around.
