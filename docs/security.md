...
**And a second bucket, keyed by ROUTE alone, because the first can be walked around.** A route handler never sees the socket's peer address, so behind a bare `next start` with no proxy the header IS the identity, and rotating it lands every request in a fresh per-IP bucket. That was the only control on `/api/nostr/site-sign`, an unauthenticated signing oracle. The route bucket caps the instance's total at `limit × GLOBAL_MULTIPLIER` (20) a minute whoever is asking — 1,200 for `/api/feed`, 600 for site-sign — so rotation buys a bounded amount, not an unbounded one. The trade-off is that a flooded instance 429s honest clients too; `lib/podcast-meta.ts` files a 429 under `COULD_NOT_ASK` (uncached null, no breaker trip), so the client degrades for a minute rather than poisoning its caches. No env var and no trusted-proxy list: the ceiling holds on every deployment shape without configuration.

**`q` is in the allowlist now, under a rule of its own.** A site-published boost note quotes its summary receipt — a kind:9735 this same key signed through the second oracle below — so the note carries one `q` tag. At most ONE, and only one whose author field is the site's own pubkey: bound that way, it cannot be what the `e` refusal exists to prevent, a signed instruction from a verified identity to render some stranger's note beneath the site's words. The check is on the author field, the only thing this oracle can see; a reader that fetches the event checks the signature.

### The receipt oracle (`app/api/nostr/zap-receipt-sign`)

The second signing oracle for the site's key, and it is narrower than the first. It signs one shape, a kind:9735 summary receipt, and it never signs caller-supplied tags: it takes a kind:9734 the SENDER signed (or, for a site-published note, three bounded facts — amount, relays, refs — from which it builds and signs the 9734 itself), validates it with `validateSummaryRequest` (`lib/nostr/zap-request.ts`, pinned by `check:zapreceipt`), proves the signature with `verifyEvent`, and derives the receipt. So a caller can make the site attest a payment from their own key, or an anonymous one from the site's — the same claims a boost note already lets them make — and nothing else. The rules: `p` the site and once, `e` refused, `client` refused, `content` empty, amount whole sats at most 1,000,000, relays `wss://` at most eight, NIP-73 refs in the podcast namespace with an `i` needing its `k` and a hint needing `http(s)`, `created_at` within ±5 minutes. Same rate limit, content-type check, capped body read and 503-when-unconfigured as site-sign. What the receipt claims and does not claim is in [`money-boosts.md`](money-boosts.md).

### Response size caps (`readCappedText` / `readCappedJson` / `readCappedBytes`, in `lib/capped-body.ts`)

**The readers live in `lib/capped-body.ts`, an import-free leaf, and not beside the SSRF guard — because the browser needs them too.** They sat in `lib/safe-fetch.ts`, which imports `node:dns/promises`, so no client module could reach them, and the one third-party body the BROWSER reads — the direct LNURL fetch in `lib/v4v/lnurl-fetch.ts`, in the origin that holds the NWC spending credential — went through a bare `res.text()` while every server read was metered. It is capped at the same 256 KB `/api/lnurl` enforces now, and the Drive/Google reads in `lib/nostr/` are capped with the same helper. `safe-fetch.ts` deliberately does NOT re-export them: `check:ssrf` loads that file under plain Node, which cannot resolve an extensionless relative import. Pinned by `check:cappedbody`, including the one path that had no cap at all — a response with no stream (`res.body === null`) was buffered whole and returned unmeasured; it is now measured after the buffer and refused past the limit, the best a bodyless response allows.
...
**`readCappedBytes` is that same loop without the decode, and it is where a binary body goes.** `/api/og/boost.png` fetches a feed's cover art to draw into the boost banner, and `TextDecoder` over a PNG returns replacement characters — so the text reader is not an option there, and the tempting fallback, `res.arrayBuffer()`, buffers the whole body first and measures after, which is the exact behaviour this section exists to prevent. `readCappedText` now delegates to it: one cap, two shapes. That route pairs it with a 2 MB ceiling (real covers are 50–500 KB) and a `Content-Type` allowlist of what the rasterizer can actually decode. **An earlier version of this paragraph then claimed the bytes are "never proxied back", so an internal response that got past `safeFetch` could not be read out of it. That was wrong.** The bytes are base64'd into a data URI and drawn as an `<img>` inside the returned `ImageResponse`, so a caller can read them back as pixels. The `RASTERIZABLE` allowlist (`image/png`, `image/jpeg`, `image/jpg`, `image/gif`) narrows it to four types — it is not a text-read oracle — but it is a read primitive, not a blind write. **So there are THREE reads behind `safeFetch`, not one**: `/api/transcript` returns the body verbatim, `/api/art` returns it re-encoded, and this route returns it rendered. Nothing here is reachable while `safeFetch` holds; the point is what a successful rebind buys, and the open TOCTOU item above should be read with that in mind rather than as "blind SSRF at worst".
...
What *is* bounded is the amplifier. Tags are **allowlisted** to the exact vocabulary `buildBoostNoteTemplate` emits (`i`, `k`, `r`, `p`, `amount`, `client`, `t`, `imeta`), which is provably non-regressive because that vocabulary is enumerable, and `p` tags are capped at 8. The two tags that carry a URL are bound to their real shape too: at most two `r` tags, each an http(s) URL under `httpUrl` (the episode's landing page and this site's own link), and one `imeta` whose `url` starts with this deploy's own `/api/og/boost.png?` — a note naming another host is not one this app built. The check is `httpUrl(v) !== null`, not `=== v`, because `bmbLandingUrl` does not encode the guid it embeds. Every POST route also refuses a body without `Content-Type: application/json` (`requireJsonBody`, 415): a cross-origin `<form enctype="text/plain">` reaches a route without a preflight, and while no cookie makes that worth anything today, forcing the preflight is one line. The allowlist closes the vector actually worth attacking: an `e` tag. A boost note never has one, and with one a signed event from the site key appears to **reply** to any note in the world. **If `buildBoostNoteTemplate` gains a tag, add it here in the same change** or site-signed notes start failing.
...
`X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: strict-origin-when-cross-origin`, `Strict-Transport-Security: max-age=63072000; includeSubDomains` (Vercel sets HSTS on `*.vercel.app` only, and the app serves from a custom apex), `Cross-Origin-Opener-Policy: same-origin-allow-popups` (Google Sign-In's popup reports back through the opener; plain `same-origin` severs that), `Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=(), usb=(), serial=(), bluetooth=(), midi=()` (the last five because an origin holding a spending credential should not expose the Payment Request API or a hardware bridge to injected script, and nothing here uses them), and a deliberately partial CSP: `base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'` — `form-action` because an injected `<form>` is the one exfiltration route that needs no script, and `connect-src` cannot be constrained.
...
`npm audit --omit=dev` reports **2** (one high, one moderate), both the `postcss@8.4.31` that `next@15.5.24` vendors under its own `node_modules` (`npm ls postcss` shows the app's own at 8.5.x). **`next@15.5.24` is the newest release on the 15.x line** — the fix exists only in Next 16, so "run `npm audit fix`" does not resolve it; a major upgrade does. That is its own branch, not a line in a hardening pass, and `CLAUDE.md` warns that bumping Next has previously dragged in a `nostr-tools` upgrade that breaks `nostrconnect://` login. Re-run the audit when editing this section; the count here goes stale.
...
- **`sharp` / libvips is NOT unreachable, and this section used to say it was.** `/_next/image` sees only `public/hero.jpg` (the `images` block carries `formats` and no `remotePatterns`, which is why the wildcard must never return) — but `app/api/art` hands sharp a body fetched from a feed-supplied URL on every cover it resizes, the app's busiest route. Keep `sharp` current on its own merits; the art route's own guards (12 MB cap, `limitInputPixels`, a 3 s `.timeout`, a `Content-Type` allowlist) are the ones that matter.
...
### Feed XML and show notes are SCANNED, never regexed with a `[\s\S]*?` or `[^>]*` body

`/api/feed?url=` (and search, by-guid, publisher, playlist) fetches a caller-chosen URL, caps it at 8 MB, and then parses it. Every parser used to walk the document with regexes of the shape `<item\b[^>]*>([\s\S]*?)<\/item>`, and every one of those is **O(n²) on a document of repeated open tags with no close tag**: a regex retries from each candidate start, and each attempt scans to the end and fails. Measured: 800 KB of `<!--` took **38.7 s** in the sanitizer's comment strip, 720 KB of `<script >` 10.8 s, 420 KB of `<item >` 6.3 s — and the 8 MB cap extrapolates to tens of minutes of pinned CPU for one unauthenticated request, cache-bustable by varying the query string. The whole "Response size caps" section above is about MEMORY; nothing bounded parse COST, and the byte cap was precisely what made it exploitable. The `[^>]*` family has the same shape with the `>` missing instead of the close tag.

**The fix is a forward scanner, not a better regex** — `findTags`, `firstTag`, `findBlocks`, `firstBlock`, `stripBlocks`, `stripComments`, `splitAnchors` in `lib/feed-xml.ts`, and every parser in `lib/pi.ts`, `lib/feed-xml.ts` and `lib/musicl-resolver.ts` goes through them. A scan cannot retry: it finds the next open tag with `indexOf`, walks the attributes once (quote-aware, so `feedGuid="a>b"` reads correctly, which the regexes got wrong), finds the close with `indexOf`, and resumes AFTER the hit. Two rules make it linear and both fail closed: an open tag with no `>` STOPS the scan, and a block with no close tag STOPS the scan — nothing later could complete either, so the answer is fewer tags, never a wrong one. A malformed feed loses its tail instead of costing a request its lifetime.

Three more things ride on it. **`escapeDanglingLt`**: the sanitizer keeps two greedy `[^>]*` regexes (the allowlist tag pass and `mapNotesText`'s tag split), and they are linear only when every `<` has a `>` somewhere after it, so every `<` after the document's LAST `>` becomes `&lt;` first — and again inside `mapNotesText`, because the tag pass can delete that last `>`. **`MAX_SHOW_NOTES_CHARS`** (512 KB) truncates the notes before any pass; PI itself caps a description near 3 KB, so nothing real is lost. **`MAX_RSS_ITEMS`** (5000) bounds how many `<item>` blocks are walked — 5000 rather than `PI_EPISODE_MAX`, because an oldest-first feed longer than PI's cap would otherwise lose enrichment for its newest episodes.

Pinned by `npm run check:feedscan`: parity vectors (the scanner and the old regexes agree on ordinary feeds), bite vectors (the regexes were wrong — `<item-decoy>`, a `>` in a quoted attribute, `</item >`, an unclosed `<script>`), and timing vectors (1 MB of `<!--` under 250 ms, with the regexes deliberately NOT run on them). **Never add a `[\s\S]*?` or `<tag\b[^>]*>` regex over feed data or show-notes HTML**; if a new tag needs reading, it goes through the scanner.
