...
**On that route the list is not height-capped, and from `lg:` the heading takes a sticky left column.** It kept the panel's `max-h-80` inner scroll after the other mounts went, so a desktop page of its own showed six rows in a 320px box beside an empty right half. The list stays ONE column at every width — the order is the data.
