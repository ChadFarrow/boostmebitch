...
**`<PlaylistsLink>` is `hidden lg:inline-flex`, and `sm:` was the obvious breakpoint and the wrong one.** A `.btn-ghost` is `px-4` + `text-sm` + `tracking-wider` over an uppercased word, so the chip is **140.77px** — a peer of `<FavoritesLink>`'s 134.25px, not a glyph. Measured under CDP on the bmb brand, signed out: the header has **zero slack from `sm:` up to ~810px**, the wordmark already taking exactly the 169px it needs, so at 640px this chip took "Boost Me Bitch" down to **31px** and at 768px to 159px. It fits from 820px, which is why the gate is `lg:` rather than `md:`. Below `sm:` it is worse again — 141.67px in a 141.6px box on bmb, 154.03px in 154.03px on buddy. **Re-measure on the buddy brand before lowering it**; the bmb wordmark is the shorter one and hides the failure. Below `lg:` the way in is the hero's BROWSE PLAYLISTS button. That is **not** one control in two places: the hero button is discovery and hides the moment the visitor searches, the header chip is navigation and persists across `/favorites`, a search and a drilled-in show. Both are a bare `<Link href="/playlists">`, so there is no logic to drift. Adding nothing below `sm:` is also what leaves `--app-header-h` — and `<EpisodeList>`'s sticky offset on the other route — untouched.
