...
**Medium comes from `groupByMedium`'s own output, never a hand-written list.** Running the real grouper and rendering its buckets is what keeps `MEDIUM_ORDER`'s ordering, the feed-supplied label that is never normalized beyond lowercasing, and, the one that matters, **`~unknown` as its own bucket**. An entry whose medium nobody has declared is not a podcast, and folding it into one makes it findable only by accident. The segments *replace* the stacked medium headings rather than nesting inside them: a "Music" segment above a "Music —" heading is the same word twice, which is the two-lists-for-one-thing mistake the tracks-vs-chapters section below records.

**ONE segmented row, clustered by medium, and every segment sets BOTH axes.** It was two rows — a medium tab strip, and under it a half row whose contents changed with the tab — and a control that reshapes when you press it is a control you cannot learn. A single word for a half across the whole library can only be a compound ("albums & shows"), which is the thing being fixed, so a half is always named INSIDE a medium: `ALL · MUSIC · ALBUMS · TRACKS · PODCAST · SHOWS · EPISODES`, and `SHOWS` sets `{tab: 'podcast', split: 'feeds'}` in one update.

**Each cluster opens with the medium itself, and that segment is not decoration.** It is the "all of this medium, both halves" state. `bmb:fav_view` is a device setting rather than a cache — deliberately absent from `EVICTABLE_PREFIXES` — and every medium press in the two-row version wrote `{tab: '<medium>', split: 'all'}`, so that value is on real devices. Without a segment for it the strip renders with **nothing active** over a list still filtered to one medium, and nothing writes `split` back, so it never heals. A second way in needs no storage at all: pick MUSIC | TRACKS and unfavorite the last music track — the medium still has feeds so the tab survives, the `music|items` segment is filtered out by `itemCount > 0`, and `view.split` still says `items`.

**So the two halves of the rule move together.** A half segment is emitted **only where the medium has both** — with one half it would select the same rows as the medium segment — and `split` is DERIVED to match: `tab === 'all' || !tabHasBothHalves || view.split === 'all' ? 'all' : view.split`. Every reachable stored state lands on exactly one segment. The count rides on the medium segment, which is where the deleted medium strip carried it.

The segments are built from `tabs`, so only media the user actually has appear — no hand-written list of media, same rule the tab strip had. **A pair with no rows gets no segment**, which is why `tabs` carries `feedCount`/`itemCount` and not just a total: `~unknown` routinely holds items and no feeds, and a segment filtering to an empty section is worse than an absent one. **`crossSplitLabel` gives the bare noun only to `music` and `podcast`** — `feedNoun` collapses every other medium to `show` and `itemNoun` to `episode`, which is right for a lone label under a tab that already names the medium and is a *collision* here, where an audiobook and a podcast would render two identical `SHOWS` buttons filtering to different lists. Everything else is qualified by its own bucket label and takes the neutral half-word, and `medium unknown feeds` is deliberately not `medium unknown shows`. The section headings keep the compound under `ALL`, where it is accurate: that section really does hold both.

**The half segments name the medium's own nouns, and the section headings under them use the same two words.** They shipped as a fixed "albums & shows" / "tracks & episodes", which crams two concepts into one box on a library that is overwhelmingly one medium — 258 music of 277 on the reference list, where every chip said "& shows" about seventeen podcasts sitting on a different tab. `splitLabels(tab)` (`components/lists/grouping.tsx`) returns one noun per half whenever the tab names a medium (MUSIC → albums / tracks, PODCAST → shows / episodes) and keeps the compound for the two keys that name none. **`all` and `~unknown` keeping the compound is the point, not a fallback left unfinished**: `all` really is mixed, and `~unknown` is the bucket for entries nobody declared a medium for, so `shows`/`episodes` there would assert exactly what `groupByMedium` keeps that bucket separate to refuse. It is one call returning both words because the segment and the heading are one row apart — a segment reading ALBUMS above a heading reading "albums & shows" reintroduces the confusion one level down. Note it is deliberately NOT `feedNoun`/`itemNoun`: those label a "show N more …" control that already has a heading above it saying which half it belongs to, so they collapse to the generic `favorites` on a mediumless key, while these two chips sit side by side and are the only thing telling the halves apart.
...
`<SearchTypeLanes>` — a row of `ALL · ♫ MUSIC · PODCASTS · PLAYLISTS · ⚡ NPUB` under the input, active one in bolt. It sits **below** the npub suggestion and the `nsec` warning, not between them and the box: those two are `border-t-0` and hang off the field with no gap so the three read as one control, and the lanes are a separate row beneath all of it. Inside the box the `×` already owns the right edge of a field that is 288px wide at 320px, so nothing may be added there.

**Three answers, and the third is the row. It shipped as five chips, which wrapped to two lines at 320px** — a filter standing taller than the search box it filters. A dropdown fixed the wrap by folding the choice away, and that cost the thing the control is for: the one fact a visitor most needs about this box, that it finds music and playlists and not just podcasts, sat behind a tap. It also put the current mode **above** the input while the results and the explainer hang **below** it, so one control spanned both sides of the thing it acts on. The row keeps both properties the dropdown bought — every lane visible, one line at every width — because it **scrolls sideways instead of wrapping**, with `overscroll-x-contain` for the reason every rail in this app carries it. It is `role="radiogroup"` / `role="radio"` / `aria-checked`, **not** a tablist: one choice out of a fixed set that re-runs the same query, not pages of a document.
...
**`<Chip>` is shared now** (`components/chip.tsx`), lifted out of `<FavoritesPage>` where it was module-private; the empty state's way back to ALL is one. Its `px-2.5 py-1 text-[11px]` puts it at 27px, over WCAG 2.5.8's 24×24 floor — measure under CDP rather than estimating, per the favorites-page note above. The lanes are `h-8`, which clears the same floor and is **short of the 44px the rest of this redesign holds**; see the open items at the end of this file.
...
- **`<UnderlineTabs>` and the favorites row declare `role="tablist"`/`role="tab"` without the keyboard contract**: no `aria-controls`, no `role="tabpanel"` on the panels, no roving `tabIndex`, no arrow keys. That is a step back from plain unroled buttons, which promised nothing. The favorites row is a **filter**, so it should follow `<SearchTypeLanes>`' `role="radiogroup"` instead; the two content strips should either complete the tab pattern or drop the roles. One shape, three roles, is the drift `<UnderlineTabs>` was extracted to stop.
- **The search lanes are `h-8` (32px).** Over WCAG 2.5.8's 24×24 floor and short of the 44px the rest of this redesign holds.
