...
**Where the wallet opens from, now that the dock has no Wallet tab — and the one gap, accepted.** The header's balance chip (or *Connect wallet*) on every `<AppHeader>` route — `/`, `/live`, `/favorites`, `/playlists`, `/downloads`, `/queue`; and the ⚡ WALLET button in every boost modal, in both states. **The fullscreen player's bar does NOT carry one any more.** `<AuthControl overlay>` renders the wallet chips for nobody: the boost modal opens from the same screen and says *"⚡ NO WALLET — CONNECT ONE"* when there is none, so the bar was a second control for one modal on the app's most crowded bar — ← BACK, the ⋯ menu and ✕. Its SIGN IN entries stay, because #413's gap was about those too: a Nostr login with Google in reach, on routes that render no header. **`/npub/<npub>` and `/privacy` have no trigger while nothing is playing and no boost is open.** That was decided on 2026-09-19 when the queue landed, in exchange for the dock in the reference screenshot; neither page is a place a wallet is needed without one of the other two surfaces in reach. Nothing there sets `walletOpen`, so it is a missing control, not a dead one.
