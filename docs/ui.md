...
**`/playlists` has exactly two ways in, and they are different controls.** The hero's BROWSE PLAYLISTS button on `/` is **discovery**: it renders at every width and hides the moment the visitor searches or opens a show. The link in `<FavoritesPage>`'s header is **navigation**: it is there whatever state the library is in, and it sits above that page's loading / empty / rows split so it is reachable in all three. Playlists are deliberately **not** a tab — they are content, not a destination. Both are a bare `<Link href="/playlists">`, so there is no logic to drift. **Deleting either leaves a route with no way into the collection**, which is exactly what happened when the header chip went and only the hero button was left: from `/favorites`, from a search, or from a drilled-in show there was no route at all, and the search box's Playlists *lane* is a search mode rather than that page.
...
**Playlists is deliberately not a tab.** Playlists are content, not a destination; the two ways in are documented in the playlists section.
