...
- **`<UnderlineTabs>` and the favorites row declare `role="tablist"`/`role="tab"` without the keyboard contract**: no `aria-controls`, no `role="tabpanel"` on the panels, no roving `tabIndex`, no arrow keys. That is a step back from plain unroled buttons, which promised nothing. The favorites row is a **filter**, so `role="radiogroup"` / `aria-checked` is the honest pattern for it — the same one `<SearchTypeMenu>`'s items use; the two content strips should either complete the tab pattern or drop the roles.
