...
**Anything that draws ❚❚ must pause, and the glyph is gated on `isPlaying` — never on identity alone.** Three surfaces shipped calling `play()` unconditionally: the episode row's artwork (`components/lists/episode-list.tsx`), the fullscreen album tracklist, and the live-stream card's PLAY button. **A press on the item already playing was a total no-op**, not a mis-fire: `play()` writes `isPlaying: true` over `true`, so zustand emits no change and `<Player>`'s `[isPlaying]` effect does not re-run, while `current.episode.id` is unchanged so the source effect does not either. The `positionSec: 0` it also writes is a store mirror the next `timeupdate` overwrites — so nothing moves, nothing errors, and the control reads as dead. The branch is `if (current) togglePlay(); else play(…)`, which the header cover above, `<EpisodeDetailView>` and `<TransportControls>` already had.
...
**On the live-stream card the toggle also fixes the resume**, and that is why it uses `togglePlay()` rather than re-calling `onPlay()`: the `[isPlaying]` effect flags a paused live item and bumps `reloadNonce` on the way back, so the stream re-sources at the **live edge** instead of resuming from a point that has since gone stale.
