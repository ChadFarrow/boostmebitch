...
- **Right pane:** title/seek → control row (`<TransportControls size="lg">` + a `flex-1` `⚡ BOOST`) → **from `lg:` only, the ⋯ menu's actions as ONE row of five tiles** (the ⋯ trigger and its popover are `lg:hidden`, the row `hidden lg:flex`). The menu exists because a long title pushed the tiles off a phone's bottom edge; beside a side-by-side cover the pane has the height, and a 224px popover in the corner of a 1440px screen read as unfinished. **The row is shorter than the menu on purpose**: the two SHAREs are one tile opening a two-item list (`<ShareMenuTile>` — still two controls, each naming its target, so the `<ShareTargets>` rule holds), and SPEED, 3.5× and 5× are one tile opening every rate (`<SpeedMenuTile>`, one press to any speed, which is what the fast tiles were split out for). Eight tiles on the screen read as a lot of buttons for two settings. Both open through `<TileMenu>` (`components/player/tile-menu.tsx`), at the ⋯ menu's z-[55]. The phone menu keeps its eight → value-split disclosure → **Up Next** (the listen queue, when it holds anything) → **album tracklist** for music (`Album · N tracks`, `episodeQueue` clickable, current highlighted, `max-h-80` scroll) → `<EpisodeInfoPanel>` → `socialInteract` thread. **For Nostr live streams the right pane is replaced by the live chat.**
...
**On that route the list is not height-capped, and from `lg:` it takes the page's full width.** It kept the panel's `max-h-80` inner scroll after the other mounts went, and the route kept a `max-w-3xl` cap, so a desktop page of its own showed six rows in a 320px box in the left half of the screen. A sticky left column for the heading was tried and rejected on sight: it still left the list in part of the screen. The list stays ONE column at every width — the order is the data.
