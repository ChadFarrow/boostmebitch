...
`⇩ DOWNLOAD` writes the whole library to a JSON file. The pure half — the document and its filename — is `lib/favorites-export.ts`; `components/favorites-page.tsx` owns only the Blob and the `<a download>`. Before it existed the only routes out were a `localStorage` read in a developer console and a raw kind:10333 fetch, neither of which is a thing to tell a listener with two hundred saved albums.

**It exports the STORE maps, never the filtered view.** The page hands it `feedRows`/`itemRows`, not `feeds`/`items`. Those two pairs are one `.filter()` apart and the wrong one type-checks perfectly — a file named "my favorites" holding whichever tab happened to be open. It is the same class of lie as a section heading whose count shrinks to match the visible slice, one step worse: the reader opens the file weeks later with no filter row on screen to explain the gap. Which is also why the control sits beside the `N saved` count and above the filter row. The count is what the file is a copy of, and the two numbers have to agree.

**The file carries the NIP-73 identifier, not just the render fields.** The favorite IS the guid — `title`, `author` and `image` are whatever Podcast Index answered, and an `<UnresolvedFavoriteRow>` has none of them. A `JSON.stringify(favorites)` therefore writes rows no other app can open, and writes them blank for exactly the independent releases this app exists to pay. `showId`/`itemId` are the same helpers the kind:10333 writer uses, so a row is named in the file the way it is named on the shared list.

**`addedAt: 0` becomes `null`, never the epoch.** Zero means "not known yet" in the store (see the field's note in `lib/types.ts`), so writing `1970-01-01` into a dated field invents a fact, and a reader sorting on it buries every unresolved entry under a date nobody chose.

**"Is every favorite here" and "does every favorite have a title" are two questions, and the file must answer both.** `complete` is membership; `counts.unresolvedFeeds` / `counts.unresolvedItems` is description. They settle at different times, and the gap is wide enough to have shipped a wrong file: `setFavoritesSync('ok')` runs in `lib/nostr/favorites-hydrator.ts` at the moment the relay read is trusted, which is BEFORE the paint and before both Podcast Index resolve passes have written a single title back. So a download taken in that window is a complete list of bare guids — every `title`, `author`, `feedUrl` and `image` null, every `addedAt` null because a wire-seeded placeholder is `{ id: 0, podcastGuid, medium, addedAt: 0 }` — and the first version of this file printed `complete: true` and *"The full favorites list"* over it. Reported from a real account: **286 of 286 rows**, on a phone that had just adopted the list off the relays. The counts are taken off the EXPORTED rows rather than the store entries, so the number can never disagree with the nulls the reader is looking at, and `resolutionClause` ends by saying those entries are still complete — without that sentence the count reads as data loss, which is the opposite of the truth. It prints nothing at all when everything resolved.

**A snapshot that may be incomplete says so ON THE CONTROL, and again inside the file.** `complete`, `sync` and a one-sentence `note` are top-level fields, because the person who opens the file has no `<FavoritesSyncNotice>` on screen to tell them the relay read had failed — the notice is on a page they are no longer looking at, possibly on a device they no longer have. The button says it too, as a `(this device)` suffix rather than only a `title`: a tooltip is invisible to a touch user, and this is the one moment the caveat is actionable, since waiting a second or pressing the notice's retry is what makes the file whole. `exportIsComplete` is the single predicate. Signed out is COMPLETE, for the same reason `<FavoritesPage>` excludes it from `checking` — with no key there is no read to be waiting on. `'off'` is complete too, and the note rather than the flag is what explains it: the user chose device-only storage, so the device does hold the whole list this app knows about. `'idle'` and `'loading'` are both incomplete, which is the pair a user reaches by pressing ♥ in the header and going straight for the download.

**`<DownloadFavorites>` reads `identity` and `favoritesSync` from the store itself** rather than taking them as props. Both are what decide `complete`, and a prop is what a future surface forgets to pass — which here would not render a stale label, it would write a file confidently claiming to be a whole library it is not. Same reasoning that keeps `onFavoritesModeNeeded` in one place.

Two browser details that are not decoration. The anchor is **appended to the document before the click**, because Firefox ignores a click on a detached one and the download then silently never starts. The object URL is revoked on the **next macrotask**, not in the same one, where revoking can cancel a download the browser has not yet taken — a `setTimeout` that touches no React state, so an unmount in between needs no `clearTimeout`. A refused `createObjectURL` (private mode, a blocked store) renders its message beside the button rather than doing nothing, per the button-that-freezes rule in [`storage.md`](storage.md).

Measured under CDP at 390px with `mobile: true`, five favorites seeded: the button is **112×34** normally and **213.6×34** with the `(this device)` suffix, both clearing the 24×24 floor, and the header row wraps with **zero horizontal overflow** in either state.
