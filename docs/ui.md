...
**Medium comes from `groupByMedium`'s own output, never a hand-written list.** Running the real grouper and rendering its buckets is what keeps `MEDIUM_ORDER`'s ordering, the feed-supplied label that is never normalized beyond lowercasing, and, the one that matters, **`~unknown` as its own bucket**. An entry whose medium nobody has declared is not a podcast, and folding it into one makes it findable only by accident. The filter's options *replace* the stacked medium headings rather than nesting inside them: a "Music" option above a "Music —" heading is the same word twice, which is the two-lists-for-one-thing mistake the tracks-vs-chapters section below records.

**ONE control, clustered by medium, and every option sets BOTH axes.** It was two rows — a medium tab strip, and under it a half row whose contents changed with the tab — and a control that reshapes when you press it is a control you cannot learn. A single word for a half across the whole library can only be a compound ("albums & shows"), which is the thing being fixed, so a half is always named INSIDE a medium: `all`, then `music · albums · tracks`, `podcast · shows · episodes`, and picking `shows` sets `{tab: 'podcast', split: 'feeds'}` in one update. **It is a `<SelectMenu>`, not a strip** — the segmented version clipped mid-word at 390px, and the reasoning is in that widget's section.

**Each cluster opens with the medium itself, and that entry is not decoration.** It is the "all of this medium, both halves" state. `bmb:fav_view` is a device setting rather than a cache — deliberately absent from `EVICTABLE_PREFIXES` — and every medium press in the two-row version wrote `{tab: '<medium>', split: 'all'}`, so that value is on real devices. Without an entry for it the control names a filter that is not in its own list, and nothing writes `split` back, so it never heals. A second way in needs no storage at all: pick `music · tracks` and unfavorite the last music track — the medium still has feeds so the tab survives, the `music|items` entry is filtered out by `itemCount > 0`, and `view.split` still says `items`.

**So the two halves of the rule move together.** A half entry is emitted **only where the medium has both** — with one half it would select the same rows as the medium entry — and `split` is DERIVED to match: `tab === 'all' || !tabHasBothHalves || view.split === 'all' ? 'all' : view.split`. Every reachable stored state lands on exactly one option. The `id` is `<tab>|<split>` so the two axes travel as one value, and the count rides on the medium entry, which is where the deleted medium strip carried it.

The options are built from `tabs`, so only media the user actually has appear — no hand-written list of media, same rule the tab strip had. **A pair with no rows gets no option**, which is why `tabs` carries `feedCount`/`itemCount` and not just a total: `~unknown` routinely holds items and no feeds, and an option filtering to an empty section is worse than an absent one. **`crossSplitLabel` gives the bare noun only to `music` and `podcast`** — `feedNoun` collapses every other medium to `show` and `itemNoun` to `episode`, which is right for a lone label under a tab that already names the medium and is a *collision* here, where an audiobook and a podcast would render two identical `SHOWS` buttons filtering to different lists. Everything else is qualified by its own bucket label and takes the neutral half-word, and `medium unknown feeds` is deliberately not `medium unknown shows`.

**The mixed tab emits one section per (medium, half) pair, not two compound ones.** `ALBUMS & SHOWS — 217` and `TRACKS & EPISODES — 227` sat directly under a filter that had already divided the same library into `MUSIC 421` and `PODCAST 23`, so the page said in one row what it refused to say in the next. **A compound heading is survivable; a compound COUNT is not** — 217 answers neither "how many albums" nor "how many shows", and those are the only two questions the heading raises. `mediumSections` runs `groupByMedium` over each half and names the results with `crossSplitLabel`, the same call the filter labels its own entries with, so a heading and the option that selects it cannot drift apart. A tab that names a medium is unchanged: two sections, `splitLabels`' two words, because there the medium is already stated above.

**It is built from the rows the filter LEFT, and that is what keeps it from being worse than the compound.** A medium the title filter emptied emits no section, rather than a column of `— 0 of 194` headings one per medium. `ofTotal` still comes from the unfiltered per-medium count, off the same `feedCount`/`itemCount` the filter options use — so `SINGLES — 1 of 5` and the menu's `SINGLES 5` are one number. The count is data-driven, so "four sections" describes a music-plus-podcast library and not a rule: an audiobook feed adds a fifth, and `~unknown` adds its own two.

**`~unknown` takes `crossSplitLabel` under its OWN tab too, and that is the drift this change would otherwise have created.** `splitLabels` hands that key the compound, which was harmless while `all` had it as well; once the mixed tab started saying `MEDIUM UNKNOWN FEEDS`, the same rows under their own tab would still have read `ALBUMS & SHOWS`. One of those names a bucket that exists *because* nobody declared a medium, and the other asserts two. A real medium keeps `splitLabels` — under an audiobook tab, `shows` is right and `audiobook feeds` is not.

**The fold key follows the section, not the half.** A medium tab folds under `favpage:feeds` / `favpage:items`, because there the fold means "hide the albums". The mixed tab folds per (medium, half) under `favpage:all:<medium>:<half>`, which is a different statement and gets a different key. Neither reuses the old `show:<medium>` / `ep:<medium>` keys — those named the stacked medium headings this page replaced, and a key that outlives a redesign folds a section its author never saw.

**The "show N more …" noun follows the SECTION's medium now, not the tab.** Under `all` the tab knows no medium, so the control said "show 12 more favorites" under every heading; a section does know one, so the control under ALBUMS offers albums. `~unknown` still resolves to the generic word, and that is not an unfinished fallback — it is the same refusal `groupByMedium` keeps the bucket separate to make.

**The filter's half entries carry counts.** They had none while the medium above them did, so the menu read `MUSIC 421` over two blank rows — withholding the one number that says how the 421 divides, in the control whose job is to divide it. They are the same `feedCount`/`itemCount` the section headings stand on.

**The half options name the medium's own nouns, and on a medium tab the section headings under them use the same two words.** They shipped as a fixed "albums & shows" / "tracks & episodes", which crams two concepts into one box on a library that is overwhelmingly one medium — 258 music of 277 on the reference list, where every chip said "& shows" about seventeen podcasts sitting on a different tab. `splitLabels(tab)` (`components/lists/grouping.tsx`) returns one noun per half whenever the tab names a medium (MUSIC → albums / tracks, PODCAST → shows / episodes) and keeps the compound for the two keys that name none. **The compound is the point where it labels a FILTER, not a fallback left unfinished**: `all` really is mixed, and `~unknown` is the bucket for entries nobody declared a medium for, so `shows`/`episodes` there would assert exactly what `groupByMedium` keeps that bucket separate to refuse. It is only as a section HEADING that it stopped being enough, for the reason above — a filter's number is the whole of what the filter selects, and a heading's is not. It is one call returning both words because the option and the heading it selects use the same vocabulary — an option reading ALBUMS over a heading reading "albums & shows" reintroduces the confusion one level down. Note it is deliberately NOT `feedNoun`/`itemNoun`: those label a "show N more …" control that already has a heading above it saying which half it belongs to, so they collapse to the generic `favorites` on a mediumless key, while these two chips sit side by side and are the only thing telling the halves apart.
...
**The scrub bar was the worst target in the app, and it did not look like it.** `input[type='range'].seek` is `appearance: none` with a 2px track and a 12px thumb hanging outside the box on a negative margin — so, measured in Chromium against the shipping rules, the **element was 2px tall**. Tap-to-seek anywhere along the track had a 2px target on a control that is a primary action in a podcast player, and reading the CSS suggests 12px, not 2. It now takes `height: 32px` inside the existing `@media (hover: none) and (pointer: coarse)` block, with the thumb at 16px and `margin-top` re-derived rather than nudged — `(track − thumb) / 2 = (2 − 16) / 2 = −7px`. **32 and not 44 because the element's height IS the row's height**, and that row is in the mini-bar, the most contested vertical space in the app: 44 would add 33px to a ~72px bar, 32 adds ~21px and still clears the 24px floor with margin. Giving it a real height is also the only fix that steals nothing — a negative margin would reclaim the strip holding the transport row and the chapter label, the trade `streaming-settings.tsx` already refuses. **A mouse gets a box too, and nothing moves:** the default rule is `height: 14px; margin-block: -6px`, so the margin box is still 2px and the mini-bar row (15px, set by its time labels) and the fullscreen pane (6px gap below, 14px or more above) are unchanged — measured identical to `main` on both players at 1200×900, and at 390×844 under touch emulation. Without it the chapter hover tip appeared and vanished as the pointer drifted a pixel. It is the DEFAULT, not a `(hover: hover)` block, so every device gets exactly one of the two rules; headless Chrome matches neither `hover: hover` nor `pointer: coarse`, and CDP cannot emulate either. The touch block resets `margin-block: 0`.

**The mini-player bar is not a `role="button"`, and it was twice.** First it had the role and no `tabIndex` or key handler: announced as a button, never reachable by Tab, inert when activated. Then it had all three, and was still wrong — ARIA makes a button's children presentational and its `aria-label` names the whole subtree, so a screen reader could present the bar as ONE button called "Open fullscreen player", with the seek input, the transport and BOOST inside it unreachable. A native `<button>` cannot wrap them either. The way in is now a real `<button>` on the episode title, a SIBLING of those controls (the rule below at "a real `<button>` with the other control as its SIBLING"), and the bar's `onClick` stays as a pointer-only hit area like `<ModalShell>`'s backdrop. The title button carries `-my-1 py-1` so its target clears 24px without moving the row.
...
### Backing up the list (`⇩ BACKUP`)

The control saves the **kind:10333 event itself**, exactly as the relays hold it — the seven NIP-01 fields, nothing added. `lib/favorites-export.ts` owns the refusal rule, the serializer and the filename; `components/favorites-page.tsx` owns the relay read, the Blob and the `<a download>`.

**It shipped as something else first, and the replacement is the interesting part.** The original wrote a document assembled from the store: every favorite with its title, author, artwork, feed URL and `addedAt`, plus `sync`/`complete`/`note` fields describing how much of it to believe. Two things were wrong with it, and neither was visible until a real library went through it.

*It could not be restored.* kind:10333 is one replaceable event per pubkey that every writer overwrites wholesale, so the only thing worth saving is the thing that can be put back — and a rebuilt list cannot be: `id` is a hash over the whole event and `sig` needs the secret key. What that file held was a transcript. For the one job a backup has, insuring against another app's blind publish, it was useless.

*And most of what it added was never on the relays.* Title, author, artwork and feed URL come from Podcast Index; `addedAt` is when this device resolved the row. A file carrying them answers "what does my phone know" when the question was "what is stored". Those were also the fields that came back `null` — resolution runs after the read — so the additions were exactly the part that made the file look broken. **286 of 286 rows** on the account this was built against.

**A read it cannot trust writes NO file.** `backupRefusal` refuses rather than annotating, and the two failures are deliberately not treated as symmetric: a missing file sends the user back in a minute, while a file holding an *older* event than the relays now have is indistinguishable from a good one, and restoring it replaces the newer list wholesale — the exact loss the feature exists to insure against. That is the same rule the sync cycle runs on, one surface over: never write over what you could not read. Each refusal names what to do about it, because a control that declines without a reason reads as broken. Signed out it renders nothing at all — no key, so no event.

**It re-reads the relays through `fetchFavoritesList`, never the store.** The store is merged, resolved and device-local, which is the whole thing a backup must not be. Going through the sync path's own reader also carries its filter and its `dTag: ''` intake expectation, so this cannot drift into accepting an event the sync path would reject.

**It never decrypts.** A private half rides into the file as the ciphertext the relay served — which is both what a backup needs and what keeps a signer prompt, and the user's plaintext favorites on disk, out of a control nobody asked that of. `backupSummary` says so on screen, because that is the one thing a raw event hides: **a private list looks empty**, its entries being ciphertext in `content` behind a short tag list.

**The filename is `<site>-favorites-<event date>-<id prefix>.json`**, and both halves are for the case this exists to serve — keeping several backups and telling them apart. The date is the event's `created_at`, not today, so two downloads of an unchanged list produce one filename rather than a file per day of identical bytes; the first 8 of the event id separate two lists written on one day and let a file be matched against a relay without opening it.

**`.btn-mini` is the size this cluster is allowed to be.** 27px — an 11px line box, `py-1`, a hairline — over WCAG 2.5.8's 24×24 floor and the same height `<Chip>` was measured at, and deliberately **not** the 44px floor the primary action rows hold. The privacy menu and the three relay tools are rarely pressed, each has a confirmation behind it, and at 38px they made the controls describing the library taller than the first rows of the library. `<SelectMenu>`'s trigger uses the same class, so the menu and the buttons beside it are one family by construction rather than by two utility strings that agree today.

**The controls are TWO ROWS and the page is judged on how fast you reach a favorite.** They were six — a medium strip, a half row, a filter box, a sort pair, the privacy segments and their sentence, and the backup tools — and on a 287-entry library that was most of the first screen on a phone. Now: the privacy menu and the backup tools share one row with the consequence sentence beneath it, and the library filter, the text box and the sort toggle share the other. The `<SelectMenu>` is what buys the first and last of those; the numbers it folds away are on the trigger.

**`<FavoritesPrivacyControl>` is a `<SelectMenu>`, and the consequence sentence is NOT inside it.** The control was three segments spanning the page, and at 390px "Not on Nostr" wrapped inside its own segment — so the tallest thing above the library was the control naming where the library is stored. A menu is one line at every width. What must not follow it into the fold is the sentence: `describe(mode)` stays rendered beside the trigger, because **"private" and "not on Nostr" are both invisible from the outside** and a user who cannot tell which one they are in has no way to find out. A tooltip does not satisfy that on a phone. Two more properties the menu had to keep: `active` may be **null** — never chosen is not Public, and showing Public selected claims something about the user's data that is not true yet, which is why `<SelectMenu>` takes `T | null` and a `placeholder` — and the gated `private` option **renders disabled rather than absent**, with `GATE_REASON` as its title, because a control that hides what it is withholding cannot explain itself.

**It shares ONE full-width row with `<FavoritesPrivacyControl>`**, passed in as that component's `trailing`. They are one question — where this list lives, and how to get a copy of it out or put one back — and stacked as separate rows with the consequence sentence between them they read as unrelated controls. All of them are `.btn-mini`, so the row is one family; it wraps to a second line on the narrowest phones and still reads as one cluster. `<RelayTools>` returns a **fragment** for that reason: a wrapper of its own would make the three tools one flex item that wraps as a block instead of three controls that wrap individually. What has not changed is that the row is **full width**. These were beside `N saved` in the header first, on the reasoning that the file was a copy of that count — true of the store document, void of the event, and the refusal sentence then wrapped to four lines in a column two thirds of the page wide. It is deliberately **not** gated on `total`: this reads the relays, so a device holding nothing locally can still hold the account's list, and the empty branch is exactly where somebody checking what is stored would look.

Two browser details that are not decoration. The anchor is **appended to the document before the click**, because Firefox ignores a click on a detached one and the download then silently never starts. The object URL is revoked on the **next macrotask** — revoking in the same one can cancel a download the browser has not yet taken — via a `setTimeout` that touches no React state, so an unmount in between needs no `clearTimeout`.

Verified against a real relay and a real signer (`scripts/local-relay.mjs` plus a CDP-injected `window.nostr`, the `e2e:favorites` shape): the downloaded file carries exactly the seven NIP-01 keys, its signature verifies, and its id, tags and content are identical to the event the relay holds. Pointed at a dead relay, no file is written and the reason renders.

### Seeing and repairing the private half (`⌕ CHECK PRIVATE FAVORITES`)

Opens the encrypted half of the kind:10333 event, counts what is in it, and compares it with the plaintext tags. It **publishes nothing**. The arithmetic is `lib/favorites-audit.ts`; the decrypt and the rendering are in `components/favorites-page.tsx`, on the same row as `⇩ BACKUP` because both ask the relays about the list as STORED rather than as painted.

**ONE control, ONE read, ONE signer prompt — it was two controls and that is what made it unusable.** `<InspectPrivateHalf>` looked and `<MergeEncryptedHalf>` acted, each with its own relay read and its own decrypt over the same bytes, and the success message then sent the user back to the first one to confirm. Three approval sheets for one job, two panels whose numbers were computed differently and worded differently, and no sentence anywhere saying what to do about what had just been reported. `<PrivateFavoritesTool>` reads once and offers the action inside the finding it produced. **Building the plan from that earlier read is not weaker**: `syncFavoritesNow` re-reads before it publishes, and an entry another app adds in between is simply not claimed by the baseline, so it is carried rather than deleted — which is the direction this repair must fail in anyway.

**It is hidden unless it is needed, and the gate is a free observation rather than a guess.** `FavoritesRead.content` being non-empty answers "is there a private half" for nothing — it is a plain field of the event ordinary hydration already fetches — while "what is in it" is what costs an approval sheet. `hydrateFavorites` writes that answer to `favoritesPrivateHalf` in the store: `'none'` hides the control, `'present'` shows it, `'unknown'` shows it **only while `favoritesSync` is `'degraded'`**. That last clause is the whole care in it. A read that never answered is not evidence of an empty half, and hiding the one control that repairs a private half on the strength of a failed query hides it from precisely the person who needs it — the same silent-guard failure that governs the empty-library claim above. `engaged` pins the control open once pressed, or a repair that empties the half would unmount the message saying so. **This is not the 2026-09-03 deletion in another form**: that removed the capability, and the state it repairs recurred within hours on a fresh device. This removes it from the screen of an account the app has *observed* to have no encrypted half, and puts it back the moment one appears.

**A list can hold entries in both halves at once, and no screen in this app said so.** Nothing in the format forbids it, and three ordinary routes lead there: another app wrote the private half; a mode switch ran on a device whose baseline claimed nothing and therefore copied instead of moving; or the account changed mode twice. The public half then renders, the private half is carried byte for byte by the carry rule, and neither is counted anywhere. Found on a real account holding **38,320 characters of NIP-44 ciphertext beside 445 public `i` tags** while its mode read Public — and the ambiguity warning could not fire, because `modeAmbiguous` is gated on `!seeded && !recorded` and that device had a recorded mode.

**The decision it informs is what makes it worth a control.** Switching to Private moves the entries the device's baseline claims and copies the rest, so "is my list already in both halves" decides whether a switch tidies the list or doubles it. That is a publish to a shared replaceable event, so guessing is expensive.

**A group WITH items is not a favorite, here as everywhere.** A group is opened for every parent of a favorited track, so only an *itemless* one is an album or show the user chose — the rule `hydrateFavorites` already applies when it paints. Counting every `podcast:guid:` tag would have reported 217 albums to a user who has 56, and overstated the overlap between the halves by the same 161. `favoriteIds` therefore emits an itemless group's own id and otherwise emits its items, never both.

**The decrypt is `'user-initiated'` and that is why this is a button.** Everything stored encrypted-to-self is otherwise read on page load, before the user touches anything — which is why `decryptPrivate` defaults to false and `purpose` is a required argument. An external signer renders the plaintext on its approval sheet, so an unasked decrypt puts somebody's private favorites full-screen uninvited. The purpose is passed straight through from the click and never hardcoded further down, per the `fetchEncryptedMnemonic` rule in [`signers.md`](signers.md).

**And the copy names the RETRY first, because only one of those four causes is permanent.** A NIP-07 extension that goes away does not reject — it hangs, and `withDecryptTimeout` lands the hang in this same branch — so the common case is "press it again with your signer in front", not "your list is written in a cipher I cannot read". The first version said only the second thing, and it was read as a verdict about the list by the user whose list had decrypted fine ten minutes earlier. Both sentences ship, in that order. the repair step carries the same pair, because the same state blocks it.

**A refusal to decrypt is a FINDING, not an error.** Not every signer implements NIP-44, and the half may be another app's, written in a cipher this one cannot open. That state is exactly what makes a switch to Private impossible — `planFavoritesPublish` returns `private-unreadable` for it — so it is reported as the answer it is, with that consequence spelled out, rather than as a failure. An untrustworthy read is refused for the same reason `⇩ BACKUP` refuses: a confident "no private half" over a failed query is the one answer that would send somebody into a mode switch.

**Each item also names the album it came from, and that is a second lookup rather than a fallback.** "Which track is this" fails whenever Podcast Index has not indexed the item, which is the normal case for music — 224 of 228 item favorites on the reference account resolve to no title at all. "Which album is it from" needs only the FEED, which PI is far more likely to hold, so the parent is what actually lets someone recognise their own favorite. Both go through one `warmPodcastCache` call, the parents deduped against the feed favorites in the same batch. It cannot tell you a track was favorited from a PLAYLIST: a `musicL` container is never recorded as an item's parent (`containerIsParent`), because the playlist is not the track's publisher, so the album is the only origin the wire keeps.

**It names the private-only entries, and a count alone would not have been enough.** "3 entries exist only in the encrypted half" leaves nobody able to tell a favorite they deleted from one another app wrote, and that difference is what decides whether the half is safe to retire. Only the private-only ones are listed — the overlap is already on the page as rows. `parentFeedGuid` comes from the group each item sits under **in the private half**, which is the only place it can come from: Podcast Index resolves an item by `(podcastguid, guid)`, the wire records a parent as ADJACENCY rather than a field, and an entry that is private-only by definition has no group in the public half to read it from. Naming goes through the batch door first and then the per-entry resolvers (`<Podroll>`'s rule: the warm fills the memory the resolvers read, while keeping the one thing a batch cannot do), and it is capped at `NAME_LIMIT` — the list length is whatever another writer put in `content`, so an uncapped fan-out is an unbounded burst against our own per-IP limiter. The identifier is printed **even when a title resolved**: it is what the entry IS, and the only thing that can be matched against a raw event or handed to another app.

Verified against a real relay and a real NIP-44 signer, three seeded shapes plus a no-publish assertion: overlap (2 public, 2 private, 1 in both, 1 private-only, with the placement group excluded from both counts — and the private-only entry listed by identifier, titled from a seeded display cache, while the overlapping one is not listed), an empty `content`, and a `content` the signer cannot open. After all three the relay still holds exactly the one seeded event with its original bytes. The number-agreement bug that pass caught — *"1 entry sit in BOTH halves"* — is why `auditSummary` inflects the verb as well as the noun: a count of one is the ordinary case here, and bad grammar beside a number makes a reader distrust the number.

#### The repair (`MOVE THEM INTO MY PUBLIC LIST`)

**This repairs a stuck state; it is not a second way to choose a mode.** The mode switch already moves entries: on Public, `syncFavorites` sets `privateLocal = EMPTY_LOCAL` and merges the private half against it, so everything the baseline claims is dropped and `content` goes to `''`. All-in-one-half is the design, and the code implements it.

**What blocks it is the BASELINE.** The removal is gated on `baselineHalf(baseline, 'private')`, and a device that never published the private half claims nothing in it — so the merge drops nothing, and the ciphertext rides forward on every publish forever. Found on a real account at **284 entries in both halves at once**, with the mode reading Public and every screen in the app agreeing that everything was fine. **It recurred on 2026-09-04 on the same account** — 451 public `i` tags beside 38,320 characters of ciphertext — three hours after a backup that showed the list fully private, on a `localhost` dev origin: a separate origin is a FRESH DEVICE, with no baseline and no recorded mode, so the mode question was asked again and the answer copied instead of moving. That is worth knowing before testing against a real account, and it is why these controls were restored the same day they were removed. The guard is right to be there: a baseline is a promise that these entries are ours to delete, and this app will not delete a private half it cannot prove it owns. What was missing was the evidence — which is what `⌕ CHECK PRIVATE FAVORITES` produces, and why the finding must come before the repair.

**Three steps, and the order IS the safety argument.**

1. **Adopt** the private-only entries into the local store. They are about to stop existing in the half that holds them, and the public merge publishes what the STORE holds — so an entry not adopted before the sync is *deleted* rather than moved. The `medium` hint travels with them, because for an entry Podcast Index cannot resolve it is the only description that will ever exist.
2. **Claim** the private half in the baseline, from the read just verified. The public claims are left untouched: this promise is about one half, and rewriting the other would disown entries this device published.
3. **Sync** normally. Nothing bespoke publishes: the ordinary cycle sees a store holding everything and a private baseline claiming the half, and converges by itself.

**What it deliberately cannot reach.** `baselineOfList` claims GROUPS only, so a loose node in the private half — an identifier kind outside our table, or an item that appeared before any feed group — is never claimed and therefore never dropped. Another writer's entry in a shape this app does not model survives the repair. That is the correct direction to fail in.

**It proposes before it acts, and the confirmation states the outcome in numbers** — how many favorites will be public afterwards, how many are being adopted first, how many were already in both halves — plus the one instruction that is only useful beforehand: take a backup. Emptying the encrypted half cannot be undone by re-running anything here, which is what the instruction is for — `⇧ RESTORE FROM BACKUP` is the only way back, and only from a file taken beforehand.

**A refusal is a `null`, not a throw.** `syncFavoritesNow` returns null without recording anything on a degraded read, a wholesale-delete guard, an unreadable private half, or a publish that reached no relay. `await` looks like success and `try/catch` catches nothing, so the return value is tested — telling someone their list was merged when it was not is the one error they cannot check without another round trip.

**And the success sentence is DERIVED from `note.event.content`, never assumed.** "The encrypted half is now empty" is a claim about bytes we have just published, and the published event is in hand — so it is read rather than inferred from the absence of an error. It matters because the claim can be false while the publish succeeded: `baselineOfList` claims groups only, so a loose node another writer left in the half is carried forward, `content` stays non-empty, and the honest report is *"published, but the encrypted half is not empty"*. Guessing here would send someone away believing a repair had finished that had not.

**The repair is REFUSED in Private mode, where it would do the exact opposite of its label.** `syncFavorites` in mode `'private'` sets `local = EMPTY_LOCAL` and puts everything the store holds into the encrypted half — so "move them into my public list" would have emptied the public tags instead, from a button whose own text promised the reverse. The mode switch is the right instrument there, so the panel says so and offers no button. This is why the finding reads `storage.favPrivacy` for the action gate as well as for `auditSummary`'s wording: one control, one reading of the mode.

**The visibility gate is asserted in both directions by `npm run e2e:favorites`**, against a real relay and a real NIP-44 signer rather than a seeded flag: after scenario 1 the account's `content` is `''` and the control must be absent; after scenario 2 it carries a private half and the control must be present, report the counts, and — the account being in Private mode there — offer no move button and say why. Both directions matter. A gate that only ever hides is indistinguishable from the 2026-09-03 deletion.

**What that pass does NOT cover is the move itself, and the reason is worth knowing before you go looking for a bug.** Reproducing a reachable stuck state inside the harness turned out to be impossible: seeding an event with both halves populated, clearing the baseline and setting the mode to Public, the app **repaired it by itself within five seconds of the page load** — one publish carrying four public `i` tags and an empty `content`, with nothing clicked. `listDecryptOnLoadOk` is true for a NIP-07 signer, so hydration decrypts the half and the ordinary cycle absorbs it. The button's action is therefore reachable only where the load does not decrypt — an Amber or bunker signer, which `unattendedDecryptOk` excludes on purpose — or where the half is in a cipher this signer cannot open. That narrows what this control is for; it does not remove it, and it is measured behaviour of the harness rather than a claim about every account. Flipping the mode key alone does not work either: `correctedModeFromWire` answers 'private' for a wire with a private half and no public tags, and corrects the setting straight back.

The earlier manual pass, against a real relay and a real NIP-44 signer, reproducing the stuck state (both halves populated, no private baseline): the proposal step publishes nothing and the seeded event is untouched; after merging, `content` is `''`, the private-only track has become a public `i` tag **under its own parent group**, the previously-public entries are intact, and the relay holds exactly one event. `⌕ CHECK PRIVATE FAVORITES` then reports no private entries. Both grammar slips in this feature's copy — *"1 entry sit"*, *"1 of them exist"* — were caught by that pass, which is why the summaries inflect verbs as well as nouns.

### Restoring a backup (`⇧ RESTORE FROM BACKUP`)

The other half of `⇩ BACKUP`, and the most destructive control on this page: it replaces the whole kind:10333 event, for every app that reads the account's favorites.

**It republishes the backup's CONTENT, not the backup event.** A replaceable event is superseded by `created_at`, so re-sending the original signed bytes would be ignored by every relay already holding something newer — which is exactly the case a restore is for. So it signs a NEW event carrying the file's `tags` and `content` verbatim, through `publishFavoritesTags`, which asserts the publish reached a relay. The signature in the file is therefore not what lands; it is what proves the file is genuine before we agree to publish it.

**Every check in `parseFavoritesBackup` is a refusal to publish something the user did not sign.** `verifyEvent` is the load-bearing one — it proves the bytes are a real event and that nothing in `tags` or `content` was altered afterwards, so a file edited in a text editor is refused even when the edit was well-meant. The pubkey must be the signed-in account's, because restoring someone else's list under your key replaces yours wholesale AND publishes theirs as yours. The kind must be 10333, because the app's other backups (wallet, settings) are addressable events at the same pubkey and publishing one of those as a favorites list destroys both. It deliberately does **not** compare `created_at` with the relays: restoring an older list over a newer one is the point of the feature, and whether that is wanted is the confirmation's question, not the parser's.

**It refuses on an untrustworthy read, for a different reason than everything else here.** Elsewhere the danger of a bad read is writing over data we could not see; here the write happens regardless, because the user asked for it. The read is what makes the CONFIRMATION honest — without it the panel cannot say what is being replaced, and a restore with no statement of the cost is the one thing this control must never be. The panel states both counts and both dates, and when the backup is smaller it states the difference **as a loss** ("that is N fewer than you have now") rather than as a diff, because arithmetic under pressure is how entries get thrown away.

**Afterwards the device is deliberately reset to a fresh-device state, and the ordering is the correctness argument.** The store and the baseline are cleared, `favCleared` is set false, and only then does `hydrateFavorites` run. Leave the OLD favorites in the store and the next cycle reads them as local additions and republishes them, silently undoing the restore; leave the OLD baseline and it claims ids the restored list does not have, so the next cycle publishes their removal. An empty store beside an empty baseline is the one combination meaning "adopt what the relay holds" — the path a new browser already takes — and `favCleared` has to go with them or the wholesale-delete guard reads the empty store as a deliberate clear-all.

Verified end to end against a real relay and a real signer, with the relays holding a NEWER list than the file: four bad files are each refused with their own reason and **none of them publishes anything** (junk, another account's backup, a kind:10000 event, and a backup with one tag appended — caught by the signature). A good file proposes without publishing, naming both counts and the loss. Restoring replaces the event, its tags are the backup's byte for byte, and it is signed fresh rather than re-sent. After a reload the device holds the restored favorites, its baseline claims what was published, and **the reload republishes and deletes nothing** — the assertion that actually proves the fresh-device reset is right. One expectation in that suite was wrong rather than the code: a baseline claims the WIRE's groups (an itemless favorite and a placement group), while the store holds FAVORITES, so the counts differ by design.

### OPML import and export (`⇩ OPML`, `⇧ IMPORT OPML`, `components/opml-tools.tsx`)

The subscription list other podcast apps read and write. **Show favorites only** — OPML has no standard way to name one episode or track, so episode favorites stay on the Nostr list, and the export says so. Rendered signed in (inside `<RelayTools>`) AND signed out (its own row, since `<FavoritesPrivacyControl>` renders nothing then); the store has a guest bucket.

The reader and writer are `parseOpml` / `buildOpml` in `lib/feed-xml.ts`, pinned by `check:opml`. They live there, not in a module of their own, because the reader must walk with `findTags` and a second module importing `feed-xml.ts` could not load under strip-types. `parseOpml` keeps http(s) URLs only (the URL is sent to Podcast Index and stored as the favorite's `url`, which renders) and counts the rest as skipped.

**The import proposes before it writes**, because the store is what gets published to the shared kind:10333 event: *N new · M already saved · K not found*, with the not-found feeds listed by name. A favorite is keyed by `podcastGuid`, so a feed Podcast Index does not index cannot be saved — it is **reported, never dropped unseen**. Confirming calls `addFavorite` per entry (the heart's own action, so `favCleared` is handled) and then `requestFavoritesSync`, which owns the privacy prompt and `'off'`. **Signed in, the button waits for `favoritesSync` to leave `'idle'`/`'loading'`** — painting into the store before hydration is how a planner mis-reads local changes. Lookups go warm-first (`warmPodcastCache` with `url:` keys, skipping any URL holding a comma because the batch route splits on commas), then `resolvePodcastByFeedUrl` under `mapLimit`. The heart and the import build the favorite with one function, `favoriteFromPodcast` (`components/fav-heart.tsx`).

## The playlists page (`/playlists`)

The curated collection used to be a state swap inside `<HomePage>`: **BROWSE PLAYLISTS** called `handleSelect` on a synthetic publisher stub, the results aside became the collection, and `history.replaceState` wrote `?publisher=<feedUrl>` behind it. So the app's one non-search surface had no URL anyone could arrive at deliberately and no exit the back button could reach — the same two faults that moved favorites off the home page. It is `app/playlists/page.tsx` + `components/playlists-page.tsx` now. The aside on `/` **stayed**, because it still has a job: a third-party publisher feed found by searching, and every `?publisher=` link already shared. Which is why the fetch moved down into `lib/playlist-collection.ts` instead of moving with the button.

**Opening a playlist is the store-then-navigate handoff, all three halves** (`<FavoritesPage>` is the reference, and the rule is in `CLAUDE.md`): `selectPodcast(p)`, then `setShowOrigin({ path: '/playlists', label: 'playlists' })` — **after**, never before, since `selectPodcast` clears it — then `router.push('/')`. `<AppHeader>`'s wordmark supplies the third half, calling `clearShowSelection` on a route that is not `/`.

**A partial collection says so.** `/api/publisher` drops a child it cannot resolve either way, so `feeds.length` alone cannot tell a collection of four from a collection of eleven with seven missing — and the count line would print the short number as a fact. It compares against `listed` (see [`feeds.md`](feeds.md)) and reads "4 of 11 playlists", with a quiet line and a retry underneath. That is deliberately **not** the load error: the fetch succeeded and the rows on screen are real, so it annotates the list rather than replacing it.

**A collection read without Podcast Index must not CLAIM anything about Podcast Index.** When PI is rate limiting, `/api/publisher` answers from the children's own RSS — the list is complete and right — but every record is `isPreview`, and the row stamp turns that into `NOT IN PI` on feeds PI does hold. The page passes `piUnasked` to suppress it (see [`feeds.md`](feeds.md)); a real miss still stamps. Nothing else is shown: hearts are absent for that one load because there is no guid to write, and that silence is a smaller fault than the false stamp was. Not the same question as the count line below, which is about a number this page states rather than a claim a row makes.

**Four states, and the count line belongs to only two of them.** Loading says `loading playlists…`; a failed load says `couldn't load these` beside a `↻ RETRY`; a load that succeeded over an empty feed says `this collection lists nothing`; rows say how many. The count is suppressed while loading **and on a failure** — `0 playlists` over a load error asserts the very thing the error just said we could not determine, which is `<EmptyLibrary>`'s `degraded` rule and the publisher aside's own. A retry that fails leaves the previously loaded rows readable underneath rather than emptying the list.

**The track count is an annotation, and it arrives late on purpose.** `playlistTrackCount` asks `/api/playlist` with `limit: 1` per row after the covers paint; holding ten rows back for a line of small text would trade the page against it. A row with no answer renders **no count** — never `0 tracks` — and that one branch covers both "still asking" and "the request was rate limited", which are the same thing on screen. It rides on `<PodcastResults>`' optional `meta` prop rather than a bespoke row, so the covers, the `♫ PLAYLIST` stamp and `<FavHeart>` stay one implementation.

**The header holds no navigation, and the measurements are why.** `<PlaylistsLink>` and `<FavoritesLink>` are deleted; both were `.btn-ghost` chips in `<AppHeader>`'s right-hand cluster. A `.btn-ghost` is `px-4` + `text-sm` + `tracking-wider` over an uppercased word, so each was ~140px — a peer of the wordmark, not a glyph. Measured under CDP on the bmb brand, signed out: the header had **zero slack from `sm:` up to ~810px**, the wordmark already taking exactly the 169px it needs, so at 640px the playlists chip took "Boost Me Bitch" down to **31px** and at 768px to 159px. It only fit from 820px, which is why it was gated `lg:` and not `md:`; below `sm:` it was worse again — 141.67px in a 141.6px box on bmb, 154.03px in 154.03px on buddy, and **the bmb wordmark is the shorter one, so a change measured only there hides the failure**. Favorites shed its word and its count on phones for the same reason. Navigation is `<TabBar>` now (see the dock section), which is what makes that pressure go away rather than be re-divided.

**`/playlists` has ONE way in from the app, and it is discovery.** The hero's BROWSE PLAYLISTS button on `/` renders at every width and hides the moment the visitor searches or opens a show; the search box's Playlists *lane* is a search mode rather than that page, and playlists are deliberately **not** a tab — they are content, not a destination. There was a second, a `<Link href="/playlists">` in `<FavoritesPage>`'s header, added when the old header chip went and the hero button alone left `/favorites`, a search and a drilled-in show with no route at all. It was removed on 2026-09-19 at the user's request — *"that playlist button in the top right needs removed"* — with that consequence stated and accepted: from a used home page, the collection is reachable by its address and by going back to an empty `/`. **So the hero button is now the only one, and deleting IT leaves the route with no way in at all.**

## The search box and its content-type selector (`components/search-bar.tsx`)

`<SearchTypeMenu>` — one button naming the current mode, and a menu of `ALL · ♫ MUSIC · PODCASTS · PLAYLISTS · ⚡ NPUB` with a ✓ on the active one. **The mechanics live in `<SelectMenu>` (`components/select-menu.tsx`), shared with `/favorites`' library filter** — see the shared-widget section at the end of this file. It sits **above** the input. Below is where the npub suggestion and the `nsec` warning live, hanging off the box with no gap and no top border so the three read as one control, so nothing may be added down there; and inside the box the `×` already owns the right edge of a field that is 288px wide at 320px, so nothing may be added there either. Above costs one line and touches neither.

**It shipped as a row of five chips and that was wrong at 320px** — the row wrapped to two lines, leaving the filter standing taller than the search box it filters. A menu states the mode in one word and costs one line at every width. It is also the shape podcastindex.org uses for the same choice, which is worth something by itself: these users are already on that site, and a control they recognise needs no explaining. **It was tried a second time as a scrolling lane row and reverted**, which is what produced the shared component: the row did not wrap, it clipped — `⚡ N…` sliced in half at 390px — and an option cut in two does not read as scrollable. Everything about the menu's mechanics, and the identical history on `/favorites`, is in `<SelectMenu>`.
...
**A NEW EPISODES row opens its episode, and the handoff lives on the page.** The rows are Podcast Index's indexed record, so `<FavoritesNewEpisodes>` has a `feedId` and a guid and nothing it can trust about the value block — the same reason its queue control round-trips through `loadEpisodeFromFeed` (`<NoteQueueButton>`). The press does too: `openNewEpisode` in `<FavoritesPage>` puts the SHOW up first and unconditionally (the three reasons `openItem` gives), sets `showOrigin`, `router.push('/')`, and opens the episode once the feed answers, ignoring the answer if the selection changed under it. **It is the page's function, passed in** — the handoff is `selectPodcast` → `setShowOrigin` → `router.push('/')` and a second copy is how one of the three goes missing (CLAUDE.md states that rule for exactly this reason). The cover and the text are ONE control, so a screen reader hears one link to one destination, and its text content is the accessible name; the queue button stays a sibling, because a button may not contain a button. Without `onOpen` the row renders as plain text rather than a control that looks live and does nothing. **The row names its SHOW from the reader's favorites (`feedTitles`), never from the record**: Podcast Index's episode records on this path carry no `feedTitle`, so every row lost the show from its line and from that accessible name, and the line opened on a bare "·" (a Pixel 6, 2026-09-21). The line joins only the parts that exist.

**Each NEW EPISODES row has a ✕ that removes that row alone** (`dismissRow`), with CLEAR's rule: the row goes, the marks stay, so the next pass does not fetch it again. It sits last in the row, after QUEUE, at 36px square. **A removal during a running pass must survive the pass**: the pass copies the list when it starts, paints it after each request and writes it back at the end, so both ✕ and CLEAR record what they removed in `dismissed`, and every paint and the final write filter through it. The marks are computed over the unfiltered list on purpose — a dismissed row was shown, so its mark must move past it.
...
**A row that renders it wraps it in `flex-shrink-0 self-center`, and that is not spacing.** A bare flex child inherits `align-items: stretch`, so the heart's BORDER grows to the full height of its row — a 76px magenta box where the `sm` chip's `min-h`/`min-w` describe a 44px touch target. `<PodcastRow>` (`components/lists/podcast-results.tsx`) shipped without the wrapper while `<FavoriteItemRows>`, `<Podroll>` and the episode list all had it, and it is the row `/favorites` is almost entirely made of, so the whole library rendered as a column of tall pink rectangles. The chip's own classes cannot defend against it: `min-h` sets a floor, not a ceiling, and there is no `max-h` that would not also break the `md` and `tile` sizes.
...
**It is in the SIGNED-OUT menu too, not only the account menu.** Search, playback and favorites all work signed out, so hosting a preference only where an identity exists takes it from every visitor who hasn't got one — the same reasoning that keeps the tab bar's Favorites tab ungated. And **one state has neither menu**: a wallet connected with no Nostr identity, where `<AuthControl>` renders a direct "Sign in" button rather than the combined dropdown and `<AccountMenu>` does not exist at all. `<AuthControl>` renders the bare `<ThemeToggle>` itself for exactly that case, because it is the component that knows. Moving a control into a menu is an improvement only where the menu is always there.

**`overlay` is the one exception to all of that, and it was asked for.** The fullscreen player's copy of `<AuthControl>` renders neither form, so on `/live/<npub>`, `/stream/<naddr>` and `/npub/<npub>` — the three routes with no `<AppHeader>`, and `<TabBar>` carries no theme either — **the theme control is not reachable at all**. That contradicts the paragraph above on its own terms, so it is recorded rather than left to be discovered: the player is a media surface, a palette switch is not what anyone opened it for, and the header is one tap away on every route that is not those three. If that trade is ever revisited, revisit it here.
...
**The way into `/favorites` is the tab bar, and it is never gated on `identity`** — favorites work signed out, local-only, which is also why the account menu was the wrong home for it (that menu is Nostr-only, so a signed-out user with forty saved albums would have no route at all). The header chip that used to do this job carried a count behind its own `mounted` flag; the tab does not show one, which is what removed the last module-scope-hydration hazard from this row.

**What the chip cost is the record of why nothing goes back in this row.** The wordmark needs 142px and the header offered it 141.6 before that chip existed, so its width had to come from somewhere — and `truncate` is all-or-nothing, so a sub-pixel shortfall eats three characters rather than a hair. It shed its word and its number below `sm:`, took `px-2` instead of `.btn-compact`'s `px-2.5`, and the row's mobile gap dropped from `gap-2` to `gap-1`, all to land the wordmark back at 141.6 so "Boost Me Bitch" read whole at 390px. That is the budget any future addition here inherits. **Measure it under CDP device emulation, and on the BUDDY brand** — its wordmark is the longer one.

## The dock: `<TabBar>` and the two layout variables

**Navigation is a bottom tab bar on every route** (`components/tab-bar.tsx`, mounted in the root layout beside `<Player>`). Home / **Queue** / Live / Favorites / Downloads. Every tab is a LINK; the Wallet tab was the only `kind: 'modal'` one and it is gone, which is why that union type collapsed to one shape. It exists because `<AppHeader>`'s right-hand cluster had zero slack and every link added there came off the wordmark — the measurements are in the header section above, and they are why this is a move rather than a re-division.
...
**Both sections carry the same tab strip, and the two differ on ONE rule for a reason.** Live and Upcoming are the two states every broadcast is in, so they stay on screen with a count of `0` rather than vanishing — a hidden tab and a tab reading `0` look identical from the reader's side only if you already know the tab exists. 24/7 is a *subdivision* of live rather than a state of its own, so it appears only when something is in it.

**That rule is load-bearing on the RSS side specifically, because that section makes a CLAIM the Nostr one does not.** "Nothing is broadcasting on RSS right now" is a statement about the world, spoken only once the route has answered; Nostr renders nothing at all when empty and so asserts nothing. Hiding an empty Live tab would take that claim off the page at exactly the moment it is true — `● LIVE 0` says the same thing in less space, and says it without the reader having to switch tabs to find out.

**The default tab follows the content and the explicit choice always wins.** With nothing on air and something scheduled, the section opens on Upcoming: landing on an empty tab while four shows sit one press away is a bad first screen. That normally costs the reader the answer to "is anything on right now" — it does not here, because the `● LIVE 0` tab is still visible saying so. `rssTab` is `null` until pressed, so the default can follow the data without ever overriding a decision.

**The Nostr section was a horizontal rail and is now a grid, because the reason changed rather than being forgotten.** A rail was right while it was one section of the home page competing with a search box and a feed — hiding cards off the side is a real trade there. On a page whose whole job is listing what is on, beside a section of RSS rows in a grid, it read as a different kind of thing: reported as *"one is a list and one is a row"*. Deleted with it: `overscroll-x-contain`, which guarded a sideways gesture that no longer exists, and `useHorizontalWheelScroll`, which existed because a mouse has no sideways wheel. **The hook itself stays — `<Podroll>` is still a rail.**

**`min-w-0` on the card is load-bearing and its absence is invisible until one title is long.** A grid item defaults to `min-width: auto`, so it may not shrink below its own min-content — meaning ONE card whose title cannot break widens the whole grid TRACK and every card in that section with it. Measured at 390px: a **586px card in a 390px viewport**. Nothing scrolled, because `html, body` are `overflow-x: clip`, so the cards were simply cut off at the right edge — which is how it was reported, and why a `scrollWidth` check alone said the page was fine. `truncate` on the lines inside cannot help while the box around them is free to grow.

**Queue replaced Wallet, and the ORDER of those two changes is the whole story.** The wallet looked duplicated — the header carries it in both states, `walletConnected` giving ⚡ plus a balance chip and `!walletConnected` giving *Connect wallet*. But `<AppHeader>` renders on `/`, `/live`, `/favorites`, `/playlists` and now `/queue` **only**. On `/stream/<naddr>`, `/npub/<npub>` and `/live/<npub>` there is no header at all, and those are exactly the routes a shared link lands on and where BOOST is the point. `app/layout.tsx` had already written this down: `<WalletModalHost>` sits in the layout so the Wallet tab "works on every route, not only the three that render `<AppHeader>`".

**So the boost modal grew its own connect control first, and it fixed a live bug on the way.** With no rail it used to say *"No wallet connected — connect one with ⚡ Connect wallet (top right)"*. On those three routes the top right is empty space: the message named a control that was not there. It is now a button that opens the wallet modal itself. That stacks correctly — `lockScroll` is refcounted app-wide, `<WalletModalHost>` is in the layout, and `<BoostModal>` already re-picks its rail through `useWalletChange`, so connecting a wallet updates the picker underneath without losing the amount or the message already typed. **Removing the tab before that existed would have stranded the wallet on the three routes where it matters most.**

**The dock is still five tabs, because Queue took Wallet's slot rather than adding one** — so the measurement below stands and no new one was owed. `/queue` renders `<AppHeader>`, like `/favorites`, `/playlists` and `/downloads`, so the route the dock now points at is not another surface with no way to reach a wallet.

**Playlists is deliberately not a tab.** Playlists are content, not a destination; the one way in is documented in the playlists section.
...
**Five tabs, and the touch floor is still not close.** At 390px five columns of `max-w-7xl` are 78px each, and the row is the full `--tabbar-h` (56px), so height is the binding dimension at 56 > 44 — the tap area is the grid cell and the icon-and-label stack is centred inside it, which is why no item needs a `min-h`. The floor is not threatened until **seven** tabs (390/7 = 55.7px). That number is here so the next person adding one does not re-derive it. The grid is data-driven off `TABS.length`, so neither Downloads nor Queue needed a CSS change; `npm run e2e:downloads` measures all five under real device emulation.

**Wallet is a modal, not a route, and that is why `<WalletModal>` moved to the layout.** The dock's Wallet tab flipped `walletOpen` — the tab is gone, the listen queue has its slot — and so do the header's balance chip, the boost modal's ⚡ WALLET button and the fullscreen player's `overlay` `<AuthControl>`. That flag was only ever *read* by `<AuthControl>`, which at the time rendered on the `<AppHeader>` routes alone (`/`, `/live`, `/favorites`, `/playlists` — three when this was written, four since `/live` gained an index) — so a tab bar on every route flipping it would have been a **dead control** on `/live/<npub>`, `/stream/<naddr>` and `/npub/<npub>`: flag set, nothing mounted to answer, no error. **The premise has since moved and the repair still stands:** `<FullscreenPlayer>` renders a second `<AuthControl>`, so those three routes now have a wallet trigger — but only while the player is open, and the tab bar is on every route whether it is or not. `<WalletModalHost>` renders it from the root layout instead. This is the same failure and the same repair as `<FavoritesPrivacyPrompt>` and `onFavoritesModeNeeded` — a single consumer mounted on fewer surfaces than the controls that address it.

**Where the wallet opens from, now that the dock has no Wallet tab — and the one gap, accepted.** The header's balance chip (or *Connect wallet*) on every `<AppHeader>` route — `/`, `/live`, `/favorites`, `/playlists`, `/downloads`, `/queue`; and the ⚡ WALLET button in every boost modal, in both states. **The fullscreen player's bar carries the BALANCE and no wallet control.** `<AuthControl overlay>` renders the wallet chips for nobody: the boost modal opens from the same screen and says *"⚡ NO WALLET — CONNECT ONE"* when there is none, so the bar was a second control for one modal on the app's most crowded bar — ← BACK, the ⋯ menu and ✕. Its SIGN IN entries stay, because #413's gap was about those too: a Nostr login with Google in reach, on routes that render no header. What the listener actually reads before pressing BOOST is the number, so `<WalletBalanceBox>` sits there instead: a readout with no hover and no press, `← BACK`'s own 26px box, ordered `⋯` then balance, and **mounted on `open` and not on `everOpened`** — the overlay never unmounts once opened, and a mounted `useWalletBalance` is a NIP-47 read on every `payment_sent` (this file's own rule, and the reason the overlay dropped the number in the first place). **`/npub/<npub>` and `/privacy` have no trigger while nothing is playing and no boost is open.** That was decided on 2026-09-19 when the queue landed, in exchange for the dock in the reference screenshot; neither page is a place a wallet is needed without one of the other two surfaces in reach. Nothing there sets `walletOpen`, so it is a missing control, not a dead one.
...
**THE BAR IS ON EVERY ROUTE, INCLUDING `/live/<npub>` AND `/stream/<naddr>`. Decided, not inherited.** Those two had no `<AppHeader>` by design — one is a player overlay and the other is a stream — so the dock is the first navigation they have ever carried, and the alternative was a hide list on `<TabBar>`. It is wanted: a listener who opens a live stream from a shared link has otherwise no route into the app at all, and the bar is the same 56px there as everywhere, so nothing about those pages needs a second layout. Their full-screen `z-40` "connecting…" covers paint UNDER it, for the `relative z-0` reason below, and that is the same answer — a cover you can leave is better than one you cannot. **Do not add a hide list without a new reason**; the empty `TABS` case does not exist, and a bar that appears on some routes and not others is worse than one that is always there.

### The on-screen keyboard moves the dock, and iOS does not always move it back

**Reported 2026-09-06, iPhone 16 Pro, installed home-screen app: the tab bar sat 68 CSS px above the bottom of the screen with the feed showing underneath it, and it stayed there.** The step before it was a reply. `<NostrNoteCard>`'s composer is an in-flow `<textarea>` on the main feed — no overlay, no scroll lock, nothing in this app aware that a keyboard had opened at all — so nothing here was doing it and nothing here was undoing it.

**What iOS does, and it took three attempts to state it correctly.** There are two viewports. The keyboard does not resize the LAYOUT viewport, so `bottom: 0` keeps resolving against a box whose bottom edge iOS is free to move off the screen; what shrinks, and what iOS then SCROLLS, is the VISUAL one. To keep the focused field in sight iOS scrolls the visual viewport down — past the end of the layout viewport if it has to — and on dismissal it does not always scroll back. `visualViewport.offsetTop` is left holding that leftover, the layout viewport's bottom edge sits that far above the screen's, and the dock is sitting on it. A reload clears it, which is exactly why it reads as a rendering glitch rather than as state.

**The first two versions modelled it the other way round — as WebKit lifting the fixed layer to keep it on screen — and that is why the symptom outlived both of them.** Under that reading the correction is `clientHeight - (offsetTop + height)`, which is `covered - lift`: the two terms cancel. A leftover of 68px with nothing covering the screen measures **minus 68**, fails the "is this a keyboard" test, publishes `0px`, and leaves the transform the identity while the bar sits 68px up the page. **Reported a third time, 2026-09-08, as "the bar at the bottom still moves up when scrolling"** — scroll-shaped, because `offsetTop` *is* a scroll offset.

**The measurement is what identifies it, so take it before opening a file.** The `fixed top-0` status strip was at y=0 and exactly `env(safe-area-inset-top)` tall, while the bar's top border was 68px high of where `bottom: 0` puts it. **One end of the fixed layer displaced and the other not** is a bottom-anchored offset; both ends missing at once is a moved layer, which is the scroll-lock failure in `lib/scroll-lock.ts` and a different repair entirely.

**So the correction is a SUM OF TWO INDEPENDENT TERMS, never one subtraction.** `lift` is `visualViewport.offsetTop`, how far the visual viewport has been scrolled past the layout one; `covered` is `clientHeight - visualViewport.height`, how much of the layout viewport something sits on top of. `lib/keyboard-inset.ts` publishes `--kb-inset` as `lift + (covered, if a keyboard explains it)`, and `<TabBar>` and the mini-bar both carry `translateY(var(--kb-inset))` — which pushes the dock back down onto the bottom of the screen when the viewport is stranded, and behind the keyboard while you type. **Neither term implies the other**: a stranded viewport has a lift and covers nothing, an open keyboard covers and usually also lifts, and only `covered` has to prove itself. It is what a native app does with its tab bar, and it makes the resting position **derived every frame it changes rather than remembered**, so "the keyboard closed" is not a state this can get wrong. **Anything new anchored to the bottom of the viewport needs the same transform**, or it is left riding the keyboard alone.

**A SHORTER VISUAL VIEWPORT IS NOT BY ITSELF A KEYBOARD, and the first version read it as one.** Two other things on iOS produce the identical arithmetic — and both of them happen *while the composer is focused*, which is why the focus test alone does not answer either. Reported 2026-09-07 off the same phone as one sentence describing both: **"keyboard is fine while typing but [the dock] pops up after I'm done and returns to the bottom when I scroll down but moves back up when I scroll up."** The first clause is the resting position landing ~51px high instead of 0; the second is that same 51px coming and going with the scroll. Measured against the shipping module under CDP at 51px and 90px of phantom lift; scenarios 8 and 9 of `npm run e2e:keyboard`.

Seven properties are load-bearing, and most of them are only visible from a driven browser:

- **The inset is counted only while an EDITABLE element has focus.** It is what keeps a viewport that shrinks for some *other* reason — a focused button, a page nobody is typing into — from moving the dock. Read it as that and no more: **iOS leaves the field focused when the keyboard is dismissed by a scroll**, so this test still answers yes for the whole time the two cases below are being measured, which is exactly how they shipped.
- **`lift` is counted whatever has focus, and is clamped at 0 rather than subtracted.** Counted unconditionally, because the state this file exists for outlives the focus — the leftover is read on a page nobody is typing into, minutes after the reply. Clamped, because at the top of the document an overscroll bounce carries the visual viewport *above* the layout viewport and `offsetTop` goes negative; the dock is already below the fold there, and pushing it further down is not the repair.
- **Coverage below `MIN_KEYBOARD_PX` (120) is BROWSER CHROME, not a keyboard.** Safari's bottom toolbar collapses on a downward scroll and re-expands on an upward one, taking its ~51px out of the visual viewport while leaving the layout viewport alone — the same shape as a keyboard, an order of magnitude smaller, and tied to the scroll **direction**, which is what made the dock oscillate. The floor is the one judgement in the file and it is sized from both sides: the chrome it must reject is ~51px (toolbar) and ~44px (the accessory bar a hardware keyboard leaves behind), the shortest keyboard it must still accept is an iPhone's **landscape** one at ~162px. Raising it past that restores the original bug on landscape only, which is where nobody is looking — scenario 10 is that edge.
- **Every event schedules the measurement for the next frame; none measures inline.** `focusout` is why: during it the field being left is **still `document.activeElement`**, so an inline read answers "a text field has focus" and holds the inset at the keyboard's full height — and nothing fires afterwards, because the stuck offset is precisely the case where iOS sends no closing `resize`. The dock then stays parked off the bottom of the screen for the rest of the session. That version passed review; it is scenario 5b of `npm run e2e:keyboard`, red against it and green against the deferral.
- **The nudge — a pixel of scroll and back — is armed whenever the dock has to move with no keyboard to explain it**, not only on the way back from one, and after the dismissal animation rather than on the resize event. The transform only HIDES a stranded viewport; the nudge is what settles it at the source, and the event that reports full height arrives while the keyboard is still sliding away. It is armed on a CHANGE, which makes it a debounce rather than something on the scroll path, and it gets **one attempt per stranded episode** — a nudge that moves the viewport publishes a new value, and without that bound it would arm the next one every 300ms for the life of the page.
- **Its two halves are in DIFFERENT FRAMES, or nothing is settled at all.** Both `scrollTo` calls in one task leave the scroll offset exactly where it started, so no scroll is ever composited: the nudge runs, costs two function calls, and the bar stays stranded behind a transform that is already correct. It shipped that way, which is why the symptom was reported once more against a build that had every other rule here right. **At the bottom of the document it is worse than a no-op** — the downward half is clamped away and nothing retried the other direction — and that is exactly where someone replying to the last note in the feed is standing. The restore is relative to where the page is a frame later, not to the captured offset, so a nudge landing mid-flick does not yank the reader back. Scenarios 6 and 6b, which count the calls, assert the page MOVED, and read back the frame each half landed in.
- **A pinch-zoomed page is left alone.** Above scale 1, `offsetTop` is a pan offset in layout pixels against a screen no longer painting them 1:1, so correcting by it moves the dock somewhere nobody asked for.
- **All of it is guarded on `window.visualViewport`.** Where it is absent the inset stays `0px` and every transform is the identity — including on the server, where the CSS declaration in `globals.css` is the only value there is.

**`npm run e2e:keyboard` drives the real `<TabBar>` over CDP** (`npm run build && npm start` first; `CHROME_PATH` picks the browser, `--headed` to watch). The visual viewport is faked because no browser exposes a way to raise a keyboard — but only the SOURCE is faked, and everything downstream is the shipping module painting a real transform read back off `getBoundingClientRect()`. **Two things the script has to do that look like scaffolding**: `Emulation.setFocusEmulationEnabled`, because a headless page is not the focused page and Chromium dispatches no focus events to one; and dispatching the `focusout` **before** the `blur`, because that is the order a browser uses and the reversed order passes against the broken version too. **`?kbdebug=1` on any route renders `<KbDebug>`** (`components/kb-debug.tsx`), a read-only overlay of every number the module decides from — display mode, layout height, visual height and offset, both terms of the sum, the published variable, what has focus, and how far the dock is sitting above `bottom: 0`. It exists because this bug only appears on a real iPhone, usually the reporter's, and two rounds of it were diagnosed by measuring a screenshot in image pixels and dividing by a guessed device ratio — which is how a browser toolbar and a stranded layer came to look like one fault. It mounts nothing without the flag. **Scenario 7 and scenario 9 are the same bounce with and without focus, and only the second one discriminates** — 7 is answered by the focus test, so it stayed green through the whole time the phantom was shipping. **Scenario 11 is the report itself** — a stranded viewport, nothing focused, nothing covering the screen — and 12 is the half nobody would have looked for: with the terms netted off, the *keyboard* case was under-corrected by exactly the number the dock was later stranded by. Both are red against the shipped module and green against the sum.
...
## `<UnderlineTabs>` — one tab strip

The episode page and the fullscreen player's `<EpisodeInfoPanel>` each carried their own `rounded-full` pill strip, and they had already drifted: different label sets for the same tab, `shadow-sm` on one. Both render `components/underline-tabs.tsx` now; **each still owns its tab list and its active state**, so which tabs appear, the lone-section label fallback and `episodeContentsLabel`'s Chapters/Tracks naming are unchanged.

**Square, not pills**: every other control in the app is square-edged and the yellow pill was the one rounded shape on a page of right angles. Active tab is a 2px bolt underline sitting on the strip's hairline. **44px tall, up from 32** — the floor the rest of the app holds. `overscroll-x-contain` is kept and is load-bearing: the fullscreen strip sits inside a `fixed` overlay, and a swipe chained past the end of the strip drags the overlay off-screen.

**NOTHING INSIDE MAY OVERFLOW IT VERTICALLY, and the active underline is why that is written down.** `overflow-x-auto` computes `overflow-y` to `auto` — CSS will not give one axis a scroll and leave the other `visible` — so this row is a vertical scroll container whether or not anyone wanted one. The underline shipped at `-bottom-px`, one pixel below the button's content box, and that pixel is scrollable overflow inside it: measured under Chromium at 390px against the built CSS, `scrollHeight` 45 against `clientHeight` 44. **One pixel is draggable on a touchscreen**, so the strip crept up and down under a thumb while the page stayed still. It is `bottom-0` now — a 2px bolt stroke sitting on the `border-bone/15` hairline rather than over it, the same stroke to look at, and `scrollHeight` back to 44. Fixing the overflow rather than the symptom, because `overflow-y-hidden` on the strip would clip the focus ring off the tab buttons.

This is the same computation that makes `html, body` use `overflow-x: clip`, and the same one that turned the episode page's show-notes container into a scroll box. The horizontal rails in `podroll.tsx` and `nostr-live-streams.tsx` get away with `overflow-x-auto` for the reason stated in reverse: their `pb-1`/`pb-2` keeps the cards' overflow inside the box.
...
- **Playback auto-advances, and it did not until #279.** See the next section: the gate was `isMusicMedium`, which is `music` alone.
...
**Anything that draws ❚❚ must pause, and the glyph is gated on `isPlaying` — never on identity alone.** Three surfaces shipped calling `play()` unconditionally: the episode row's artwork (`components/lists/episode-list.tsx`), the fullscreen album tracklist, and the live-stream card's PLAY button. **A press on the item already playing was a total no-op**, not a mis-fire: `play()` writes `isPlaying: true` over `true`, so zustand emits no change and `<Player>`'s `[isPlaying]` effect does not re-run, while `current.episode.id` is unchanged so the source effect does not either. The `positionSec: 0` it also writes is a store mirror the next `timeupdate` overwrites — so nothing moves, nothing errors, and the control reads as dead. The branch is `if (THIS ITEM is current) togglePlay(); else play(…)` — the condition is identity against *this row*, not "is anything playing" — and the header cover above and `<EpisodeDetailView>` already had it. **`<TransportControls>` did not**, and an earlier version of this sentence said it did: that component is an unconditional `togglePlay()` with no `play()` call anywhere in it, so a maintainer sent there to copy the pattern finds nothing. **The row BODY's tap needed it too** and was left out of the first pass: it kept calling `play()`, which writes `positionSec: 0`, so on a stalled track the reload took that zero and restarted the song while the artwork button two elements away resumed where the listener was — one row, one press, two behaviours.

**The second half is that identity alone makes the ICON lie in both directions.** `playing` on a row is `current?.episode.id === e.id`, so a *paused* current episode still drew ❚❚ — the control was advertising a pause it could not perform, and hiding the ▶ that says "press to resume". So the row keeps **two** flags: `playing` (identity) drives the row tint and the artwork scrim's visibility, and `isThisPlaying = playing && isPlaying` drives the glyph and the labels. **The scrim deliberately does NOT follow the transport** — gating it on `isThisPlaying` fades the control out the instant someone pauses, on the one row they are most likely to press next. Labels name the ACTION (`Pause` / `Resume` / `Play`), never the status: the old `aria-label="Now playing"` described the state to a screen reader while the button claimed to be a pause.
...
**The title was paying for every button, and at two widths it had nothing left.** The row is one flex line — cover, title column, then the controls, each `flex-shrink-0` — so `min-w-0 flex-1` on the title makes it absorb the whole shortfall. DOWNLOAD's 44px on a phone took the title to **108px at 390px**: "Episode 459 ..." with the date, duration and ⚡ V4V stacked on three lines, reported from an iPhone. From `sm:` BOOST, DOWNLOAD, QUEUE and FAVORITE take their words (~450px together), measured on Bowl After Bowl with all four on the line: **2px at 640**, 130 at 768, 262 at 900, 386 at 1024. So the break is `lg:`, not `sm:`.

**Below `lg:` the row keeps BOOST and gains a `⋯` (`<EpisodeRowMenu>`, `components/lists/episode-row-menu.tsx`); QUEUE, FAV and DOWNLOAD are in its menu. From `lg:` up all four are on the row as before.** The title takes two lines below `lg:` (`max-lg:line-clamp-2`, and `lg:truncate` — not `truncate` overridden at `lg:`, because `truncate` and `line-clamp-none` both set `overflow` and would resolve by stylesheet order). Measured: 168px at 390, where a row is 100px; 331 at 640; 459 at 768.
...
- **Its contents are the shared controls as `.tile`s**, never menu items that re-implement them — `<FavEpisodeHeart>` carries the container-is-not-the-parent rule, `<DownloadButton>` five states, `<QueueButton>` the cap. It stays open after a press, so the tile's own state change is the confirmation.
...
- **No trigger when the menu would open empty**, asked through each control's own refusal — `canQueueEpisode`, `canFavoriteEpisode`, `downloadManager.canDownload` — never a copy of it. An unresolved playlist row has no enclosure, so QUEUE and DOWNLOAD refuse it while its heart may still render off the remote item's guids; hiding `⋯` on `e.unresolved` would have dropped that heart.
- **The row says what the menu hides** (`<EpisodeRowMarks>`): `· ♥`, `· in queue`, `· ✓ downloaded` or `· ↓ 47%` on the date line, below `lg:` only. Without it a phone showed none of the three states, and a download running for minutes showed its progress nowhere. Each mark reads the SAME expression as its control — `useEpisodeFavorited`, `useEpisodeQueued`, `<DownloadMark>` — exported from the control's own module.
...
**THE NOW-PLAYING SCREEN KEEPS THE SHOW, THE TRANSPORT AND BOOST; the six secondary actions are in a `⋯` menu.** Both hearts, DOWNLOAD, both SHAREs and STREAM were a row of `.tile`s under BOOST, and that row was what a long title pushed off the bottom of a phone — first as a sixth tile wrapping ≋ STREAM onto a second line, then as the whole row. Asked for by the user on 2026-09-19: *"the show info, playback controls and boost button are the key items"*. The trigger is a 30 × 26 chip in the top bar — ← BACK's and ✕'s own `px-2 py-1` shape, because those are what it sits between, and the panel is `useAnchoredMenu` + `createPortal` at **`z-[55]`** — the rows' `z-40` would be under this overlay's own `z-50`, and `z-[60]`/`z-[70]` belong to `<ModalShell>` and the iOS status strip, so a boost modal opened from a tile still covers the menu. `grid-cols-3`, not `auto-fit`: six tiles fill two rows exactly, where a four-wide grid left STREAM alone beside three empty cells.

**EVERY CONTROL ON THAT BAR IS 26px, and the bar is 51px.** ← BACK, ⋯, the balance box, `<AuthControl overlay>`'s SIGN IN and ✕ are all `px-2 py-1` over a 16px line box — `text-xs`'s line-height is 1rem, `text-base` takes `leading-none`, and the balance box's 11px digits sit inside a `text-base` strut. A `.btn-ghost` at its own 38px among them read as a different kind of control, and it made the bar 12px taller than it had to be; those 12px are the cover's now. What the bar taught while the six actions moved through it stands: DOWNLOAD spent an afternoon there as its own `size="header"` chip and the size went with it; the right-hand cluster and ← BACK are `flex-shrink-0`, because signed out below ~400px the SIGN IN chip wrapped onto two lines and took the bar to 83px; and **NOW PLAYING is `hidden sm:inline`**, because with ⋯ in the cluster the label had 39px for a word that needs 54 and overlapped the ⋯ instead of folding. The screen it labels is a full-screen cover with the show's title under it.

**WHO SIGNS, beside what pays.** `<AuthControl overlay>` offers the two logins while the user is signed OUT and renders **null** once they are in — the account menu belongs to `<NostrAuth>`, which the header-less routes mount hidden — so a signed-in listener saw no Nostr at all on the one screen whose BOOST publishes a note under their key. `<NostrIdentityChip>` (`components/nostr-auth/identity-chip.tsx`) is the avatar and the name in the same 26px box, a readout like the balance: the menu is one ← BACK away, and a second mounted `<AccountMenu>` would duplicate its bunker-health and restore subscriptions for the session. **The name is `hidden sm:inline`**: with the balance box beside it the chip ran 117px and the bar overflowed by 18px at 320 (375 cleared by 1), and the picture is the part that says *which* account. `<AuthControl overlay>` returns null rather than an empty wrapper for the same arithmetic — an empty flex child still takes one of the bar's `gap-2`s.
...
- **The gate is `playsAsTracks`, not `isMusicMedium`.** `isMusicMedium` is `music` alone, so a `musicL` **playlist stopped dead at the end of every track** — the surface whose entire purpose is to play one song after another was the only one that would not. `playsAsTracks` is the gate the rest of the app already used for exactly this question, and its own doc comment names auto-advance as a behaviour it decides; the two had simply drifted. Reported from a phone, on ChadF's Greatest Hits.
- **The step is `nextPlayableIndex`, not `idx ± 1`.** A playlist's queue holds one row per `<podcast:remoteItem>` *including* the ones Podcast Index could not resolve, and those carry an empty `enclosureUrl`. Landing on one puts a dead track in the player: it reports as playing and is silent, and the only way out is to pick another row by hand. `<EpisodeList>` already suppressed the tap and the header ▶ on such a row — this is the same test for the paths that arrive at a row without anybody choosing it. **`<TransportControls>` reads the same helper**, so ⏭ can never be enabled over a step `playNext` refuses to take; the alternative is a control that looks live and does nothing, which this repo has shipped twice.
- **`playNext` returns whether it moved**, and `onEnded` falls through to `setPlaying(false)` when it did not. Without that, the end of the last track left `isPlaying` true over a stopped element and the transport drew ❚❚ over silence — **true of albums since long before playlists existed**, and the same class of lie as a ✗ on an unanswered wallet.
- **The listener must not run out of PAGE.** Auto-advance ends at the end of the queue, and a playlist's queue is one fetched page rather than the list — so playback would stop at track 50 of 300 with a LOAD MORE TRACKS button nobody is looking at. `<EpisodeList>` fetches the next page when the **playing** track comes within `PLAYLIST_PREFETCH_RUNUP` rows of the end. Hung off what is playing rather than off scrolling, and gated on the playing item being in this queue: a page is a real request against the PI quota, so it is spent for somebody listening through the list and never for a tab left open.
...
- **The mini-bar sheds controls below `sm:`, and the numbers are why.** Its row is one flex line — art, text column, controls — and the controls could not shrink: every button is `flex-shrink-0`, so `min-w-0 flex-1` on the text column meant the text absorbed the entire shortfall. Measured in-browser at 390px: the title column got **31px** (it needs 206px) and the seek `<input>` got **0px** — the two time labels printed on top of each other, and the title read `Epi…`. Nothing overflowed, so no scrollbar and no clipping ever hinted at it. Three changes take the title to **202px** and the seek bar to **120px**, with the 1280px layout byte-identical (controls 248px, BOOST 104×38, seek 839px): `px-3 sm:px-4` + `gap-3 sm:gap-4` on the row, BOOST collapsing to a 44px icon square (`btn-compact` + `min-w-[44px] sm:min-w-0`, `aria-label` **required** since `title` is not an accessible name), and `<TransportControls sidesOnDesktopOnly>`, which hides ⏮/⏭ under `sm:`. That last one is a **hide, not a drop** — a tap anywhere on the mini-bar opens the fullscreen player, where the full transport lives one tap away, the same trade the `<VideoToggle className="hidden sm:inline-flex">` beside it already makes. `sidesOnDesktopOnly` swaps the side buttons' base `flex` for `hidden sm:flex` rather than appending a class: both are display utilities at equal specificity, so a string that still says `flex` would be deciding the layout on Tailwind's internal emit order. Verified at 320 / 390 / 639 / 640 / 1280 — no horizontal overflow at any of them.
- **It sheds LINES below `sm:` too, and that is a different squeeze from the one above.** The bullet above is horizontal — one flex row that could not shrink. This one is vertical: the text column stacks the show title, a buffering readout and the chapter label on top of the episode title and the seek row, and each of those arrived separately, each for a good reason. Measured at 390px on a chaptered episode: the bar stood **95px** tall, and **132px** while buffering, over a `--dock-b` of 56px plus the safe area — reported as "the small now playing bar is too busy now on mobile", off a phone screenshot where the buffering sentence wrapped to two lines. Below `sm:` those three now fold into ONE truncating line, `miniSecondary`, in the order of what the listener needs at that second: **stall → chapter → show title**. Measured after: **78px** in both states, with 640px and 1280px byte-identical to before (95px, seek 273px / 817px, all four lines, and 132px at 640 while buffering).
  - **`audioErr` is deliberately NOT in that ladder.** It keeps its own wrapping line at every width, for the reason the line already carries in the source: truncating the one sentence that says why playback died is the failure this bar exists to answer. A stall and an error are mutually exclusive here (`stalled && !audioErr`), so the collapsed line never has to choose between them.
  - **A fold, never a drop.** The buffering `<div>` and `<ChapterLabel>` take `hidden sm:block`, and the show title's own span `hidden sm:inline` — the same hide-don't-drop trade `sidesOnDesktopOnly` and `<VideoToggle className="hidden sm:inline-flex">` already make on this row, and honest for the same reason: a tap on the bar (or its title button) opens the fullscreen player, which carries all of it at every width. The chapter keeps `text-bolt/90` on the collapsed line, so which of the three is showing is still readable; the buffering text is shortened rather than truncated, because a truncated instruction is not an instruction.
  - **`<StreamPulse>` stays on that line at every width.** It is the only thing on screen saying that money is moving unattended, and `streaming-settings.tsx` already refuses to trade it away for space.
- **`playNext` auto-advances on `onEnded` only for music** — unless the item that ended is in the listen queue, which is asked first. See the next section.
...
- **The closed overlay is `invisible`, and `translate-y-full` alone is not enough to get it off the screen.** The transform moves the element by 100% of **its own** height, which is `100dvh`; a `position: fixed` element is laid out against the initial containing block, which is the **large** (`100lvh`) viewport. Those two units are equal only while the browser chrome is hidden, and the safe-area padding on the same element pulls them apart again in the installed app, where there is no chrome to blame. Every moment they disagree, the closed player stops `lvh - dvh` short of clearing the screen and parks its header — `← back  NOW PLAYING  ✕` — as a strip along the bottom of the page, under the dock, over whatever the user is reading. **It does not read as the player.** The rest of the player is off-screen, so what a user reports is a second BACK button that appeared at the bottom of a search result. Reported 2026-09-07 off a phone screenshot; the strip sat below `<TabBar>`, which is the tell that it is the `z-50` overlay and not anything the page owns. **`visibility` goes in the transition list** (`transition-[transform,visibility]`), because CSS holds a `visible → hidden` transition visible for its whole duration and flips at the end, while `hidden → visible` shows immediately — so the slide-out still plays and the slide-in is not delayed. `display: none` would work as well for hiding, and is wrong here: the subtree is deliberately kept mounted and measurable across a close/reopen (see `everOpened`).
...
- **Right pane:** title/seek → control row (`<TransportControls size="lg">` + a `flex-1` `⚡ BOOST`), then a second row of `<FavHeart size="md">` + SHARE + the STREAM button → value-split disclosure → **Up Next** (the listen queue, when it holds anything) → **album tracklist** for music (`Album · N tracks`, `episodeQueue` clickable, current highlighted, `max-h-80` scroll) → `<EpisodeInfoPanel>` → `socialInteract` thread. **For Nostr live streams the right pane is replaced by the live chat.**
...
- **The hls.js constructor config is four decisions, and two of them are bounds rather than preferences.** `lowLatencyMode: false` stops part loading — it hugged the live edge with a tiny buffer and any jitter stalled playback — and `liveSyncDurationCount: 4` keeps a four-segment cushion in front of the play head.
  ...
  - **`backBufferLength: 90` is a MEMORY BOUND on a live stream, not a tuning knob.** hls.js defaults it to `Infinity`, and `trimBuffers` skips the flush entirely on a non-finite value — so a broadcast keeps every second it ever appended, for the whole show, in one SourceBuffer on a phone. It is held for a rewind this player does not offer on the item that matters: a live item has no seek bar (the ● LIVE stamp replaces it) and no skip buttons. The cost is real — an append that runs out of room raises `QuotaExceededError`, hls.js answers by flushing and reloading, and that is heard as a dropout that gets worse the longer somebody listens. An HLS `alternateEnclosure` VOD *is* seekable and pays a re-download past 90 s, which is what every other player does.
- **`levelParsingError` IS NOT A REPORT THAT THE PLAYLIST COULD NOT BE READ, AND NO AMOUNT OF RECONNECTING FIXES ONE — `ignorePlaylistParsingErrors: true` is the fix, and it is in the hls.js constructor config.** On a LIVE playlist that error is a continuity assertion over two snapshots: hls.js merges each reload over the previous one (`mapFragmentIntersection`) and gives up if a segment URI moved to a different media-sequence number, or if the discontinuity counter changed under it — "media sequence mismatch" / "discontinuity sequence mismatch". A broadcaster restarting its encoder, an origin rolling its live window across backends, a stream stitched from more than one source: all of them trip it while the media stays perfectly playable. **Every zap.stream broadcast is single-rendition, and that is what makes it FATAL rather than a warning** — `error-controller` answers a parsing error by switching level, there is no other level, so it escalates and calls `stopLoad()`. Retrying cannot help, because `stream-controller` stores the new snapshot even on the error path: the next reload merges against the newest playlist and mismatches again, so an origin that mismatches every consecutive pair keeps the stream behind `stream unreachable — reconnecting…` forever. **Reported twice from an iPhone.** What the flag gives up is nothing a viewer wants: a body that yields no fragments still becomes `levelEmptyError` (which hls.js retries by itself on a live playlist, escalating only after exhausting that) and an unreachable one is still `levelLoadError`/`levelLoadTimeout`. We are a viewer, not a packager.
- **A fatal hls.js NETWORK error is not proof the listener heard anything stop — start the recovery on the first one and make the CLAIM wait for the GAP. A count of failures is not a measure of silence.** The fragments already in the buffer play on for as long as they last, routinely tens of seconds, so an error-driven banner is a message over a stream the listener can still hear; the previous version also spoke on the SECOND consecutive failure, which on a deep buffer is the same lie two seconds later. **The element is the only witness.** `readyState` below `HAVE_FUTURE_DATA` means it cannot play the next frame — so the error paints only when it already starved, and otherwise parks its sentence in `reconnectMsg` for the `<video>`'s `waiting`/`stalled` handlers (`noteLiveGap`) to paint at the moment the gap happens. **Both halves are needed**: the buffer usually outlives the error that killed the loader, and the retry backoff runs out to 15 s, so waiting for the next error to coincide with the gap would leave a dead stream silent and unexplained. `reconnectMsg` is cleared by `FRAG_BUFFERED` and by a fresh source, and a non-recoverable error clears it and speaks immediately — there is no gap to wait for. The `startLoad()` backoff is untouched. **The plain rebuffer stays distinguishable**: `stalled` with no parked sentence is still the mini-bar's `⋯ buffering`, never a claim about the broadcaster.
- **`primePlaybackAudioSession` sets `navigator.audioSession.type = 'playback'` on the first play attempt.** iOS decides from the page's audio-session type whether audio survives a lock screen, the ringer switch and an app switch, and the default (`auto`) gives a web app the conservative answer. It is set at the first play rather than at mount because `playback` interrupts other apps and ignores the silent switch — right for a player, rude for a page nobody has pressed play on. Feature-detected, so it is a no-op everywhere else. **It does not keep a backgrounded `<video>`'s picture alive**; iOS suspends that regardless, and Picture-in-Picture is the answer there.
- **The foreground-resume watchdog covers BOTH HLS paths and re-sources through `reloadNonce`, never by hand.** It sat on the native branch only, and `startLoad()` alone is not always enough for hls.js: iOS suspends the app on a switch away, and a `ManagedMediaSource` torn down while hidden comes back with a loader that restarts onto nothing. The hand-rolled `el.src = url; el.load()` it replaces was also blind to `nativeHlsFailedUrl`, so it would put a stream that only plays under hls.js back on the native element that had already refused it. Re-running the source effect is the one reload that rebuilds whichever path the url needs.
- **A `play()` the browser REFUSES must reach the store, on the `<video>` path as well as the `<audio>` one — `playOrPark` (`components/player.tsx`).** The audio path has answered a rejection with `setPlaying(false)` since the day it was written, and the `?t=` note below already treats that as the reason a blocked autoplay costs nothing: the item sits loaded and the transport draws ▶, so one tap inside a real gesture starts it. The three `<video>` attempts in the source effect swallowed theirs (`.catch(() => {})`), and that is the one path a visitor reaches with **no gesture at all**: `/live/<npub>` and `/stream/<naddr>` call `play()` from an effect, after a relay round trip that is seconds long — every mobile browser and desktop Safari decline it. `isPlaying` stayed true over a paused element, so the fullscreen player drew ❚❚ over silence and the only way out was pause-then-play, which few people try. **Reported as "this stream isn't playing."** The three foreground-resume nudges take it too: coming back to a tab is not a gesture either, and no amount of re-sourcing fixes a refusal that wants one.
  - **It parks on `NotAllowedError` ONLY, and a blanket catch here is not the audio path's blanket catch arriving late — it breaks the live resume.** Re-sourcing a live stream (the `reloadNonce` path) tears the old source down, and that rejects the PREVIOUS run's still-pending `play()` with `AbortError`. Parking on that clears `isPlayingRef` *behind* the new run's own async `play()` — which reads the ref inside `import('hls.js').then(…)` — so the resume the user just pressed for silently declines to start. `NotAllowedError` is the one rejection a tap can fix, and the only one that means the element is parked rather than progressing.
...
### The listen queue ("Up Next")

**Two queues, one player, and they answer different questions.** `episodeQueue` is one feed's display order: a single writer (`<EpisodeList>`) sets it on every mount, nobody chose its contents, and it dies with the tab. `listenQueue` is a list the listener assembles by hand, across shows, and it persists per npub in `bmb:listen_queue:<npub>`. Ported from PR #132, whose Inbox half had no host component left.

**Each item carries its own podcast, and that pair is the whole reason for `QueueItem`.** `stepTo` can reuse `current.podcast` when it walks `episodeQueue`, because every row there belongs to one feed — the sentence its comment used to end on. A cross-show queue breaks it: the next item is usually a different show, so its artwork, its title and **its value block** have to travel with it. Carrying the previous podcast forward would point BOOST and the streaming engine at the artist who was playing a moment ago.

**The precedence is stated once, in `stepTo`, and it is guarded rather than global.** `queueIndexOf` answers `-1` for anything that was never queued, so every line below that guard is the code that shipped before the queue existed, reached on exactly the states it was reached on before. Its `.length` short-circuit makes "an empty listen queue costs nothing" literally true, down to not running `epKey`. **That is the property to protect when editing here**, and the acceptance test for the whole feature is that a user with no queue sees no behaviour change at all.

**`epKey`, not `id`.** `stepTo` locates a row in `episodeQueue` by `e.id`, which is sound only because that array holds one feed's rows — a Podcast Index episode id is unique *within* its feed and not between feeds, and a cross-show queue puts two feeds' rows in one array. So the item guid is the key, with `feedId:id` as the floor. Deliberately not named `episodeKey`: `lib/pi-batch.ts` already exports one of those, keyed on `{feedGuid, itemGuid}`, server-side.

#### The four motions

| Motion | What happens to the queue |
| --- | --- |
| Tap a row (`playFromQueue`) | jumps, **removes nothing** — the row is a destination, not a completion |
| ⏭ (`playNext`) | **drains the item you leave**, plays the next; **disabled on the last item** |
| ⏮ (`playPrev`) | moves back, removes nothing |
| the item **ends** (`handlePlaybackEnded`) | drains it and plays the next; on the last item, drains it and stops |

Forward drains and back does not, because a skip is a completion and a return is a correction. Without the drain the only way to empty a queue is to listen to all of it, and a skipped item comes round forever.

**⏭ on the last item is DISABLED, which is a deliberate departure from PR #132** — there it cleared the item and stopped playback. The rule this repo already states about that button runs both ways: it may never be *enabled* over a step `playNext` refuses to take, and equally it may not be enabled to do something other than what it draws. A skip glyph that halts playback and empties the last row is drawn wrong. The last item still leaves — when it finishes.

**A queued item auto-advances whatever its medium**, and that is the one clause `playsAsTracks` does not cover. `<Player>`'s `onEnded` asks `handlePlaybackEnded()` first; it returns false — changing nothing — unless the item that ended is in the queue, which leaves the medium gate below it on exactly the states it had before. Somebody who lines up four talk episodes has said they want them in order, which is the same statement an album makes about its tracks. The gate below decides whether a **feed** plays as tracks, and the queue is not a feed.

**`<video onEnded>` deliberately does not participate.** A queued item plays through the `<audio>` unless the listener toggles video mode, and `videoMode` resets on every item change — so a `<video>` ending can only be an item they deliberately switched mid-listen. Auto-advancing out of that is not what they asked for.

**`applyStep` returns `!!patch.current`, not `!!patch`.** A drain can change state without moving playback, and answering true for one would leave `isPlaying` true over a stopped element — the ❚❚-over-silence bug that return value exists for.

**`videoMode: false` and the start position now have six sites, not three:** `play`, both branches of `stepTo`, `playFromQueue`, `revealQueue` and `handlePlaybackEnded`. The start position is `savedStartSec(episode, podcast)` at every one of them (`play()` lets an explicit `startSec` outrank it) — the queue sites wrote `positionSec: 0` until #414 and the queue met, so a half-heard episode queued as Up Next started at 0:00 under a page saying "▶ RESUME 40:00", and fifteen seconds later the writer overwrote the saved place. Miss the `videoMode` reset and the next item inherits the toggle — the source effect (deps `[episode.id, videoMode]`) routes to the `<video>`, finds no alternate enclosure, and draws a black box with no audio and no error.

#### The chapter fall-through, and the bug it also fixes

`<Player>` passes `prev={chapterNav?.prev} next={chapterNav?.next}`, so on a chaptered episode ⏮/⏭ step chapters. A spent override now **falls through to the listen queue and to nothing else**: on the last chapter of a queued episode ⏭ was a dead button, because `buildChapterNav` disables it and an override outranks the default. It never falls through to `episodeQueue` — a chaptered show whose ⏭ silently left the episode is not what that button has ever meant.

The subtler half is `nextDisabled`, and it is the mirror of the documented dead-control bug: a queued item is usually **not** in `episodeQueue`, so `idx === -1` and `nextPlayableIndex` returns `-1` by its `from < 0` guard — the button would draw disabled while `playNext()` would in fact advance the queue. So the disabled test consults the queue too, through the same `nextPlayableIndexBy` walk `stepTo` uses. One walk, four callers.

#### Where the panel lives

`<QueueList>` has **ONE mount and one definition**: the `/queue` route the dock points at. It had a second inside `<FullscreenPlayer>`, above the album tracklist it is modelled on, and the dock tab is why it does not — the same argument that took it off the home page, made twice. Asked for on 2026-09-19 from the phone: *"I don't think the queue should be on the now playing page"*, which is the now-playing screen's own rule (the show, the transport and BOOST) applied to the list under them.

**It was on the home page first, and the dock tab is why it is not.** That mount existed because the queue had no destination of its own — the panel had to sit somewhere a reader would meet it, including on a show page, which is why it deliberately skipped `!inDetailView` when the two sections around it do not. Once `/queue` became a dock tab, the queue was one press away from every route, and the home-page block stopped paying for itself: measured against a 390px screenshot it filled a screen's worth of vertical space between BROWSE PLAYLISTS and the global boost feed, pushing the feed below the fold, to offer a second way to reach something already one tap away. **The order matters if this is ever revisited: the home-page mount was right while the queue had no route, and wrong the moment it had one.**

It also takes a hydration hazard off that page with it. The store seeds `listenQueue` from localStorage at module scope, so the server always answers "empty" — the home-page section needed a mount gate to avoid React #418, and `<QueuePage>` still does for the same reason.

**What the player mount was for was watching the drain** — you saw the row you just finished leave, on the surface you were already on. That is now the `/queue` tab's, one tap away, and `<QueuePage>`'s empty state answers there where the panel's silence did not.

**The `/queue` route is the one mount that must speak when the queue is EMPTY.** The panel renders nothing in that case, which is right where it sits under other content and wrong for a page somebody navigated to on purpose — a blank screen is not an answer. `<QueuePage>` carries the empty state, behind that same mount tick.

**A queue that survived a reload must be REACHABLE, and `revealQueue` is what makes that true.** `current` is in-memory while the queue is not, so every page load lands with items on disk and nothing playing; `<Player>` renders nothing without a `current`, which took the panel's player mount with it and left the mini-bar absent. `revealQueue` points the player at the queue's **head**, paused. It never starts playback: an autoplay nobody asked for is the wrong answer, a browser would block it anyway, and `isPlaying` would then be true over an element that never started. It runs from two places — an effect in `<Player>` (effects still run for a component that returns null, which is what lets it sit above that early return) and `enqueueEpisode`. Doing it in the store initializer instead would paint a mini-bar on the client that the server did not.

#### The controls

`<QueueButton>` (rows, episode page) is synchronous. `<NoteQueueButton>` (a note's episode card) is not, and the round trip is not overhead to optimise away: the episode a note card was labelled from is Podcast Index's indexed record, which carries **no value block**. Queueing it as it stands would put an item in Up Next that plays and then pays nobody — a boost button with nothing to boost and a streaming settle with no recipients, neither of which says anything on screen. It resolves through `loadEpisodeFromFeed`, exactly as `openBoostedEpisode` does, and shows a busy state and a retry rather than failing quietly.

**Both read "is this queued" as a BOOLEAN selector, never `s.listenQueue`.** That array returns a fresh reference on every mutation, so selecting it re-renders every memoized `<NoteCard>` in a feed on each press — hundreds of them; a boolean compares with `Object.is` and re-renders only the card whose answer changed. They share one expression, which is also what makes the note card's ✓ survive its round trip: `epKey` prefers the item guid, and both records carry the same one.

On the note card the control is a **sibling of the heart**, on the heart's own `favoritable` gate, and not in the note's action row above — that row is social actions gated on `identity`, and a listen queue needs none, so it would be hidden from exactly the signed-out reader who can use it. In an episode row both controls are siblings of the tap targets, never children: a button may not contain a button. **The row control was hidden below `sm:`, and below `lg:` it is now in the row's `⋯` menu instead.** On one flex line it cost the episode title a flat **48px** under `sm:` — at **320px, 90px down to 42px**. The menu (*Below `lg:` the episode row is BOOST and a `⋯`*, under the episode list above) holds it as a `'tile'`, so a phone can queue from the list again without the title paying for it.

**A QUEUED EPISODE IS TRIMMED, and the player is what makes that safe.** `trimForQueue` (`lib/util.ts`, pinned by `check:queue`) deletes `description` and `contentEncoded` before the queue reaches `localStorage` — they are the two large fields, the queue is a user decision rather than an evictable cache, and the list renders neither. The PLAYER renders both, so an episode reached through `playFromQueue`, `queueStepTo`, `revealQueue` or `handlePlaybackEnded` opened with an empty About tab: reported from a phone as *"now I don't have any shownotes"*, on a build where the same episode opened from a feed row showed them. `<FullscreenPlayer>` now fetches them when the episode carries neither field — one `loadFeed`, which coalesces a request already in flight and keeps nothing once it settles, keyed by episode id so a step to the next item cannot paint the previous one's notes. **Anything else trimmed from a queued episode needs the same question asked**: the queue's own list is not the only surface that reads one.

**That effect is keyed on PRIMITIVES — id, guid, "notes missing", `liveStatus`, feed id — never on the episode object.** It also hands the feed's value block to `refreshCurrentValue`, which replaces `current.episode` whenever the block differs by reference, and a fresh parse never equals the last one. Keyed on the object, the refresh re-ran the fetch for as long as the episode sat in the player — **984 `/api/feed` reads in 15 s on a PAUSED queue head**, each a full parse of up to 1000 items that the 60 s browser cache answered, so the server log showed six and the cost landed on the phone. `<FullscreenPlayer>` is mounted under `<Player>` whether or not it is open, so nothing had to be pressed. The live watcher's `syncCurrentValue` replaces the same object on every poll, and **a live item skips the value refresh**: its `value` is the on-air track's block, and the feed's copy is the show's. `e2e:queue` section 8 counts the reads at `fetch`.
...
**Nothing may write `episodeQueue` from the queue.** `setEpisodeQueue(orderedEpisodes)` stays its single writer. A "sync the queue into `episodeQueue`" shortcut would reorder the show list under the user's finger.

#### Verification

**No `check:*` script can see any of this** — `lib/store.ts` is not strip-types loadable, and every rule above is wiring between a store action, a media event and a component. `npm run e2e:playlist` covers the `episodeQueue` path it must not disturb; the queue's own rules are manual, and the first scenario is the one that matters: **with an empty listen queue, album and playlist auto-advance, the ⏮/⏭ disabled states and chapter stepping are all identical to before.**
...
**Both players** carry the same affordances: seek-bar **tick marks** (`<ChapterTicks>`), a **current-chapter label** (`<ChapterLabel>`, `start–end · title`, plus the chapter's own `↗` when it published a `url`) — both in `components/chapter-ui.tsx` — and **chapter-stepping ⏮/⏭** via `buildChapterNav` feeding the `prev`/`next` override (absent → falls back to episode/track nav). Prev restarts the current chapter if >3 s in, else jumps back. The tick wrapper uses a `block` input + `flex items-center` so ticks center on the 2px track (an inline-block input leaves a baseline descender gap that drops them below the line).

**Hovering either seek bar with a mouse names the chapter under the pointer** — `<ChapterHoverTip>` (`components/chapter-ui.tsx`), a tip above the pointer reading `1:57:47 · First Time I Ever...`. It sits in the same `relative` wrapper as `<ChapterTicks>` and listens on that wrapper, so each player adds one element and no handlers. Four decisions in it, each easy to undo by accident:

- **The tip, the ticks and a click all use the THUMB INSET, and they must keep agreeing.** A range input keeps its thumb inside the box, so a value maps to `thumb/2 + (width − thumb) × fraction`, not to `fraction` of the width. The ticks were drawn at plain percentages, so a tick sat up to half a thumb from the point that seeks to it — measured on Bowl After Bowl 456 (4:38:34 on a ~460px bar): **3.2px, and a click just right of the old tick landed in the PREVIOUS chapter.** The tip made that visible, because it names the chapter at the pointer and the tick claimed a different one. `--seek-thumb` (`app/globals.css`) is the one width the thumb rules, the ticks' `calc()` and the tip's arithmetic all read; the touch block overrides it to 16px.
- **Mouse only**, on `pointerType === 'mouse'`. A touch has no hover, and a finger on the thumb already covers the place a tip would show.
- **Text only, never the chapter's image.** Chapter art is third-party media, often tens of megabytes on the audio's own host, and `artOk` exists to keep it from starving playback (see "Artwork must never outrank the audio"). A tip fetching one per chapter hovered would go around that gate.
- **It is kept inside the bar by its own transform**, `translateX(−fraction × 100%)`: left-aligned at the start, centred in the middle, right-aligned at the end. Nothing is measured, so there is no layout effect and no second render per move.

`npm run e2e:seekhover` drives both bars with real CDP mouse events and asserts the three agree; its step 3 replays the old plain-percent tick and asserts that it lands in the wrong chapter.

**The now-playing label links the chapter too: the TITLE is the link and a trailing `↗` marks it.** The `↗` was on `<EpisodeContents>`' rows and nowhere else, so the chapter playing *right now* named itself and silently dropped the URL that goes with it — reported against *Chad and Reeds* 006, whose 5:00 chapter links Zapstore: the link was real, published, and reachable only by opening the Chapters tab and finding the row again. Both players render `<ChapterLabel>`, so it lands on both at once. Castamatic is the reference — it puts a link glyph at the trailing edge of the same line.

**Two affordances, one destination, and exactly ONE link in the a11y tree.** The title carries the accessible name a link should have; the `↗` carries none, so it is `aria-hidden` with `tabIndex={-1}` rather than given a second label. Two adjacent anchors to one href otherwise read out twice, take two tab stops, and the second announces only "link, Open chapter link". The arrow earns its place anyway, for sighted discovery: an underline-on-hover is invisible on touch and says nothing at rest about the chapter pointing anywhere. **The timestamp stays outside the anchor** — the link's subject is the chapter, not the minute it starts at. **A linked title does change what a tap on that line does in the mini-bar**: it used to expand the player and now follows the URL. That is confined to `sm:` and up (the mini-bar folds the line away below it) and the rest of the bar still expands the player.

**Height is spoken for on this row and width is free, which decides the whole shape of the control.** The fullscreen cover is capped against the hard-coded `30rem` reserve measured above — the active-chapter label is one of the rows that reserve counts — and the clearance under the tile row is **+22px at 390×844** and already **−60px at 375×667**. A `min-h-6` would have eaten 8 of those 22 and falsified the constant on the surface where nobody would look for the cause, and it would have made the mini-bar change height by itself as playback crossed between chapters that link and chapters that do not (feeds link a handful and leave the rest bare). Width costs nothing, because the title truncates. So the arrow is `self-stretch w-6`: 24px wide, as tall as the line box the title already sets. Measured against the built CSS — mini-bar label **15px**, fullscreen **16px**, both unchanged, and the mini-bar label is 15px **with and without** the `↗`, which is the no-jitter half. The arrow is 24×16 and the linked title **192×16** on the reported chapter, so the real target is the title and the arrow is the marker. Both sit under WCAG 2.5.8's *inline* exception: a target "whose size is otherwise constrained by the line-height of non-target text". `<EpisodeContents>`' row `↗` is **not** line-constrained — it is a flex sibling of the row button — which is why that one pays for `px-2 py-1.5` and this one must not copy it.

**`stopPropagation` is load-bearing here and only here.** `<Player>`'s whole mini-bar is a click target that opens the fullscreen player, so without it one tap both follows the link and expands the player behind it. The href needs no guard of its own: `useChapters` runs every chapter `url` through `httpUrl` at the parse boundary, so a rejected scheme arrives `undefined` and renders no `↗` at all.
...
- **`waiting`/`stalled` are wired to the gate, and they MEASURE — they do not assert.** Closing it drops the src to the episode cover and thereby **cancels the in-flight chapter image**, and these two events are what make that possible once playback has run dry, because `sampleHeadroom` rides on `timeupdate` — *which a wedged element stops firing*. A gate that only closed from there stayed open in exactly the case it exists for. That was a real intermediate failure, not a hypothetical. But they shipped as a flat `setArtOk(false)`, and **neither event is evidence on its own** — see the next entry.
- **`stalled` IS A CLAIM ABOUT THE FETCH, NOT ABOUT THE BUFFER, and reading it as starvation is a WebKit-only flicker.** The spec fires it when no media data has arrived for three seconds, whatever the element is holding; AVFoundation pulls a huge chunk and then goes quiet, so WebKit raises it on a completely healthy element. Measured in Safari on 2026-09-09 against Bowl After Bowl 456: **`stalled` twice inside eleven seconds, `readyState` 4, 2,001 seconds of audio buffered ahead of the play head**, with the sound never interrupted. Each one shut the gate, so the fullscreen cover dropped to the episode art and came back a second later when the next headroom sample reopened it. Reported as *"the chapter art is flickering between the chapter art and the show art"* — **iPhone only**, because Chromium does not fire it here at all, and *"no buffering"*, because there was none. Both handlers now call `noteStarvation`, which measures and lets `artGateOpen` decide; the yield survives because a genuinely wedged element has no playable audio in front of the play head. Three cheap wrong turns were ruled out first, each by measurement: the `/api/art` proxy (all 11 chapter images 200, cold and warm, under 0.5 s), the headroom sampler (Chrome holds 140–270 s and refills long before the 5 s floor), and the chapter transition itself (three natural boundaries painted chapter-art-to-chapter-art with no drop).
- **Measure the buffered range the play head is IN, never the last range held** (`playableAhead`, `lib/util.ts`). They are the same number only while there is one range, and a seek is what makes a second one: an element at 0:50 holding `[0–100]` and `[6880–8900]` can play for 50 more seconds while the end of its last range is 8,850 seconds away. `b.end(b.length - 1)` reports that as comfortable headroom over a play head about to run dry — the failure the gate exists to prevent, reached from the other side. `null` is not `0`: zero means the buffer ran out under the play head, `null` means the play head is not where the buffer is, which is what a forward seek into undownloaded audio leaves. Both shut the gate; only one means the element is stuck. The front edge carries a 0.5 s tolerance because elements report `currentTime` a hair outside the range they are playing from.
- **The buffering line and the resume path ride on the same claim, so they moved with it.** `stalled` used to set the flag behind *"⋯ buffering — press play to retry"*, which also hides the chapter title in the mini-bar and arms the re-source path on the next play press — over sound that was running, and with nothing to clear it, since `playing` only fires after playback actually stopped. Saying "buffering" over audio that never broke is the same error on a second surface, so `noteStarvation` sets it only when nothing is playable from the play head.
...
- **Two pins, and they answer different questions.** `npm run check:art` pins `playableAhead` and `artGateOpen` — the arithmetic — and both were already correct while the flicker was live, because the fault was in which event called them and whether it measured at all. `npm run e2e:artgate` drives the built app, dispatches each event at the real element and asserts on the SEQUENCE of hero `src`s at 100 ms: scenario 1 is the flicker (`episode-456.jpg -> behind-the-curtain.jpg` before the fix, one entry after it), scenario 2 is the yield that must survive fixing it. An endpoint read shows neither, because the gate reopens inside a second — which is what makes this a flicker rather than a stuck cover.
...
- **It READS through the existing start-position path, and that is why the source effect was not touched.** `play()`, `stepTo` and the four listen-queue sites (`queueStepTo`, `playFromQueue`, `revealQueue`, `handlePlaybackEnded`) put `savedStartSec(...)` into `positionSec`, in the same update as `current`; `<Player>`'s source effect already seeks to that on `loadedmetadata` and already forces 0 for `isLiveMedia`. The streaming engine takes its ledger baseline from the same `positionSec` in `openContext`, so a resume is not billed as a forward seek — a `requestSeek` after a play from 0 would have been (one tick, but a jump all the same).
- **Podcast episodes only.** `canResume` refuses a `playsAsTracks` feed (a track restarts at 0:00), an HLS enclosure, and a `live`/`pending` item. It must refuse everything `<Player>` forces to 0: otherwise the store's `positionSec` claims a start the element never takes, and the streaming ledger's baseline is wrong by that much.
- **Under 15 s is never WRITTEN — and never deletes either.** A new source resets `lastTick`, so the element's first `timeupdate` reports ~0 before `loadedmetadata` seeks. Writing that would erase the saved point during the very load that returns the listener to it. Do not "fix" the floor into a delete.
- **Within 30 s of the end is FINISHED, and the entry is deleted.** `ended` clears an entry with no change to `onEnded`: it sets `isPlaying` false, the writer flushes on pause, and the element's `ended` flag sends it to `forgetPosition` — which needs no duration, so a file that never reported one still clears instead of resuming at its own end forever.
- **An element reporting an INFINITE duration saves nothing** — it is a stream nothing tagged as live, with no place in it to come back to.
- **The writer is a raw `useApp.subscribe`, never a selector.** Selecting `positionSec` would re-render all of `<Player>` every second. And the OUTGOING episode is flushed from `prev` inside that listener: `play()` replaces `current` and `positionSec` in one `set()`, so after the render the old position is gone. The listener runs synchronously inside `set()`, before the source effect repoints the element, so the element's `duration` is still the old file's.
- **Flush points:** episode change, pause, every 10 s of movement (a scrub included), `pagehide`, and `visibilitychange` → hidden — iOS kills a backgrounded home-screen app without `pagehide`. Audio that keeps playing while iOS has suspended the page's JavaScript is not recorded; the last write before suspension is what survives.
...
**Every page-level view is URL-restorable on refresh** — `?podcast=<guid>` (detail), **`?feed=<id>`** (detail fallback for shows with no `podcastGuid`, resolved via `/api/feed`), `?episode=<guid>`, **`?discussion=1`** (layered on podcast/feed + episode; needs `socialInteract` from `/api/feed`), **`?t=<seconds>`** (a link to one moment, layered on podcast/feed + episode), and **`?publisher=<feedUrl>`** (reconstructs a minimal stub, so the back-button label reads "Publisher" on a cold restore). Live streams use `/stream/<naddr>`. All restores re-check `useApp.getState()` before `set` and gate on the PI breaker. **A restore reads the query captured at LOAD, not `window.location.search`** — the URL-mirror effects are declared first, so they rewrite the address bar from unhydrated state before a restore looks; that silently broke `?publisher=` entirely. See feeds.md. The PLAYER is NOT restored on reload — only the view. (An episode played again starts where it was left — see "Resume position" above — which is a different thing: nothing plays until someone presses play.) **`?t=` is the one exception, and it is an explicit request rather than a resume**: the restore calls `play(episode, podcast, startSec)`, the same path a chapter tap already takes for a not-current episode, so `<Player>` applies the position on `loadedmetadata`. A browser that blocks the autoplay costs nothing — `player.tsx`'s `el.play().catch(() => setPlaying(false))` has already flipped the flag, so the episode sits loaded and parked at that second and the transport never claims to be playing over silence. It is guarded on `!useApp.getState().current`, so a replay cannot yank playback the visitor has already started.
...
**A show-level share and an episode-level share are different links, so where both are on screen they are different buttons.** `showShareUrl` takes an optional `episodeGuid` and adds `&episode=<itemGuid>`, and an optional `startSec` on top of that adding `&t=<seconds>`; the same `<HomePage>` mount effect that resolves `?podcast=` then loads that episode and opens it, so an episode link is a real deep link and not a show link with a suffix. A surface with one subject shares that subject — the show header a show, the episode page an episode. The **fullscreen player** presents both, and its `<ShareTargets>` renders both: `[↗ SHOW] [↗ EPISODE]`, or `[↗ ALBUM] [↗ TRACK]` on a music feed, sitting under the two hearts that already name their targets the same way. One button there has to pick, and either pick is wrong for someone — the show link loses the episode you were listening to, the episode link loses the way to hand somebody the show — and neither failure is visible from the sharing side, because the copy succeeds and the URL reads plausibly. `targetWord` moved from `fav-heart.tsx` to `lib/util.ts` for this: the hearts stopped being the only control that needs the noun, and the alternative was one component importing another for a string. The player's live-stream SHARE is a separate component rather than a branch inside this one — it renders only in the live-chat pane, where `liveStreamId` is the condition of the branch, so the podcast case inside it was unreachable code that read as a fallback.

**A third target exists and it does not live in that cluster: a link to one MOMENT.** Every row of `<EpisodeContents>` carries an icon-only `<CopyLinkButton>` copying `showShareUrl(podcastGuid, episodeGuid, row.startTime)`. It is a row control rather than a header button because the subject *is* the row — a header button would have to ask which moment, which is the question the list already answers.

**Every row gets one, chapter and track alike, and that is the merge rule showing through rather than a widened scope.** `mergeEpisodeContents` drops a chapter that starts within 2 s of a window — 14 of 31 on Homegrown Hits 146 — so a button gated on `kind === 'chapter'` would be missing from exactly the moments a listener most wants to send, with nothing on screen explaining the gap. A timestamp is a property of the row, not of which source produced it. This is deliberately unlike the heart, which needs the window's `feedGuid`/`itemGuid` pair and therefore genuinely cannot ride on a chapter.

**The call site must withhold the builder entirely when the episode has no guid — a builder that falls through hands every row a SHOW link.** `showShareUrl(guid, undefined, t)` drops the `t` as well (a timestamp indexes into an episode, so it is written only alongside one) and returns the plain show URL, so the row would offer a button that copies something about the whole show. Both call sites pass `undefined` rather than a closure in that case; it is the same guard `<ShareTargets>` makes a few hundred lines up, for the same reason. `startSec` is also floored — a chapters JSON `startTime` is fractional (`5045.605`) while the row reads as a whole second — and dropped at zero, where it would differ from the plain episode link in noise alone.

**`<EpisodeContents>` still takes no `episode`/`podcast` prop.** It takes `shareUrlFor?: (startSec: number) => string | null`, which keeps URL building at the call site — the rule `<CopyLinkButton>` exists to enforce — and keeps the guard above where `episode` and `podcast` are in scope. The fullscreen player threads it through `<EpisodeInfoPanel>`, which has never carried either. The button is a **sibling** of the seek button beside the chapter's `↗` anchor, never a child of it: a button inside a button is invalid HTML and browsers reparent it, dropping the row's own click target.
...
**The MORE menu portals to `document.body`, for that second reason.** `{children}` sits inside `<div className="relative z-0">` (`app/layout.tsx`), which is a stacking context — so no z-index inside the page can beat the root-level `<TabBar>` and mini-bar at `z-30`. Rendered in place, the menu opened downward from a row that scrolls and its items were painted over by the dock. It is `fixed`, positioned off the trigger's rect and re-measured on scroll and resize, flipping above the trigger when there is no room below. This is the same rule that sends modals to `document.body`; a dropdown near the bottom of the viewport has the identical problem.

**Its outside-click guard tests the trigger AND the menu, with `?.` on both.** The trigger is conditionally rendered — an episode with no tracks and no `link` has no MORE tile — so the usual `if (ref.current && !ref.current.contains(t))` idiom, copied from `<AuthControl>` whose wrapper is always mounted, turns "the trigger went away" into "do nothing": the flag stays true, the effect never re-runs its cleanup, and the mousedown and keydown listeners outlive the menu. Come back to an episode that has the tile and it is already open with no gesture. The episode-change effect resets the flag as well.
...
**`<SelectMenu>` (`components/select-menu.tsx`) — one choice out of a fixed set.** Three surfaces, and the first two arrived at it the same way. The search box's content-type selector and `/favorites`' library filter are the same control: a short list of mutually exclusive options over a list you are looking at. Both shipped as a ROW of chips, and both rows ran off the edge of a phone — the search chips wrapped to two lines at 320px, and the favorites row (up to sixteen segments once every medium carries three) clipped mid-word at 390px, `PO…` and `⚡ N…` sliced in half. **A row that scrolls does not read as scrollable when its last option is cut**, and the options past the cut are the ones nobody thinks to look for.

Writing the second dropdown as a copy of the first is exactly the drift this section exists to record, so there is one: trigger, `role="menuitemradio"` items with the fixed ✓ gutter, both dismissals, and a `max-h` the favorites list needs and the search box's five never reach. **It PORTALS to `document.body`** — both of today's callers sit near the top of their page and would survive `absolute`, but a third one lower down would be painted under the dock rather than error, for the `relative z-0` reason in the dock section. `triggerLabel` exists because an indented option needs to name its parent when the menu is shut: "albums" does not say which medium's.

The third caller is `<FavoritesPrivacyControl>`, and it is the one that shaped the harder half of the API: `active` accepts **null** with a `placeholder`, because an account that has never chosen a privacy mode is not Public and must not be shown as such; and an option can be **`disabled` with a `title`** rather than omitted, because a withheld choice that is simply absent cannot say why. Neither is decoration on the other two callers — they are rules that were already written down for the control that needed them.
...
## Open items from the dock redesign
...
- **The page-level `pb-32`s are GONE, and the rule that replaced them is in the dock section above**: the layout footer carries the only bottom clearance. `app/favorites`, `app/playlists`, `app/npub/[npub]`, `app/privacy`, `app/error`, `app/downloads` and `app/queue` each had one, and `<HomePage>`, `app/not-found` and `app/amber-callback` each had a `min-h-screen` on top of `<body>`'s. Fixed 2026-09-22, on the user's report that a short page scrolled with nothing below the fold.
...
**Space / `k` toggle play, ← / `j` skip back 15 s, → / `l` skip forward 30 s, on every route** (`components/player/use-player-hotkeys.ts`, mounted once by `<Player>` in the root layout). The keys are the ones every video site has trained people on; a media player on a laptop is missing something until it has them.

**The whole difficulty is when NOT to fire, and each guard is a real breakage.** Nothing fires while an element that raises a keyboard has focus (`raisesKeyboard`, the same predicate the dock uses — Space in the search box is a space); nor while a button, link, menu item, tab, switch, select or range input has focus (each already uses Space/Enter/arrows for itself, and the seek slider scrubs with the arrows); nor while a `[role="dialog"]` is open (its focus trap owns the keyboard); nor with a modifier held (⌘←, Ctrl+Space are the browser's); nor with nothing loaded. `preventDefault` is called ONLY on a key that was handled — Space would otherwise scroll the page, and a swallowed unhandled key is how a shortcut layer breaks find-in-page.

**Menus and tabs got their keys in the same pass.** `useMenuKeys` (`components/use-menu-keys.ts`) gives every `role="menu"` — the search-type / favorites-filter `<SelectMenu>`, the sign-in dropdown, the account menu, the episode page's `⋯ MORE` — initial focus on the checked item, ArrowUp/Down/Home/End, and Escape/Tab that close AND return focus to the trigger (only when focus was still inside the menu, so an outside click that landed on another control keeps it). `<UnderlineTabs>` carries roving `tabIndex` and ←/→/Home/End, which is what declaring `role="tablist"` promised. A `role` is a promise of a keyboard pattern; declaring one without the keys tells a screen-reader user "menu" and then leaves the arrows inert.

**`window.confirm()` is gone.** `<ConfirmDialog>` / `useConfirm()` (`components/confirm-dialog.tsx`) sits on `<ModalShell>`, so a confirmation has the focus trap, `aria-modal`, Escape and the shared scroll lock like every other modal — and it names the action on its button ("Overwrite backup"), never "OK". The four sites were the two Spark seed overwrites, the local-key sign-out and a mute; a native dialog is unstyleable, is suppressed outright once the browser's "prevent additional dialogs" box is ticked, and is unreliable inside the installed app. Cancel is the first focusable, so a stray Enter cannot confirm an erase.
