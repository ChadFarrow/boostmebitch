...
On the note card the control is a **sibling of the heart**, on the heart's own `favoritable` gate, and not in the note's action row above — that row is social actions gated on `identity`, and a listen queue needs none, so it would be hidden from exactly the signed-out reader who can use it. In an episode row both controls are siblings of the tap targets, never children: a button may not contain a button. **The row control is hidden below `sm:`, and the number is why.** Measured A/B on the same row, control present vs `display:none`, on Homegrown Hits 149: it costs the episode title a flat **48px** under `sm:`, which at **320px takes the title from 90px to 42px** — the "Vi…" / "Co…" collapse the BOOST button on that same row already sheds its word to avoid. From `sm:` up it costs 84px of a 265px title and leaves 181px, which is fine. So: 320 / 390 / 639px are byte-identical to the row before the queue existed, and 640 / 1280px carry the control. **A hide, not a drop** — the episode page carries QUEUE as a tile at every width and is one tap away through the row's own handler, the same trade `<VideoToggle>` and `sidesOnDesktopOnly` already make.
