...
**The controls are THREE ROWS and the page is judged on how fast you reach a favorite.** They were six — a medium strip, a half row, a filter box, a sort pair, the privacy segments and their sentence, and the backup tools — and on a 287-entry library that was most of the first screen on a phone. Now: the privacy menu and its consequence sentence share one row, the backup tools keep theirs, and the library filter, the text box and the sort toggle share the last one. The `<SelectMenu>` is what buys the first and last of those; the numbers it folds away are on the trigger.
