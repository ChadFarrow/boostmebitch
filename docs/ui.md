...
### Removed: `⌕ CHECK PRIVATE HALF` and `⇄ MERGE ENCRYPTED HALF IN` (2026-09-03)

Two controls sat on this row between #289 and 2026-09-03: one opened the encrypted half and counted what was in it, the other folded those entries back into the public list. They were removed as clutter on a page whose job is a library. **The state they existed for was not removed with them**, and nothing in the app detects it now, so the findings are kept here rather than deleted with the code.

**A list can hold entries in the plaintext `i` tags and in the encrypted `content` at the same time.** Nothing in the format forbids it and three ordinary routes lead there: another app wrote the private half; a mode switch ran on a device whose baseline claimed nothing and therefore copied instead of moving; or the account changed mode twice. Found on a real account at **284 entries in both halves at once**, and on another at **38,320 characters of NIP-44 ciphertext beside 445 public `i` tags** — both with the mode reading Public and every screen in the app agreeing nothing was wrong. The ambiguity warning cannot fire on it either: `modeAmbiguous` is gated on `!seeded && !recorded`, and those devices had a recorded mode.

**It is invisible because neither half is counted.** The public half renders, the private half is carried byte for byte by the carry rule, and no screen states the size of either.

**What blocked the repair was the BASELINE, correctly.** The removal was gated on `baselineHalf(baseline, 'private')`, and a device that never published the private half claims nothing in it — so the merge dropped nothing and the ciphertext rode forward on every publish forever. The guard is right: a baseline is a promise that these entries are ours to delete, and this app must not delete a private half it cannot prove it owns. What was missing was evidence of ownership, which is why the two controls arrived in that order and why re-adding one alone would not help.

**Two rules from that work that still bind code in the tree.** `favoriteEntries` (`lib/favorites-audit.ts`) survives because a group WITH items is a placement group and not a favorite — counting every `podcast:guid:` tag reported **217 albums to a user who has 56**, and `⇩ BACKUP` and `⇧ RESTORE FROM BACKUP` both state entry counts that a user checks against the `N SAVED` on the same page. And a decrypt is spent only on `'user-initiated'`: an external signer renders the plaintext on its approval sheet, so an unasked one puts somebody's private favorites full-screen uninvited.
