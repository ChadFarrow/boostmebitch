...
- **Six page-level `pb-32`s should read `--dock-b`** — `app/favorites/page.tsx`, `app/playlists/page.tsx`, `app/npub/[npub]/page.tsx`, `app/privacy/page.tsx`, `app/error.tsx`, and now `app/downloads/page.tsx`. Five of them predate the dock; `<LivePage>` was written after this was recorded and uses `calc(var(--dock-b) + 8rem)`, the same expression `<HomePage>` uses. **`/downloads` is the first page to copy the wart after this warning was written**, which is recorded here rather than quietly fixed, because the count is the whole point of the entry. A new page copying one of the five is how a documented wart becomes a convention. Nothing clips, because the layout footer's `calc(var(--dock-b) + 7rem)` sits below all of them. The cost is dead space and a rule with five exceptions: change `--tabbar-h` and two elements move, not seven.
