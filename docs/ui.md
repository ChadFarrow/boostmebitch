...
- **The lock screen is the one surface that deliberately does NOT follow the chapter** (`navigator.mediaSession.metadata`, keyed on `current?.episode.id` alone). An OS artwork fetch is issued on our behalf: it takes no priority hint and is not cancelled when the next chapter supersedes it, so it can't be gated the way an `<img>` can. One fetch per episode, not one per skip.
