...
**Wallet is a modal, not a route, and that is why `<WalletModal>` moved to the layout.** The tab flips `walletOpen`, the same store flag the header's balance chip flips. That flag was only ever *read* by `<AuthControl>`, which renders on the three `<AppHeader>` routes — so a tab bar on every route flipping it would have been a **dead control** on `/live`, `/stream` and `/npub`: flag set, nothing mounted to answer, no error. `<WalletModalHost>` renders it from the root layout instead. This is the same failure and the same repair as `<FavoritesPrivacyPrompt>` and `onFavoritesModeNeeded` — a single consumer mounted on fewer surfaces than the controls that address it.
...
- **Five page-level `pb-32`s predate the dock and should read `--dock-b`** — `app/favorites/page.tsx`, `app/playlists/page.tsx`, `app/npub/[npub]/page.tsx`, `app/privacy/page.tsx`, `app/error.tsx`. Nothing clips, because the layout footer's `calc(var(--dock-b) + 7rem)` sits below all of them. The cost is dead space and a rule with five exceptions: change `--tabbar-h` and two elements move, not seven.
...
- **`<UnderlineTabs>` and the favorites row declare `role="tablist"`/`role="tab"` without the keyboard contract**: no `aria-controls`, no `role="tabpanel"` on the panels, no roving `tabIndex`, no arrow keys. That is a step back from plain unroled buttons, which promised nothing. The favorites filter is a `<SelectMenu>` now and no longer has this problem; the two content strips should either complete the tab pattern or drop the roles.
...
- **`/live/<npub>` and `/stream/<naddr>` had no header by design and now get a tab bar**, and their full-screen `z-40` loading covers paint under it (the `relative z-0` wrapper again). If that is wrong, `<TabBar>` needs a hide list.
