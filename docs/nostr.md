...
- **The artwork, in the BODY as a bare URL and in an `imeta` tag** (`boostArtUrl` + `withArt`). A boost note is a public post about one track, and text-only it is a wall of prose beside every other client's — the reader cannot see what was boosted without opening a link. **The body URL is what actually renders the picture**; a NIP-92 `imeta` describes an image the body already names, it does not add one, so the tag alone shows nothing anywhere. Four rules:
  - **Preference order is `episode.image` → `episode.feedImage` → `podcast.image` → `podcast.artwork`** — the item's own art first, then the same channel-level pair, in the same order, that `<PodcastCover>` tries on screen (PI mirrors RSS `<image><url>` as `image` and `<itunes:image>` as `artwork`, and they routinely disagree).
  - **Gated on `isImageUrl`, not on `httpUrl` alone.** A URL no client recognizes as an image is not neutral: it renders as a third bare link under the two the note already carries, which reads worse than no picture. **`isImageUrl` lives in `lib/util.ts`, not in `lib/format.tsx`** — the writer (`boost-notes.ts`) and the reader (`extractImages`, which pulls the URL back out of the body so `<NoteCard>` can render it big) must agree on one extension list, and `format.tsx` is a `'use client'` React module that must not be imported from inside the Nostr boundary. Two copies is how the writer comes to emit a URL its own card shows as a link.
  - **Appended by `withArt` in `buildBoostNoteTemplate`, above the mentions** — same convergence point as `withMentions` and for the same reason: `BoostAllModal`'s `contentOverride` replaces the whole body, so art added inside `formatContent` would be missing from every boost-all note. Above the mentions because a trailing `nostr:npub…` run is what every compose box writes last.
  - **The `imeta` tag is dropped when `url <art>` exceeds 512 characters.** The site-sign route rejects the WHOLE template on one over-long tag item (`MAX_TAG_ITEM_LEN`), so a long signed-CDN artwork URL would stop a signed-out user's note publishing at all. The body still names the image, so the soft failure costs nothing on screen. **The route's `ALLOWED_TAG_NAMES` had to learn `imeta` in the same change** — that allowlist is fail-closed, and a new tag emitted here without it breaks every site-signed note.
...
[artwork URL — rendered as the picture, when one has an image extension]
