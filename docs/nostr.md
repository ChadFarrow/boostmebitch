...
- **REPLIES AND QUOTE REPOSTS CARRY MENTIONS TOO, through the same leaf** (`mentionParts` in `lib/nostr/interactions.ts`). The composer in `<NoteCard>` is `<MessageInput>` — the same picker, which is why that component moved out of `components/boost-modal/` to `components/message-input.tsx` when a third surface wanted it. Three things about that path are not obvious from reading it:
  - **`selfSigned` is hardcoded `true`, and it is a fact rather than an assumption.** `publishReply` and `publishQuoteRepost` both go through `signAndPublish`, which reads `activeNostr()` and throws without a signer, so the note is always signed by the person who typed it — there is no site-signed reply route for the gate to protect against. **If one is ever added, that argument becomes a parameter on the same day.**
