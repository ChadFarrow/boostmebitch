...
- **The `a` root tag stays FIRST and the `p` tags are appended after it.** That tag is how every other NIP-53 client finds the message in the room at all; the mention tags are additive and change nothing about the shape those clients read.
