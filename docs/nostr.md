...
**It presents as absence, not as an error.** On a device with a local cache both lists still paint, so nothing looks wrong; on a fresh one it is the whole feature. Signing in on a second browser showed no favorites at all and a mute list that filtered nobody — both events lived on the user's own write relays and the defaults had never been asked. A kind:10000 makes that the *common* case rather than the exotic one, because Damus and Amethyst publish it to the user's outbox and not to ours. `hydrateMutes` compounded it by passing `undefined` for `queryRelays`, which `fetchMutedPubkeys` reads as `DEFAULT_RELAYS` — the read was narrower than the write in that module too, independently of the race.
