...
- **REPLIES AND QUOTE REPOSTS CARRY MENTIONS TOO, through the same leaf** (`mentionParts` in `lib/nostr/mention-tags.ts`). The composer in `<NoteCard>` is `<MessageInput>` — the same picker, which is why that component moved out of `components/boost-modal/` to `components/message-input.tsx` when a third surface wanted it. **`mentionParts` itself lives in the import-free leaf rather than in `interactions.ts`**, because the one function that has to take both halves together was otherwise the one thing `check:mentions` could not see, and a new publisher would have been free to hand-roll it and drop a half again. It carries eight vectors of its own, six of which `naive()` — keep the inlined content, tag everybody picked — gets wrong. Three things about that path are not obvious from reading it:
  - **`selfSigned` is a REQUIRED argument of `mentionParts`, passed `true` at every call site, and that is a fact rather than an assumption.** `publishReply` and `publishQuoteRepost` both go through `signAndPublish`, which reads `activeNostr()` and throws without a signer, so the note is always signed by the person who typed it — there is no site-signed reply route for the gate to protect against. Required rather than defaulted for the reason `decryptWithTimeout`'s `purpose` is: only the layer that knows which key will sign can answer it, and a default of `true` would let a future site-signed surface acquire the amplifier by saying nothing. **If one is ever added, it answers this differently on the same day.**
