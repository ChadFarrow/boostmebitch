...
**This app is now a WRITER of that wrapper shape too, and the reading side needed no change.** A boost whose lnaddress leg qualified goes out as a real NIP-57 zap, and `buildBoostNoteTemplate` quotes the resulting receipt with a `q` tag ONLY — `['q', id, relayHint, zapperPubkey]`, the same shape `publishQuoteRepost` writes — and puts no `nostr:nevent1…` line in the body. It wrote both at first, and the first production boost showed why not: every general client unfurls a body reference into an embedded zap card under the note, one per zapped leg, reading "Sent 33 sats to …" beneath a note that said 100. The tag form is what a general client leaves alone. Fountain reads the `q` tag (its own notes carry only the body form — two captured in `scripts/check-zap-receipt.mjs` — but its reader takes either, confirmed 2026-09-16); `parseQuoteRefs` reads `q` and body alike, so the dedupe below is unchanged. `check:zapreceipt` now fails if the body form comes back. The note's NIP-73 `i` tags now carry this site's show or episode page as the optional third element, the URL hint Fountain writes on every boost note; nothing parses it back. Boost All's summary quotes nothing, on purpose — see [`money-boosts.md`](money-boosts.md). Two consequences fall out of machinery that already existed: `quotedEventIds` dedupes our own wrapper against its receipt in `<BoostExplorer>`, exactly as it does Fountain's, so one payment stays one card; and `buildNote` leaves the amount alone, because it adopts a quoted receipt's figure only when the wrapper carries no `amount` tag and ours always does. Why the receipt rather than our own tag, and why the quoted figure is a per-leg number, is in [`money-boosts.md`](money-boosts.md) — that is the money side, and the rules that can lose funds or name the wrong payer live there.
