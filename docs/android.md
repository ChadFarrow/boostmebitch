...
7. **Link the signing certificate to the publisher identity, once, by hand.** zsp checks the relays for a kind 30509 identity proof before publishing and, in `--quiet` mode, treats a missing one as a warning rather than a prompt — so the first CI publish goes out unlinked, and Zapstore cannot show that the key that signed the APK belongs to the npub that published it. The proof needs the keystore, which CI deliberately deletes, so this is a local step:
   ...
   **CAUTION: the file called `boostmebitch-release.jks` is NOT a JKS, and zsp picks its loader from the EXTENSION.** `keytool` has written **PKCS#12** by default since Java 9 whatever filename you give it, so the store's first four bytes are `3082…` (a DER SEQUENCE) rather than JKS's `feedfeed`, and `--link-key …jks` fails with `load JKS: got invalid magic`. Point zsp at a `.p12` path instead — a **symlink**, never a copy, because duplicating a signing key on disk is how one ends up somewhere nobody is tracking. `--key-alias` is a JKS-only flag and is not needed for PKCS#12. Check the bytes before believing the name: `xxd -p -l 4 <keystore>`.

   **This step needs a real TTY and cannot be driven through an agent.** zsp prompts for the keystore password, and a prompt needs a terminal — `KEYSTORE_PASSWORD` in the environment or a `.env` is the only non-interactive route (`internal/config/wizard.go`), which puts the password somewhere it is easy to forget it lives. Run it by hand. Do not wrap the call in `OUT=$(...)` either: command substitution swallows the prompt, and the symptom is a blank screen that looks like a hang.
...
| **The signing certificate is linked to the publisher identity** | 2026-09-04. `zsp identity --verify` against the published APK: cert hash `4201c774…` **match YES**, status **ACTIVE**, signature **VALID**, expiry **2028-09-03**. Before this the relays held exactly one kind 30509 for this npub — `2d1e9bfc1810` from 2026-04-18, `d=27f4191931ee…`, which is StableKraft's certificate and not this one |
...
| **The buddy TWA sends real boosts, and the wire carries the right brand** | 2026-09-04, on the Pixel 6, installed from Zapstore. Two boosts out of `com.boostmebuddy` — 2222 and 1111 sats — and both published kind:1 notes carry **`client=BoostMeBuddy`**, against `client=BoostMeBitch` on two boosts sent from the bitch origin earlier the same day. That is the brand system proved on the one surface that cannot be edited afterwards. Streaming sats reported working in the same session; it publishes no kind:1 by design, so there is nothing to read back for it |
| Rows 2–7 are still unmeasured on EITHER build | A boost proves the money path, not row 2: nobody has tested NWC after five minutes backgrounded, the reconnect dial storm, or that an unanswered wallet renders `?` and not `✗` |
...
## A fork of the APK keeps your origin, and the referrer is the only thing that says so

**A TWA wraps an origin, so a fork that keeps `host` runs on this deployment.** Change the package id, sign with your own key, publish on Zapstore — and every `/api/*` call, the Podcast Index quota, the BoostBox proxy and the site signer are still ours, while the fork shows this brand and stamps `BoostMeBitch` on every boost it sends. StableKraft had exactly this happen. The Digital Asset Links statement does not stop it: a fork's certificate is not in `ANDROID_CERT_SHA256`, so Chrome shows the URL bar in the fork — and still loads the site. Nothing at the HTTP level stops a browser opening a website. A fork that changes `host` to its own domain is the one that does NOT touch this deployment; it then needs its own Podcast Index key.

**The signal is the referrer.** When an Android app opens a URL in Chrome — a TWA or a Custom Tab — the first navigation carries `Referer: android-app://<package>/`, and `document.referrer` holds it for that document. A verified build of ours reports `com.boostmebitch` / `com.boostmebuddy` (`BRAND.androidPackageId`, pinned to the manifests by `check:brand`); a fork reports its own package. `lib/launcher.ts` reads it and `<UnofficialAppNotice>` (root layout) names the package, says whose servers this is, and links the official build.

**It is a notice and must never become a gate.** Every Android app that opens a link reports its package the same way — Telegram, Signal, Fountain, a mail client — so "a package that is not ours" is, most of the time, a person tapping a shared link. Refusing on it would break every Android visitor who arrived from a chat. Two heuristics narrow it to a wrapper: a wrapper launches at its `startUrl` (`/` with no query, where a shared link is nearly always a deep link), and the package is remembered for the tab only when the landing matched that. A determined fork can strip the referrer or use a WebView; the remaining defences are the ones that hold anyway — the per-IP limits and per-route ceiling in `lib/rate-limit.ts`, and the fact that no credential leaves the server. Watch Vercel usage and the Podcast Index quota after a suspicious listing appears.
