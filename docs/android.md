...
*Two things a TWA changes that are NOT yet verified.* `scope: "/"` already covers `/amber-callback`, so no manifest change was needed — but with the app installed and verified, Amber's `ACTION_VIEW` on this origin may resolve to the **app** rather than to the browser tab that made the request. Different storage partition, no pending record, so the callback page falls back to a manual paste. And `navigator.clipboard.readText()` still needs a focused document plus, on Android 10+, foreground status and a permission prompt whose behaviour in a TWA remains unproven — which matters because every request other than sign-in still depends on it. *Verify:* install a signed build, sign in with Amber, then exercise a background signing prompt (toggling a favorite debounce-publishes) and record which app wins the callback.
