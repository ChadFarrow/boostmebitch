...
- **The origins above are `boostmebitch.com` only, and Google's refusal of any OTHER origin is INVISIBLE from the app.** `boostmebuddy.com` is a second deploy of this same code (see `lib/brand.ts`), so it ships the same `<GoogleAuthPanel>` and the same GIS call — but the origin GIS reports is the domain it is served from, and an origin the client does not list is refused *after* the account chooser. The user sees a popup, taps their account, and the popup closes; GIS hands us `popup_closed`, the same type it uses for a real cancel. **That is why "Google sign-in was cancelled" is no longer the copy** — see the `popup_closed` branch in `lib/nostr/google-auth.ts`, and read the `[google]` console line, which names the origin and the client id it actually used.
- **A second brand needs its own Google Cloud PROJECT, not just another origin on this client.** Adding `https://www.boostmebuddy.com` to the origins above is the one-line fix that makes sign-in work, and it also puts **`BoostMeBitch`** — the app name, the logo and the privacy-policy link are verified as a set on the project's consent screen — in front of every person signing in on the family-friendly site. That is the exact leak `npm run check:brand` exists to stop, one layer below where that script can see. So the brand-correct arrangement is: one Google Cloud project per brand, each with its own consent-screen branding, its own brand verification against its own domain, its own OAuth client, and its own `NEXT_PUBLIC_GOOGLE_CLIENT_ID` in that brand's Vercel project. Verification is the slow half (brand review against a Search-Console-verified domain), and until it lands the buddy consent screen shows the unverified-app notice with a 100-user cap — which is a working sign-in, not a broken one. **Leave `NEXT_PUBLIC_GOOGLE_CLIENT_ID` unset on a deploy that has no client of its own**: `isGoogleAuthConfigured()` then hides the entry point entirely, which is honest, where a shared client is either broken or off-brand.
