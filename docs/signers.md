...
- **Primal on iOS and Android** (`lib/nostr/primal.ts`). NIP-46 over the SAME `nostrconnect://` pairing every other method here builds — same client key, same relays, same `perms` — so it needs no new machinery and is a bunker like the rest. What is Primal-specific is one launch URL, and it differs by platform for a reason worth keeping straight. **iOS takes the bare scheme**, dispatched from a scripted click, which a custom scheme (unlike Clave's Universal Link) allows; StableKraft ship exactly that. **Android must name the package**: Amber already claims `nostrconnect://`, so a bare dispatch opens the OS chooser and the *Amber* row and the *Primal* row would do visibly the same thing one list apart — `intent:<connection>#Intent;scheme=nostrconnect;package=net.primal.android;end`, the shape Conduit's `androidSignerConnectUrl` ships. **The query is sliced, never re-encoded**: the URI already carries its own percent-encoded `?relay=…&secret=…&perms=…`, and a second pass through `URLSearchParams` is how a pairing reaches a signer mangled. `primalConnectUrl` returns `null` on a URI carrying a fragment, whitespace or a control character, and the caller then renders no button — safe here because nothing downstream reaches the network with it.
...
v2 writes the unpadded length as a big-endian **u16**, so 65535 bytes is the
largest plaintext that can be expressed at all, and `writeU16BE` throws
`Error("invalid plaintext size: must be between 1 and 65535 bytes")` above it —
**synchronously, inside `sendRequest`'s own `try`**, which rejects the call with
that `Error`. No kind:24133 is published. No relay is contacted. The signer is
never told anything.

`trackBunkerCall` then read that `Error` as "a local throw we did not author",
called `markBunkerStale()`, and `<BunkerHealthBanner>` accused a link that had
not been used. RECONNECT succeeded, because there was nothing wrong with it, and
the next tap put the banner straight back.

**Why only following.** Every other event this app signs is small — a boost note,
a favourites list, a mute list, a profile. A NIP-02 kind:3 is the user's *entire*
follow list in one event, and each followed pubkey costs **77 request-bytes**:
`["p","<64 hex>"]` is 74, and NIP-46 nests the event as a JSON **string** inside
the request, so every `"` is re-escaped as `\"`. Measured against the real
`nip44.encrypt`:

| follows | `content` | request bytes | `encrypt` |
|---:|---|---:|---|
| 849 | empty | 65,492 | ok |
| 850 | empty | 65,569 | **throws** |
| 836 | 1 KB legacy relay list | 65,491 | ok |
| 837 | 1 KB legacy relay list | 65,568 | **throws** |

So the ceiling is **849 follows**, less whatever `content` the user's kind:3
carries. Under it, following works. Over it, a remote signer cannot sign a kind:3
for that user **at all** — there is no retry, no reconnect and no re-pairing that
helps, and the reply would hit the same cap coming back even if the request did
not.

**What was done, and what deliberately was not.**

1. **`nip46RequestBytes` / `nip46RequestFits` measure the request before it is
   sent** (`lib/nostr/nip46-errors.ts`, pinned by `check:nip46error`). The
   adapter's three send paths — `sign_event`, `nip04_encrypt`, `nip44_encrypt` —
   go through `sizedBy`, which rejects with `BunkerRequestTooLargeError` naming
   both numbers. **Measure the REQUEST, never the event**: the double-JSON
   escaping makes the request ~5.6% larger than the document you would measure
   instead (62,033 → 65,484 at 849 follows), which is a real band of follow
   counts that looks fine and throws. `naiveSize` in the check script is exactly
   that mistake, replayed over every vector. **Bytes, never `.length`** — the cap
   is on encoded UTF-8.
2. **The `id` is measured as empty**, so the guard under-estimates by the 8-20
   bytes of `${idPrefix}-${serial}`. That direction is the safe one: the guard
   must never refuse a request that would have gone through, and anything inside
   the margin still reaches the library, throws, and is recognised post-hoc by
   `isRequestTooLarge`. Both paths end at the same error, so the margin is free.
3. **`trackBunkerCall` no longer accuses on a local throw without asking.** A
   too-large request leaves `bunkerStale` exactly as it was — setting it blames a
   working link, clearing it would erase a fault an earlier call recorded. Every
   other local throw now takes the same `probe` the timeout branch takes: a pong
   proves both directions, and only silence marks it stale. This can make the
   banner rarer, never more common.
4. **Nothing is trimmed to fit.** A kind:3 is the user's whole follow list;
   dropping entries to get under the cap would publish a list they never asked
   for, to a replaceable event with no history, on every device they own.
5. **`<FollowButton>` says so on screen**, with the count, and stops offering
   `↻ retry` for the one failure a retry cannot clear. A `title` carried it
   before and a hover tooltip is not reachable on the phone the report came from.

**The limit is the spec's, so the remedy is not in this app.** Other clients that
can change a list that size either hold the key themselves or speak something
other than NIP-46. Chunking is not available to us: NIP-46 has no continuation
and a kind:3 has no partial update.
...
The entry point lives in the combined **`<AuthControl>`** header control, not a standalone button — signed out, `<NostrAuth>` renders **only** the modal (its hydration effects and `completeSignIn` still run). Opening it flips `signInOpen`; the modal is a portal'd overlay (same pattern as `wallet-modal.tsx`) carrying **one `view`**, `'menu' | 'qr' | 'bunker' | 'primal' | 'google'`: a list of named methods, each opening a screen with `← Back`.

- **The menu** lists only what can work on THIS device. The platform's own signer is INLINE and one tap — the Clave anchor on iOS, the Amber button on Android — and everything else is a row that drills down: *Scan a QR code* (`loginWithNostrConnect`), *Bunker URI* (`loginWithBunker`), and *Browser Extension* (`loginWithExtension`), which is always shown on a desktop (disabled, with a hint, when `window.nostr` is absent) and only when present on a phone.
- **`'primal'`, `'qr'` and `'bunker'`** are the detail screens. Each prepares its own pairing on the way IN — subscribing only. **`'google'` is the fourth and prepares NONE**, because it is not a signer: it mints a key for somebody who owns none, and what its row starts instead is a parked consent request, inside the click. It is a view rather than a flag beside one precisely so `goto` governs it like the rest — see the Google row below. Reaching a screen is the choice to use that method; tapping its button is the choice to LEAVE for the app, and those are not the same decision, which is why `onPrimalConnect` carries the same `launch` flag `onAmberConnect` does. The visibility retry passes `launch: false`: the socket may have died while the user was in the app, but the app already holds the pairing, and sending them back with no user activation for a request it is already showing is the defect that flag exists for.
- **The Primal row is phones-only.** A desktop pairs Primal by scanning, which the QR row already is; a launch link there would open nothing, because the app is on the other device.

**IT WAS A TWO-TAB STRIP — *Browser Extension | Remote Signer* — WITH *Option 1* AND *Option 2* INSIDE THE SECOND, and every part of that was wrong in a different way.** A tab HID half the surface: a desktop user with an extension landed on that tab and never learned a bunker was possible, and in the other direction, opening on the wrong tab meant every return-from-the-signer effect bailed and the relay ack had nothing listening for it. The strip labelled the choice by MECHANISM, which is not how anyone thinks — "I have Amber" is. And what it led to was NUMBERED rather than named, so the two things a user might actually own had no visible names.

The shape is StableKraft's (`ChadFarrow/stablekraft-app`, `components/Nostr/LoginModal.tsx`), which carries one `'menu' | 'bunker' | 'primal' | 'amber' | 'nip05'` and renders a flat named list, each row opening a detail view whose Back does `cleanupAmberConnection()` before `setView('menu')`. **Two deliberate divergences.** Ours FILTERS by platform — theirs gates only the extension row on `hasExtension` and offers *Amber (Android)* on a desktop and on an iPhone, and a row that cannot work is worse than no row. And ours keeps the phone's own signer INLINE rather than a drill-down, because Clave cannot afford two taps and should not want to: a Universal Link opens the app only from a genuine tap on a real anchor with a live `href`, so that control has to BE the anchor, which means its pairing is prepared before the row renders.

**`goto(next)` IS A TEARDOWN, not a render change.** Each screen owns a pairing listener, and an abandoned one keeps its own `SimplePool` for the rest of the 120 s window — against the per-host socket limit [`clave.ts`](../lib/nostr/clave.ts) records. It also **bumps every attempt counter**, because abandoning makes the in-flight `ready` REJECT and each flow's catch would otherwise write an error onto the screen the user has just left, which they would meet on the way back as if their own navigation had failed. And it resets `clavePrepared` / `qrPrepared` so returning to a screen subscribes again rather than showing a control nothing is listening behind.

**THE MUTUAL VISIBILITY GUARDS ARE GONE AND THAT IS THE POINT.** The generate effect carried `if (claveBusy) return` and the Clave effect `if (genBusy || pasteBusy) return`, because all three boxes were on ONE tab: two effects firing on one return resolved two subscriptions on one pairing — two live transports, `finalizeBunkerLogin` twice, `onSuccess`/`onClose` on an unmounted modal. They are separate screens now, so at most one pairing listener is ever attached and the guards would guard a state that cannot occur. **The Amber one stays** (`if (amberBusy) return`): NIP-55 and NIP-46 Amber share the same inline block, so that collision is still real.

**Preparing the pairing follows the CHOICE.** `'qr'` generates on entry — reaching that screen *is* the choice, and asking for a *Generate* press after picking "Scan a QR code" is the second tap "Option 1" used to charge for the first. This is not a walk-back of the desktop revert, it is the other half of it: the objection was a pairing before any choice, which put a QR plus a ~400-character URI on screen and pushed the paste box off the viewport entirely (measured at 954x906), besides opening two relay sockets and writing `bmb:nc_pending` for someone who may only have come to paste a `bunker://`. **The QR screen also stays signer-NEUTRAL** — heading it "Scan with Clave" was tried and names one signer on the one platform where the code genuinely is neutral. **Do not add a Clave-branded screen with its own QR either**: one pairing per session means it would draw the identical code twice.

  **The iOS Clave box is a button and a status line, and nothing else.** It grew its own pair — *1. Open Clave* (`clave://`) and *2. Paste from Clave* (clipboard → `loginWithBunker`) — under a divider, which is the bunker screen rebuilt inside one signer's panel. The modal then offered the same flow twice and the phone box was three controls deep. The convenience was real, so it MOVED rather than went: the clipboard read is the Bunker URI screen's **Paste** button now, so nsec.app and Amber-in-server-mode get it too (`onPasteFromClipboard`; it must be a real click — `readText()` needs transient activation and iOS renders its own Paste confirmation on top). What did NOT move is the `clave://` scheme escape inside the slow and error blocks: that is not a duplicate of anything, it is the only cure for a Universal Link a user has switched off by tapping the clave.casa breadcrumb, which no page can detect. **Clave's docs call a pasted `bunker://` the reliable same-device iOS path**, so the Clave box's own copy names the **Bunker URI** row in both the stall and the error case — one sentence away is reachable, a second copy of the section is not.

**The iOS box mirrors the Android one and sits in the same slot** — first on the method list, because on the phone displaying a QR the QR is not the way in. `isLikelyIOS()` gates it; that helper had been written and exported with zero call sites since the Amber work, and this is the first. Structurally it does NOT mirror it, and that is the point: the Android box's button dispatches, while `<ClaveConnectLink>` is an `<a href>` the user taps and `prepareClave()` only ever subscribes. There is no `launch` flag on the iOS side and nothing left for JavaScript to navigate.
...
**WHICH TAB THE MODAL OPENS ON IS A CORRECTNESS QUESTION, NOT A PREFERENCE**, and **every return-from-the-signer effect bails on `tab !== 'remote'`** — so on the wrong tab the ack arriving on the way back from the signer has nothing listening for it and the handshake fails where nobody can see it. `hasExt` answers it for a phone on its own: an iPhone has no extension, so Remote Signer is already the default and the Clave box is the first thing in it. `hasPendingNostrConnect()` is the case `hasExt` cannot answer — a pairing this tab LOST is resumed by an effect below whatever tab is showing, so the tab showing has to be the one that can report it. **There was a `signInIntent === 'clave'` and a "Sign in with Clave" row in `<AuthControl>`'s dropdown, and both are gone.** The row's stated job was to open this modal on the right tab; measured on an iPhone, "Sign in with Nostr" opens on that tab anyway. It was a second door into the same room, and the price of it was a menu listing the same sign-in twice. The signer is named in the Nostr row's SUBTITLE instead — *"Already have a key — Clave, Primal or an extension"* on iOS, Amber on Android — which does the one thing the row actually did: tell someone who owns Clave that this is their way in. Do not re-add it as a peer of "Sign in with Nostr"; Google's row is a peer because it mints a key for someone who has none, which is a different path, not a shortcut into the same one. **That sentence is the whole argument for where the Google row goes, two paragraphs down — read it as licence for that one, never as licence to delete it.**

Nothing is behind a tab any more, so a desktop extension user sees the remote-signer methods without looking for them. The modal owns its per-method busy/error state and the **iOS visibility-retry** that re-attempts the nostrconnect handshake when Safari suspends the relay WebSocket on app-switch. On success it calls `index.tsx:completeSignIn(id, kind)`. `login-methods.tsx` now holds only the shared `<AmberCompletion>` clipboard-recovery helper.

**`<GoogleAuthPanel>` IS A SCREEN OF THIS MODAL — `view === 'google'` — AND THE METHOD LIST CARRIES A ROW INTO IT.** It was a boolean, `googleOpen`, seeded once from `signInIntent` with **no setter**, and that made the panel reachable from exactly one control in the app: `<AuthControl>`'s header dropdown, which renders in `<AppHeader>`, which is four routes. Everywhere else — the fullscreen player's Sign in button, the live chat's *"sign in to join the chat"* — opens this modal with the default intent, so **on `/live/<npub>` a listener with no Nostr key was offered three ways to use the key they do not have**, and the way out was to leave the page. Reported from a live show. The row is **last and under a rule**: every method above it says *"I already have a key"*, and this one mints one — the same argument that lets it be a peer in the header dropdown, one level down. Its handler calls **`startGoogleSignIn()` first and synchronously** (the click is the only moment a consent popup may open; a `false` return is not a failure, it means the script has not landed and the panel will ask for one more tap), and **`preloadGis()` now runs when the MODAL opens** rather than when it opens *on the panel*, so that tap has the script. While the panel is open the method list is hidden, and it carries its own **back affordance** on every non-destructive stage: `confirmPin` → `setupPin`, a PIN screen reached from the picker → the picker, otherwise `onCancel()`. The `working` stage is excluded — an upload may be in flight, and abandoning it is how you get a blob in Drive with no local key.

**The modal can also open straight onto that panel via `signInIntent`.** `<AuthControl>`'s dropdown lists "Continue with Google" as a peer of the other logins and calls `setSignInOpen(true, 'google')`; the modal seeds **`view`** from that intent with a **lazy `useState` initializer — read once, never subscribed**, since a late store write would otherwise yank someone out of a half-entered PIN. **The intent outranks the pending-pairing resume** in that initializer, because it is an explicit request for a different path and the `ncPending` memo survives either way. The header entry exists because burying this flow inside "Sign in with Nostr" hid it from exactly the people it was built for; the row exists because the header is not on every route.

**`onCancel` LANDS ON THE METHOD LIST, and that is an inversion.** It was the modal's own close handler, on the argument that backing out should not drop the user into signer tabs they never asked for — but there are no tabs any more, the list is one flat set of named methods with Google's own row on it, and the panel's back control has said **"← Other sign-in options"** the whole time. **The one control promising the other methods was the control that threw them away.** It goes through **`goto('menu')`, never `setView`**, because that is the teardown every screen exits through and one line of it is load-bearing here: `if (next === 'menu') clavePrepared.current = false` re-arms the iOS prepare-on-open effect, so the Clave anchor comes back with a live subscription behind it. The `error` stage's own button takes the same label for the same reason — a label may not name one destination while the handler goes to another.

**Modal open-state lives in the store** (`signInOpen`/`setSignInOpen`), so other surfaces (the fullscreen player header — `<AuthControl overlay />` since it gained one, see [`ui.md`](ui.md) — and the live-chat composer) can open the one modal `<NostrAuth>` owns — don't mount a second `<NostrAuth>`, it would double the profile-load and focus-listener effects. Portal'd at `z-[60]` so it clears the fullscreen player (`z-50`).
...
**`activeNostr()` IS THE SIGNER SURFACE — do not read `window.nostr` for it.** Publishing the polyfill to the global is a COMPATIBILITY gesture so anything reading the standard property sees the active signer. Our own code reading it back made the global load-bearing, and then an extension that owns that property took the app down with it.

Measured: **Sidecar defines `window.nostr` non-configurable**, so neither an assignment nor `defineProperty` can replace it. Reported from a desktop — the Clave QR was scanned, **the pairing succeeded**, the ack came back and `get_public_key` answered, and the sign-in still failed on the last line of `activateBunkerSigner` with the raw words *"nostr" is read-only*. Everything that mattered had already worked, which is what made it read as a pairing failure. It was never Clave-specific: every signer this module installs ended in that same assignment, so on such a browser **no remote or local sign-in could complete at all**.

Two halves to the fix. `setWindowNostr` still tries the assignment, checks it took (a sloppy-mode no-op does not throw) and falls back to `defineProperty` — which wins the *common* lock shape, `writable:false` with `configurable:true`. And `activeNostr()` makes the global optional: the polyfill lives in this module, so a lock that cannot be beaten is merely a global we did not get to write. **Publishing is allowed to fail; it logs once and carries on.**

`getNip04`, `getNip44` and `canSignUnattended` go through it too — those are how the mute list, the favorites private half and the wallet backup reach a cipher, so leaving them on the global would have left every one of them unreachable on a locked page while sign-in appeared to work.

**`extensionNostr()` is the other question and must not be conflated.** `activeNostr()` answers "who signs for this app"; this answers "what did the browser bring", and they diverge the moment a polyfill installs. `loginWithExtension` and the extension-account-switch watcher ask this one, or they end up talking to our own adapter about itself.

Pinned by `e2e-mutes` **5j**, which injects the lock with `Page.addScriptToEvaluateOnNewDocument` — it must exist before any app script runs, which a `Runtime.evaluate` after load cannot reproduce — and runs **both lock shapes**, because they have different correct outcomes: configurable, the polyfill takes the global; non-configurable, the extension keeps it and the app works anyway. **Only the last assertion of each pass discriminates**: the relay handshake is independent of the polyfill and `bmb:signer` is just a string, so the test unlocks the private mute half and requires a decrypt to actually reach the bunker. Replayed against the pre-accessor build: the configurable pass is green and the **non-configurable pass fails** on both the session and the decrypt, exactly reproducing the report.

**SIGNING OUT REVOKES THE PAIRING AT THE SIGNER, and closing the socket is not that.** A NIP-46 pairing is state on the signer's side: end the session here and the app goes on listing this site as connected forever. **Clave caps a user at five connections**, so every sign-in/sign-out cycle that does not revoke burns one of five slots the user can only reclaim by hand, in another app. `logout` is the method for it (nips#2373, Amber #460); Clave lists it in `describe`, answers `"ack"`, and runs `performLogoutTeardown` **only once that ack has been published**, so a failed round trip leaves the session intact rather than losing it. It is auto-allowed there, so it costs no prompt.

`sendBunkerLogout` (`bunker.ts`) sends it with **`sendRequest('logout', [])`, not a library method, and that is a version fact**: nostr-tools **2.19.4 has no `logout()`** — the pinned build exposes `close()` and the generic `sendRequest` and nothing else. A later version added the wrapper and [CLAUDE.md](../CLAUDE.md) forbids the bump for an unrelated reason, so the method name is written out. It is **best-effort always** — timeout, swallowed rejection, no `trackBunkerCall` (a stale-transport flag is meaningless for a transport being destroyed) and no `withApprovalWait` (a ninety-second approval loop on the way out is absurd). A signer that does not implement it must never stand between the user and being signed out.

**`revokeBunkerSession` (`signer.ts`) inverts the teardown order, and the order IS the function.** The adapter reference is dropped and `window.nostr` restored FIRST and synchronously, so nothing can sign with a session the user has just ended — but the socket and subscription stay up until the `logout` settles, because closing them first leaves nothing to publish the request on. Every other path correctly does the opposite (`closeBunkerTransport`), which is exactly why this one is easy to "fix" into silence: sign-out succeeds either way, and the leftover connection is only visible inside the signer app.

**`revoke` is opt-in and only `signout()` opts in.** `abandonRestoredSession` must not: it runs when a restore FAILED — a suspended socket, a relay that did not answer — and the user asked to disconnect nothing. Revoking there burns a working pairing over a transient fault. Pinned by `e2e-mutes` scenario **5i**, which signs out through the account menu and asserts the request reached the stub; the stub answers `unsupported: logout` on purpose, because the assertion is about the request going out, never about the answer.
