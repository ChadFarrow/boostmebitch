...
**The still-open half:** an Amber user now has no way to restore a Spark or NWC backup at all, because the only path that read them was the automatic one. A user-initiated "restore my wallet from Nostr" action is the missing piece, and it is the same missing piece the private mute list has.
