...
- **Remote Signer** — *Option 1* (`nostrconnect://` QR via `loginWithNostrConnect`) and *Option 2: Paste Bunker URI* (`loginWithBunker`) stacked, plus **"Sign in with Amber"** (`loginWithAmber`) on Android and **"Sign in with Clave"** on iOS. Default tab when no extension is detected, **and whenever the modal was opened with the Clave intent or a pairing is still pending** — see below.
  **Off both phones, Option 1 IS the Clave box**: it is headed *"Scan with Clave"* and its pairing is prepared when the tab is shown, so the code is on screen without a *Generate* press. "Clave support for the web" was never a missing mechanism — a desktop pairs Clave by scanning, and the code it scans is the same memoized URI, same client key, same two relays, same `perms`. What was missing was that the code sat behind a generic button and named Clave only inside a list of four. **Do not add a second, Clave-branded panel with its own QR**: one pairing per session means it would draw the identical code twice on one screen. The heading leads with Clave; the sentence still names Primal, nsec.app and Amber, because the code really is signer-neutral. Not on Android (Amber's box is the primary path and prepares on its own tap) and not on iOS (a QR is useless on the phone displaying it).
