...
Everything below was measured on a Pixel 6 (Android 17, Brave default, no Chrome) against Amber 6.3.0. Where a claim is reasoned rather than observed it says so.
...
**The callback URL may contain no `?` and no `&`.** Amber URL-decodes the WHOLE `nostrsigner:` URI and then splits it on `?` and on `&`, so either character truncates the callback there. Measured: `…/cb?rid=SPIKE123#result=` came back as `…/cb<result>`. **This is why `9a9330f` could not have worked either** — its fix was to terminate with `&event=`, which is the literal example in the NIP-55 text and which Amber's parser eats the same way. The first attempt at this feature failed for two reasons and only one was ever written down. `assertCallbackUrlSafe` (`lib/nostr/amber-callback-url.ts`) is that mistake turned into an assertion, pinned by `npm run check:amber`.
...
- **Background publishes on Amber still do not complete through a callback.** Favorites and mutes debounce-publish a `sign_event`, which stays on the clipboard path — so on this phone they still depend on the user returning to a live tab.
