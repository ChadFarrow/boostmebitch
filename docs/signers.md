...
- **`'primal'`, `'qr'` and `'bunker'`** are the detail screens. Each prepares its own pairing on the way IN — subscribing only. Reaching a screen is the choice to use that method; tapping its button is the choice to LEAVE for the app, and those are not the same decision, which is why `onPrimalConnect` carries the same `launch` flag `onAmberConnect` does. The visibility retry passes `launch: false`: the socket may have died while the user was in the app, but the app already holds the pairing, and sending them back with no user activation for a request it is already showing is the defect that flag exists for.
...
**`<GoogleAuthPanel>` is NOT reachable from inside this modal.** There is no "Continue with Google" row on the method list — `googleOpen` is seeded once and has no setter, so the panel renders only when the modal was opened with `signInIntent === 'google'`. `<AuthControl>`'s header dropdown is the sole entry point. While the panel is open the method list is hidden, and it carries its own **back affordance** on every non-destructive stage: `confirmPin` → `setupPin`, a PIN screen reached from the picker → the picker, otherwise `onCancel()`. The `working` stage is excluded — an upload may be in flight, and abandoning it is how you get a blob in Drive with no local key.
