...
**The Universal Link is the primary and the custom scheme is the recovery — this was the other way round, and the reversal is worth the paragraph.** `https://clave.casa/connect/?uri=…` is Apple's own mechanism: iOS resolves it against Clave's cached `apple-app-site-association` and opens the app with **no "Open in Clave?" confirmation sheet**, which a `clave://` navigation does show. **Conduit** (github.com/Conduit-BTC, `conduit-mono`) ships exactly this form and verifies on a **physical iPhone** that the app launches on the first tap with no intermediate QR or tab step — a measurement, which is more than this repo has for the scheme it originally chose.

The first version rejected it on the grounds that the URI carries the ephemeral client pubkey and the connect secret to a third-party origin. That reasoning was **overstated in the direction that cost the user speed**. When Clave is installed iOS routes the link from the cached association without fetching anything, so nothing leaves the device on the path that matters; the URI reaches clave.casa only when Clave is *absent*, which is exactly when it is worthless — there is no signer to approve it. And the secret proves a pairing, not a key: it cannot sign, and it dies with `NOSTRCONNECT_TIMEOUT_MS`.

**Its failure mode is a navigation, and that is the real cost to state.** With Clave missing the tab goes to clave.casa's install page rather than doing nothing, so this page and the subscription waiting on the pairing are gone. Acceptable for the same reason: nothing was going to approve that pairing anyway.

**`clave://` stays, and not as dead code.** A Universal Link can be switched off by the user without their realising it — one tap on the "clave.casa" breadcrumb in Safari's top-right and iOS opens the web page for that domain from then on, permanently, with no UI to undo it and **nothing on the page able to detect it**. The custom scheme is unaffected, which makes it the only cure for the one failure the primary cannot report. That is what "Open the Clave app directly" in the timed hint is; the hint now offers both answers (wrong routing, or no app) rather than guessing between two nothings that look identical.
