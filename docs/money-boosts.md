...
**One provider list runs the upgrade backwards: `LNURL_ONLY_DOMAINS` in `keysend-lookup.ts`, currently just `fountain.fm`.** Fountain is not an edge case here — it is the provider the namespace discussion was written around (`customKey` `906608` routes to a Fountain sub-account) — and because it *does* serve `.well-known/keysend`, the upgrade fired on **every** `@fountain.fm` leg. The payment succeeded, which is exactly why it went unnoticed: the sats landed, the app reported success, and a boost from here never touched Fountain's LNURL endpoint at all — the side that attributes the payment to the recipient and shows them its metadata. The upgrade was doing precisely what it was designed to do; the design was wrong for this one provider. This is **still non-regressive** under the rule above, just read the other way: LNURL works on every rail and keysend doesn't, so moving a leg *back* to LNURL can only make it more likely to pay. Spark users gain outright — that rail is BOLT11-only, so `payKeysend` threw for these recipients before.
