...
**Anonymity applies to the payment, not just the Nostr note — and to BOTH sender fields.** When the share picker's **"Anonymous"** is active (`anonymous = !!identity && shareNostr && shareAs === 'site'`), every boostagram omits **`sender_id` *and* `sender_name`** — single plus boost-all per-track/host/summary legs. Each leaked the identity on its own:

- **`sender_id`** is the user's pubkey, which recipient aggregators resolve to their profile (avatar + name). Shipped first, while the note was already site-signed — the PFP leaked despite the promise.
- **`sender_name`** is the "From" field, which recipients display verbatim **and** which `formatContent` prints into the note body as `<name> boosted N sats`. So the site-signed "anonymous" note still read *"Crimson Rook boosted 100 sats"* under the BMB npub, and `BoostAllModal`'s hand-built `contentOverride` did the same. Both now derive from one `senderName` const per modal (`anonymous ? undefined : name || undefined`) — **don't reintroduce a bare `name` at a wire site**; that const and `anonymous` are what the two modals share by convention.

`<SenderName anonymous>` disables the From input, blanks it to a placeholder and says the name isn't sent — a filled-in "From" beside an anonymity promise reads as *"this is what recipients see"*. The typed value stays in parent state (not wiped), so flipping back to "My feed" restores it, and `storage.senderName` is still written — anonymity is about what leaves the device.

**`anonymous` is gated on `identity`.** Signed out, the picker is a bare checkbox with no anonymous option and every note is site-signed, so the typed name is that note's *only* attribution — but `bmb:share_nostr_as` is a single global key, so a user who chose Anonymous while signed in would otherwise sign out and silently lose their name with no control to restore it. `sender_id` is unaffected by the gate (it's `identity?.pubkey`, already absent signed out). "My feed"/"Don't post" identify the sender as before.
...
- **Two `r` tags** when the URLs differ: a listen-link via `podcastLandingUrl` (prefers `https://pod.link/<itunesId>`, falls back to `https://podcastindex.org/podcast/<feedId>`, then the raw RSS URL) and a BMB deep-link via `bmbLandingUrl` (`https://boostmebitch.com/?podcast=<podcastGuid>`, only when the podcast has a guid). Both are appended to the body so readers see "listen elsewhere" and "boost back on BMB" as separate affordances.
