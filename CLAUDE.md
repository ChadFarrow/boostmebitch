...
Two surfaces share one `<audio>` and the store's playback state: the always-mounted mini-player (`components/player.tsx`, fixed bottom bar) and the `<FullscreenPlayer>` (`components/fullscreen-player.tsx`) it opens on tap.
...
- **Fullscreen content (right pane):** title/seek → control row (`<TransportControls size="lg">` + `⚡ BOOST` with an `ml-28` gap + `<FavHeart size="md">`) → value-split disclosure → **album tracklist** for music (`Album · N tracks`, the `episodeQueue` rendered clickable, current track highlighted, `max-h-80` internal scroll) → about → chapters → `socialInteract` thread. `FavHeart` is exported from `lists.tsx` and reused here (album-level favorite). **For Nostr live streams the right pane is replaced by the live chat** (see Nostr live streams).
...
**Fetch + filter (`lib/nostr/live-streams.ts`).** `fetchNostrLiveStreams()` queries kind:30311 over `LIVE_STREAM_RELAYS` (`DEFAULT_RELAYS` ∪ `wss://relay.zap.stream` + `wss://nostr.wine`, sanitized) within a 7-day `since` window, dedupes replaceable events by NIP-33 address (`<pubkey>:<dTag>`, newest `created_at` wins), then:
- **Drops stale `live` events.** A genuinely-live 30311 event is re-published periodically while broadcasting (zap.stream bumps `current_participants` ~every minute), so an active stream's `created_at` stays fresh. Many clients never publish the `ended` status — they just stop updating — so a `live` event older than `LIVE_FRESH_SECS` (2h) is treated as a dead broadcast and dropped. **Planned streams are exempt** (their event is set once, ahead of time).
- **Sorts upcoming-first**, then live; within a group by `startsAt` ascending.

**Stream → player/boost bridge.** `streamToEpisode(stream, value)` / `streamToPodcast(stream, profile)` map a stream onto the existing `Episode`/`Podcast` shapes so the player, boost modal, and `liveStatus` UI all work unmodified. Key fields: `episode.guid = stream.id` (the `<pubkey>:<dTag>` NIP-33 id), `episode.enclosureUrl =` the HLS `streaming` URL, `liveStatus` from the stream status. `id`/`feedId` are synthetic (`fnvHash`/0).

**V4V (`resolveStreamV4V`).** Builds a `ValueBlock` of `lnaddress` recipients from each participant's kind:0 `lud16`/`lud06`. Source: NIP-53 `zap` split tags (`["zap", pubkey, relay, weight]`) when present, else the host pubkey as sole recipient. An explicit weight of `0` means the host opted that participant out — preserved (not coerced to 1) and dropped from the recipient list; falls back to the host if every split is zeroed.

**HLS video playback (`player.tsx`).** A single `<video>` lives in a **reverse portal** (`react-reverse-portal`) so it can move between the mini-bar thumbnail and the fullscreen art pane **without remounting** (a remount kills playback + the `hls.js` attachment) — this is what keeps audio playing when you collapse the player. Load-bearing:
- `isHlsUrl(url)` (`lib/util.ts`, matches `.m3u8`) gates the video path; everything else stays on the native `<audio>`. The inactive element is left srcless/paused so the `<audio>` never errors on an `.m3u8`.
- `hls.js` is **dynamic-imported** on first stream play (out of the main bundle); native HLS (`canPlayType('application/vnd.apple.mpegurl')`, Safari/iOS) skips it. Fatal `hls.js` errors surface in the existing error chip.
- The portal node is **created lazily on the client only** (`createHtmlPortalNode()` touches `document` → would crash Next SSR; `<Player>` renders null until there's a `current`, so no hydration mismatch).
- Tapping a card opens the fullscreen player (`onOpen` → `setPlayerExpanded(true)`); the card's PLAY button plays in the mini-bar only (`onPlay`); both `stopPropagation`. Mouse-wheel over the card row scrolls it horizontally (native non-passive `wheel` listener — React `onWheel` is passive).

**Live chat (NIP-53 kind:1311).** `lib/nostr/live-chat.ts` + `components/live-chat.tsx`, shown in the fullscreen right pane for live streams. `subscribeLiveChat(streamId, onEvent)` opens a **long-lived** subscription (owns its own `SimplePool` — `withPool` is request/response and tears down too early) to kind:1311 tagged `a = 30311:<streamId>`; relays backfill recent + stream live. `publishLiveChat(streamId, content)` posts a kind:1311 (interoperates with zap.stream). `<LiveChat>` resolves author profiles the standard way (`storage.profile` tri-state + `fetchProfile` once each), renders via the shared `<Avatar>`, applies the mute filter, auto-scrolls only when already at the bottom, and gates the composer on sign-in (the prompt opens the shared sign-in modal via `signInOpen`). Input clears immediately on send, restoring the text only on failure.
...
- **`isHlsUrl(url)` lives in `lib/util.ts`** (matches `.m3u8`). Use it to branch the HLS-video path from the native `<audio>` path — don't inline the regex.
