...
| `npm run check:npub` (`scripts/check-nostr-txt.mjs`) | `parseNostrTxtNpubs` — the `<podcast:txt purpose="nostr">` → `p`-tag parser | a boost note `p`-tags a stranger under the site's NIP-05-verified identity, permanently and unsendably (over-accept: an unqualified `<podcast:txt>` holds a platform verification token, not an npub), or the artist the feature exists to reach is silently never tagged (under-accept) |
