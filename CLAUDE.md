...
| `npm run check:stream` (`scripts/check-stream-ledger.mjs`) | `accrue`, `settlePlan`, `accruedSats` | streaming drains a wallet at 60× the chosen rate, or silently pays nothing |
...
**Out of scope, deliberately: `valueTimeSplits`.** Streaming targets `episode.value ?? podcast.value`, so a music album streams to the feed's block rather than each track's own artists. Boost-all remains the per-track path.
