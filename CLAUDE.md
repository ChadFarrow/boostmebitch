...
- **The dock hides by CLIPPING, never `display: none`** (`libre-wallet-host.tsx`). Three facts, all browser-verified, that make this the only option:
  ...
  The card only *shows* for `ATTENTION_VIEWS` (blocked / halted / paused / moved-away / need-secret / setup-needed) — a working wallet needs no floating card, and a permanent one sat on top of real content.
...
`lib/v4v/*` and `lib/nostr/` are intentionally the only files that talk to wallets/signers. Components import only: `lib/v4v/boost.ts` (orchestrator), `lib/v4v/nwc.ts` (URI persistence), `lib/v4v/spark.ts` (wallet surface), `lib/v4v/webln.ts` (provider gate), `lib/v4v/libre.ts` (embedded wallet), `lib/v4v/wallets.ts` (cross-rail cleanup + `railLabel`), `lib/nostr/` barrel (auth + publish + wallet backup). Swap toolkit by replacing internals here without touching `components/` or `app/`. The one thing that crosses the line is the `<libre-wallet>` DOM node itself (`borrow`/`park`) — the widget *is* a DOM element, and the alternative was remounting it, which kills the LDK session.
