...
- **Nostr kind:30311 streams are still excluded from streaming sats.** Their payments are real NIP-57 zaps (boost-flow invariant 0) so the receipt lands in the stream's chat; a streamed settle would go out as an ordinary boostagram, uncorrelated and invisible to the host. Making that a zap means a new payment path inside the file whose whole discipline is "not a new payment path". Deferred deliberately.
