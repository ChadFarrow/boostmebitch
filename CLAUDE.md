...
A live V4V music show plays a different artist every few minutes and the payment target has to follow. **`<podcast:valueTimeSplit>` cannot do this**: it is anchored to `startTime`/`duration` offsets into a finished enclosure, and a live stream has no absolute time base to sync those to (namespace Discussion #567 says so outright). There is no live-VTS tag. The convention that ships is that the publisher **rewrites the live item mid-broadcast** and podpings; apps re-read it on a timer. CurioCaster (`github.com/thebells1111/curiocaster`, public) is the reference implementation.
...
**Three RSS signals, resolved in `lib/pi.ts:resolveLiveSplit`, precedence `remoteItem > valueTimeSplit > value`:**
