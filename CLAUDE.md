...
- **The approval sheet must outrank our modals.** The dock is `z-30`, the boost modal `z-60`; `requestApproval` settles *only* from a click and has no reject/timeout, so a covered sheet is an infinite hang, not a misdraw. Fixed **upstream**: the sheet is a top-layer `<dialog>` (`showModal()`), which no z-index can cover. Corollary: never put the host under `display: none` while a spend is in flight — the top layer renders nothing beneath a hidden ancestor (hence `hidden` never covers `'running'`).
