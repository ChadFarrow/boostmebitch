...
**Seven `check:*` scripts stand in for the tests this repo doesn't have.** Each guards a function whose silent breakage costs a user something irreversible — treat a failure as a stop:
...
| `npm run check:favsync` (`scripts/check-favsync.mjs`) | `mergeSharedFavorites`, `tagsForSharedFavorites`, `itemsFromTags`, `otherTagsFrom`, `identifierKind`, `interpretShows`/`interpretItems` | favorites the user made in ANOTHER app are silently deleted from relays with no undo (a shared replaceable event has no partial update), or unfavoriting stops propagating forever, or a URL-shaped item guid emits a `k` tag no relay filter matches |
