...
| [`docs/google-key-backup-spec.md`](docs/google-key-backup-spec.md) | Any constant or derivation in `backup-crypto.ts`, `drive-backup.ts`, `spark-derive.ts` — it's a **published format other apps implement**, not app-local config |
...
- **The `bmb-google-backup` / `bmb-spark-wallet` / `bmb_bk_` constants are a PUBLISHED wire format, not app-local config.** They read like internal naming and they aren't: `docs/google-key-backup-spec.md` is written for other apps to implement, so renaming one locks users out of their Drive backup (or moves them to an empty wallet) *in every app*, silently. Never edit a label in place — version it. `npm run check:spark` pins all of them. → [`docs/google-key-backup-spec.md`](docs/google-key-backup-spec.md)
