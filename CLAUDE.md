...
`.env.local`: `PODCAST_INDEX_KEY`/`SECRET` (server-only), `APP_NAME` (optional). The Spark rail (`@buildonspark/spark-sdk`) needs **no** API key — it talks straight to Spark's signing operators. `NEXT_PUBLIC_LIBRE_GOOGLE_CLIENT_ID` (optional) turns the experimental Libre rail on — absent everywhere except the `libre.boostmebitch.com` preview; see Libre rail.

`postinstall` copies the Libre LDK wasm into `public/` and is **non-fatal** — it warns and exits 0 when the package isn't there, so `npm install` never fails over a rail you're not using.
...
- **Browser-only:** `lib/store.ts`, `lib/v4v/nwc.ts`/`webln.ts`/`lnaddr.ts`/`spark.ts`/`boostbox.ts`/`libre.ts`/`wallets.ts`, `lib/nostr/`, `lib/storage.ts`, `lib/podcast-meta.ts` — they touch `window.*`, storage, IndexedDB, or load WASM. SSR guards exist but assume client context. `libre.ts` is the strictest of these: it mounts a custom element and owns `window.webln`, and its package import is dynamic so the LDK/WASM bundle can't reach the server or the main chunk.
...
All wallet config lives in **`components/wallet-modal.tsx`** — a portal'd overlay opened from `<AuthControl>` in the header (`walletOpen` in the store). The modal is portal'd to `document.body` so `position: fixed` resolves against the viewport, not the sticky `<header>` (the header's `backdrop-blur` creates a containing block for fixed descendants per CSS spec — without the portal, mobile renders it clipped to the header). When signed out the **Spark row is disabled** with a "◆ Sign in with Nostr" hint (its seed is NIP-44-encrypted to the user's key — see Spark rail); NWC, WebLN, and Libre all work fully signed-out (Libre roams via Google Drive, not Nostr — see Libre rail).

The modal is a three-view state machine — `WalletView = picker | connecting | connected` — showing **one rail at a time**, not a stack of cards. It opens on `connected` when `getActiveRail()` finds one, else the `picker`. Picking a rail → `connecting` (that sub-card in `mode="form"`); a successful connect → `connected` (`mode="card"`). "Switch wallet →" reopens the picker with `switching: true`, where tapping an **already-connected** rail just makes it the active payer via `recordLastRail` — no disconnect — while connecting a *new* one calls `clearOtherWallets` in `handleConnected`.

`getActiveRail()` mirrors `pickRail()` (rail pref first, then NWC > Spark > WebLN) with two deliberate differences: WebLN is gated on `isWeblnEnabled()`, not mere detection — inside the wallet UI "active" means the user enabled it — and **Libre outranks the stored pref**, because while it runs it *is* `window.webln` rather than one candidate among several (see Libre rail).

Picker rows: Libre (only when `libreConfigured()`), WebLN (only when `hasWebln()` and Libre isn't running — Libre fronts `window.webln`, so the WebLN row would be a second door to the same wallet), NWC, Spark (disabled with the sign-in hint when signed out).

Sub-cards (each its own component): `nwc-wallet.tsx`, `spark-wallet.tsx`, `webln-wallet.tsx` (`mode="form"` when connecting, `mode="card"` when connected). Libre has no sub-card — the widget draws its own UI, so `<LibreRailSlot>` just reparents the element the layout owns (see Libre rail). State changes propagate via `subscribeNwc()` / `subscribeSpark()` / `subscribeWebln()` / `subscribeLibre()`; the modal `setTick`s on any to flip views without remount.
...
**Boost modal rail picker.** Single-boost `BoostModal` picks a rail silently via `pickRail()` (NWC > Spark > WebLN priority); the only mid-modal feedback is the "no wallet connected" hint when `!rail`. The visible "Pay via [NWC] [Spark] [WebLN]" pill row lives in `BoostAllModal` (`components/boost-all-modal.tsx`) — see the boost-all section below. Labels come from `railLabel()` (`lib/v4v/wallets.ts`), so the WebLN pill reads **"Libre"** while the Libre wallet owns `window.webln`; that's why `RAIL_LABELS` is built per-render rather than as a module const. Both modals subscribe to `subscribeNwc`/`subscribeSpark` (+ `subscribeLibre` in `BoostAllModal`) so a wallet connected mid-modal updates `rail` without remount. WebLN doesn't have its own subscribe (the extension is either injected at load or it isn't), so `hasWebln()` is read on each render.
...
All three balance helpers swallow errors and return null so a missing capability (NWC connection without `get_balance` permission, WebLN provider without `getBalance`) just hides the chip rather than throwing. **Libre is permanently in that bucket** — its provider implements no `getBalance` at all, so the header chip never paints for it and the last-known cache is never written; the widget renders its own balance instead. That also makes the `cached.rail !== 'webln' || weblnReady` guard below a no-op for Libre rather than a stale-balance risk.
...
## Libre rail (`@libre/wallet-embed`) — EXPERIMENTAL, not on production

A roaming, in-page LDK Lightning node: the listener's own wallet runs *inside the page* while they use the app, so a backgrounded iOS PWA wallet can't freeze mid-boost. `lib/v4v/libre.ts` + `components/libre/`.

**There is no `'libre'` in `Rail`, and that's the design.** The widget's provider is installed as `window.webln` (`claimWebln`, on the stopped→running transition), so boosts flow through the **existing WebLN rail** — `lib/v4v/boost.ts`, `zap.ts`, `RailPref`, and `settings-backup.ts` need **zero changes** (verify with `git diff main -- lib/v4v/boost.ts lib/v4v/zap.ts`: empty). An earlier attempt (PR #118) made it a real fourth rail, which meant a `'libre'` arm in every payment dispatch and WebLN suppressed at five sites — it missed `zap.ts`, so live-stream zaps silently paid over `weblnPayInvoice` and lit the WebLN rail back up. Don't reintroduce the rail.

**`Rail` vs `WalletChoice` (`lib/v4v/wallets.ts`).** The two questions this rail forces apart:

- **`Rail`** (`'nwc' | 'spark' | 'webln'`) — *how does the money move?* Owned by `boost.ts`. A missing arm here is a silent mispayment.
- **`WalletChoice`** (`Rail | 'libre'`) — *which wallet did the user pick?* Owned by `wallets.ts`; drives the modal's picker and `clearOtherWallets`'s `keep`.

Libre is a choice without being a rail, so everything that knows Libre exists is presentation or selection (`railLabel`, `getActiveRail`, `railConnected`, `weblnDetected`, `weblnReady`, `keep: 'libre'`) and nothing is dispatch. That asymmetry is the whole trade: ~6 display-layer conditionals instead of an arm in every payment path. If a *second* `window.webln`-fronting wallet ever appears, grow `WalletChoice` — never `Rail`.

- **`NEXT_PUBLIC_LIBRE_GOOGLE_CLIENT_ID` is the kill switch.** No id → no picker row, no host, no widget chunk (`libreConfigured()`, `components/libre/libre-mount.tsx`). `NEXT_PUBLIC_*` is inlined at build time, so a build without it *cannot* enable Libre. Production deliberately doesn't set it; the experiment lives at `libre.boostmebitch.com` off `feat/libre-wallet-embed`, with the var set on Vercel **Preview only**.
- **`window.webln` is captured, not clobbered.** `claimWebln` overrides an incumbent (Alby) because picking Libre is explicit intent, but it snapshots the prior provider and `releaseWebln` puts it back — same shape as `lib/nostr/signer.ts:captureOriginal()` for `window.nostr`, and for the same reason: an extension injects once per page load, so a provider you `delete` is gone until a reload. `releaseWebln` also calls `weblnDisable()`; without it a wallet that roams away mid-session leaves `weblnEnabled` true, and the account menu claims "WebLN — connected" while `pickRail()` throws "no payment provider".
- **`storage.libreActive` (`bmb:libre_active`) is the adoption flag**, and it's what stops the rail eating your other wallets. `view` is module state that resets to `'stopped'` on every load, so a returning user's auto-mount reaching `running` is indistinguishable from a fresh connect *at the state edge* — clearing there unconditionally deleted the user's NWC URI on **every reload**. So "is this the moment Libre became this browser's wallet?" is captured **inside the transition**, before the flag is written, and reported once via `consumeFreshAdoption()`; the host clears the other rails only when that returns true. (Sampling `libreActive` in the host at effect setup fixes the reload but goes stale within a page life: disconnect Libre → connect NWC → reconnect Libre would leave both wired up, `pickRail` quietly preferring NWC.) The other half is `clearOtherWallets` knowing Libre exists (`keep: 'libre'`): any other rail winning must actually stop Libre, or it keeps running, keeps `window.webln`, keeps the flag, and re-adopts on the next load.
- **The approval sheet must outrank our modals.** The dock is `z-30`, the boost modal `z-60`; `requestApproval` settles *only* from a click and has no reject/timeout, so a covered sheet is an infinite hang, not a misdraw. Fixed **upstream**: the sheet is a top-layer `<dialog>` (`showModal()`), which no z-index can cover.
- **The dock hides by CLIPPING, never `display: none`** (`libre-wallet-host.tsx`). Three facts, all browser-verified, that make this the only option:
  1. The top layer renders **nothing** under a `display: none` ancestor — hiding the dock that way silently restores the hang above.
  2. The widget sets `:host { all: initial }`, which **resets `visibility` and `pointer-events`**. So `visibility: hidden` on the dock leaves the card fully visible, and `opacity: 0` leaves an invisible click-blocker over the page. Neither works.
  3. A `w-0 h-0 overflow-hidden` box removes the card (top-layer children escape ancestor clipping) while keeping the sheet renderable.

  The card only *shows* when the wallet wants an answer: a mounted widget in one of `ATTENTION_VIEWS` (blocked / halted / paused / moved-away / need-secret / setup-needed), or a mount that **failed** (an adopted user whose bundle 404'd has no other way to find out). `running` and `loading` deliberately don't qualify — a working wallet needs no floating card, and a permanent one sat on top of real content. Note `libreStatusKind()` returns null whenever the widget is mounted, so gating the card on `isLibreMounted() && …error` makes the error branch unreachable — that shipped once.
- **A failed mount must not re-arm itself.** `ensureLibreMounted` notifies when it settles, and that notify lands in the host's own `subscribeLibre` handler — so `mountIfWanted` is gated on `!getLibreMountError()`. Without it a failure re-enters immediately, clears the error it just set, and re-fetches the 5 MB chunk forever. Recovery is the explicit `retryLibreMount` behind the card's "Try again".
- **The widget is themed from `app/globals.css`** (`libre-wallet { --lw-*: … }`). Its colors are custom properties on `:host`; those inherit through the shadow boundary and an outer page's declarations outrank `:host`, so mapping them to our role tokens re-skins it and follows the light/dark toggle for free. Map, never hardcode.
- **Lazy by necessity:** ~5 MB JS + ~12 MB WASM, dynamic-imported only when `isLibreWanted()` — an explicit pick, an adopted user, or a Drive OAuth redirect landing (`location.hash` carries `access_token`; that check is the *only* thing that mounts the widget to finish a first-ever connect in an installed PWA, where the redirect kills module state before `libreActive` is ever set).
- **Rail labels go through `railLabel(rail)`** (`lib/v4v/wallets.ts`): the `'webln'` rail reads **"Libre"** while Libre runs, so the chip / account menu / boost-all pills don't call the user's wallet "WebLN". Libre's provider has **no `getBalance`**, so `weblnGetBalance()` returns null and the header chip stays hidden — the widget draws its own balance.
- `postinstall` copies the LDK wasm to `public/liblightningjs.wasm` (gitignored) and is **non-fatal by design** — a contributor who never sets the client id must not have `npm install` fail over a rail they can't use.
- **The dep is a GitHub release tarball pinned to a commit sha — never the rolling `libre-wallet-embed.tgz` URL.** `@libre/wallet-embed` isn't on npm; it's packed from [`ChadFarrow/libre-listener-wallet-monorepo`](https://github.com/ChadFarrow/libre-listener-wallet-monorepo) (`packages/wallet-embed`), which publishes both a rolling asset and an immutable `libre-wallet-embed-<sha>.tgz` per push to master. npm records an **integrity hash per resolved URL**, so pinning the rolling URL means the next upstream push fails our `npm ci` with `EINTEGRITY` — a red production deploy triggered by a commit in another repo. To take an upstream change: grab the new `<sha>` asset URL from the `wallet-embed-latest` release, swap it in `package.json`, `npm install`, commit the lockfile. The release workflow gates every publish on a clean-cache consumer install + a `tsc` probe, because the tarball shipped uninstallable for its entire life before that (unpublished workspace deps in `dependencies`, and bare `@libre/*` specifiers in the emitted `.d.ts` — `noExternal` only bundles JS, so the types needed `dts.resolve` too).
- **After bumping the pin, CHECK THE DEPLOYED WIDGET — Vercel will happily serve the old one.** A pin bump touches only `package.json` + `package-lock.json`, never app source, and Vercel restores Next's `.next/cache` between builds. Webpack then reuses its cached compilation of `@libre/wallet-embed` and the new tarball never reaches the bundle. `npm ci` is *not* the safety net people assume: the lockfile's integrity hash guarantees the right bytes land in `node_modules`, and the build ignores them anyway — so every link looks correct (right tarball published, right sha pinned, right commit built, right domain aliased) while the site runs old code. This shipped: `libre.boostmebitch.com` served a widget missing the #90 roaming fix — the fund-loss fix — with a green deploy and nothing anywhere to indicate it.

  **The tell is the widget chunk hash.** It's content-addressed, so a bump that took changes it; a bump that didn't leaves it byte-identical. Load the page with `bmb:libre_active=1`, find the ~1.7 MB chunk the dynamic import fetches (`502c5b3b.<hash>.js`), and grep it for a string only the new build has. Same hash after a bump = the bump didn't take. The fix is a **no-build-cache rebuild** (Vercel → Redeploy → untick "Use existing Build Cache", or `VERCEL_FORCE_NO_BUILD_CACHE=1` for one build); afterwards the chunk hash moves and the new code is there.
...
`lib/v4v/*` and `lib/nostr/` are intentionally the only files that talk to wallets/signers. Components import only: `lib/v4v/boost.ts` (orchestrator), `lib/v4v/nwc.ts` (URI persistence), `lib/v4v/spark.ts` (wallet surface), `lib/v4v/webln.ts` (provider gate), `lib/v4v/libre.ts` (embedded wallet), `lib/v4v/wallets.ts` (cross-rail cleanup + `railLabel` + `WalletChoice`), `lib/nostr/` barrel (auth + publish + wallet backup). Swap toolkit by replacing internals here without touching `components/` or `app/`.

**Libre looks like an exception and isn't.** `ensureLibreMounted(home)` / `borrowLibreElement(target)` / `parkLibreElement()` pass DOM *in* — a component hands over its own empty `<div>` as a slot and gets nothing back. The `<libre-wallet>` element never escapes into `components/`, so the boundary holds: `libre.ts` still owns the node, the LDK session, and the `window.webln` claim. (An earlier `getLibreElement()` did hand the node out; it had zero callers and was deleted — it was the only thing that made this look leaky.) Hosting the widget from `components/` instead would be the real violation: the component would own `window.webln`, and `window.webln` *is* the payment rail.
...
| `bmb:libre_active` | `'1'` once Libre is this browser's wallet. Two jobs: auto-mount the widget on later visits (so a returning user doesn't re-pick it, and the Drive OAuth redirect lands where it can be completed), **and** record that the other rails were already torn down for this adoption. The host reads it once per page load — Libre reaching `running` on a reload must NOT re-run `clearOtherWallets`, or every reload silently deletes the user's NWC URI. Cleared by `clearOtherWallets` when another rail wins, and by the modal's Disconnect. |
