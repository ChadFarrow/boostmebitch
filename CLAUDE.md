...
**Libre rail (embedded in-page wallet, `lib/v4v/libre.ts`).** The Libre Listener Wallet widget (`@libre/wallet-embed`) runs a Lightning node *inside the page* — the iOS fix, since a backgrounded PWA wallet gets frozen mid-boost. Load-bearing rules:

- **Widget lifetime is module-owned, not React-owned.** `ensureLibreWidget()` mounts the widget ONCE per page into a module-level container; `<LibreWalletHost>` (mounted in `app/layout.tsx`, like `<Player>`) only adopts/detaches that DOM node into a fixed dock (bottom-right, `z-[55]` — above boost modal z-40 and fullscreen player z-50 so the widget's approval/spending-cap prompts are reachable mid-boost, below the sign-in modal z-[60]). Effect cleanup **detaches only — never disposes**; `dispose()` lives in exactly one place, `libreDisconnect()`. This is what survives StrictMode's dev double-mount, wallet-modal closes, and route changes.
- **Connect sequencing is redirect-safe.** The wallet modal's Libre form runs `clearOtherWallets('libre')` + `libreOptIn()` (sets global `bmb:libre`) + `recordLastRail('libre')` at the Enable click, BEFORE the user touches the widget's Google sign-in — the OAuth full-page redirect back to `/` destroys the page instance, so nothing can wait for a "connected" callback. The persisted flag makes the layout host remount the widget on return.
- **`installWebln: true` makes `window.webln` BE the Libre provider**, so every WebLN surface is suppressed while `hasLibre()`: `pickRail()`'s webln arm, the wallet-modal WebLN picker row, `BoostAllModal.availableRails`, and `WalletButton`'s summary. Without this, boosts would silently route through the embedded wallet under the WebLN name.
- **Payments never go through `lib/v4v/webln.ts`.** `librePayInvoice`/`libreKeysend`/`libreGetBalance` call `window.webln` directly — reusing `weblnPayInvoice` would flip that module's `weblnEnabled` flag and light the WebLN rail up everywhere. `isLibreRunning()` (widget view === `'running'`) gates payability; the `bmb:libre` flag alone only gates mounting + webln suppression.
- **Rail gated on `NEXT_PUBLIC_LIBRE_GOOGLE_CLIENT_ID`** (`libreConfigured()`): no env var → no picker row, no dock, zero behavior change. The client id is not a secret, but the app's origins must be registered on the Google OAuth client (JS origins AND redirect URIs, bare origin, no trailing slash).
- **The dep is a mutable GitHub release tarball** (`wallet-embed-latest`). `package-lock.json` pins an integrity hash, so an upstream re-tag fails fresh installs loudly instead of silently changing code. `postinstall` copies `dist/liblightningjs.wasm` → `public/liblightningjs.wasm` (gitignored). `types/libre-wallet-embed.d.ts` is a hand-written ambient declaration transcribed from the monorepo's embed guide — verify it against the package's own `dist/*.d.ts` and update/delete it if they disagree.

**Boost modal rail picker.** Single-boost `BoostModal` picks a rail silently via `pickRail()` (NWC > Spark > Libre > WebLN priority); the only mid-modal feedback is the "no wallet connected" hint when `!rail`. The visible "Pay via [NWC] [Spark] [WebLN]" pill row lives in `BoostAllModal` (`components/boost-all-modal.tsx`) — see the boost-all section below. Both modals subscribe to `subscribeNwc`/`subscribeSpark`/`subscribeLibre` so a wallet connected mid-modal updates `rail` without remount. WebLN doesn't have its own subscribe (the extension is either injected at load or it isn't), so `hasWebln()` is read on each render.
...
| `bmb:rail_pref` | `'nwc' \| 'spark' \| 'webln' \| 'libre'` — preferred boost rail, written by `recordLastRail` after a successful boost AND by the wallet modal's "Switch wallet" picker (tapping an already-connected rail makes it the active payer without disconnecting the others) — both **synced to Nostr** (`d:boostmebitch:settings`). Honored by `pickRail()`; falls back to NWC > Spark > Libre > WebLN priority when absent or when the preferred rail isn't available. Setter notifies `subscribeRailPref`. |
| `bmb:libre` | `'1'` when the user opted into the embedded Libre wallet. Global, not per-npub (the widget's session is a browser/origin-level Google session, same reasoning as `bmb:nwc_uri`). Set/cleared ONLY via `libreOptIn()`/`libreDisconnect()` in `lib/v4v/libre.ts` so the libre observable fires. Cleared on sign-out and npub-switch. While set, the layout host mounts the widget dock and all WebLN surfaces are suppressed. |
