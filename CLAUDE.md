...
| `npm run check:stream` (`scripts/check-stream-ledger.mjs`) | `accrue`, `settlePlan`, `accruedSats`, `distribute` | streaming drains a wallet at 60× the chosen rate, silently pays nothing, or pays a whole batch to the wrong artist |
...
The **SHARE button** (`components/lists.tsx:ShareButton`) copies `origin + ?podcast=<guid>` with a 1.8 s "COPIED" flip. Clipboard-only by design — no Web Share API, no pod.link option (that's what the Nostr boost note links to via `podcastLandingUrl`). Header cluster order: `[♡ FAVORITE] [↗ SHARE] [⚡ BOOST]`; BOOST gated on `showHasValue`, the others always visible.
...
The third Podcasting 2.0 payment mode, alongside the boost button and boost-all. `lib/v4v/stream-ledger.ts` (pure math) + `lib/v4v/streaming.ts` (the engine). **Off by default** — a rate must be set before anything is spent.
...
- **Two consecutive failed settles stop streaming for that item**, surfaced through `<StreamMeter>`. A dead wallet must not accrue an unpayable debt in silence. Changing the rate is the retry.
...
- **Rate resolution: per-show override → global → off.** At show scope, absent and `0` are different states — `0` means "never stream this show" and must outrank a global rate raised later. Hence `storage.streamRate.getShow` is tri-state.
...
- **Each bucket clears its own floor independently and carries when it doesn't.** Bucket keys are `t:<feedGuid>:<itemGuid>`, so a short track tops up across the episode and across replays instead of being rounded away every batch.
...
- **An unresolved split falls back to the host block, never to nothing.** PI doesn't index every album feed (the same ~50% coverage gap that forced the RSS fallback in `resolveValueTimeSplits`), and those sats have already been accrued from the listener. Splits load fire-and-forget via `/api/value-splits`, cached per episode id; a *fetch failure* is deliberately not cached, so a blip can't pin a whole episode to the host block.
...
`<StreamMeter>` names the track currently being credited. That's the only visible proof the redirect is being followed — otherwise streaming looks identical whether the artist is being paid or not.
...
| `bmb:stream_rate` | Global streaming sats-per-minute; **absent = off**, the default. Also the prefix for the per-show override `bmb:stream_rate:<podcastGuid\|feedId>`, where `'0'` means "never stream this show" and outranks the global rate — absent-vs-zero is a real distinction there. Both reads clamp to a sane range: this number is multiplied by elapsed time and paid without a prompt, so a corrupt value must read as *off*, not as an unbounded spend. Setter notifies `subscribeStreamRate`. |
| `bmb:stream_pending` | The unsent `StreamLedger`, mirrored every ~10 s and on every edge, so closing the tab mid-window doesn't silently discard accrued sats. Adopted on the next play of the **same** item only, dropped after 24 h. Reads reject a structurally-incomplete record outright — this value becomes a payment. |
| `bmb:streamed:*` | Per-npub settled-stream log, capped 100. **Deliberately not `bmb:boosts:*`** — see the streaming section. |
