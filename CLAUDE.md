...
- **`storage.libreActive` (`bmb:libre_active`) is the adoption flag**, and it's what stops the rail eating your other wallets. `view` is module state that resets to `'stopped'` on every load, so a returning user's auto-mount reaching `running` is indistinguishable from a fresh connect at the state edge. The host therefore reads it **once at effect setup** (`adoptedBeforeThisVisit`) and only calls `clearOtherWallets` when Libre is adopted *on this visit*. Clearing on that edge unconditionally deleted the user's NWC URI on **every reload**. The other half is `clearOtherWallets` knowing Libre exists (`keep: 'libre'`): any other rail winning must actually stop Libre, or it keeps running, keeps `window.webln`, keeps the flag, and re-adopts on the next load.
