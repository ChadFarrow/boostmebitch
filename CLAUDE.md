...
**Shared follow-state singleton** (also `follows.ts`): `ensureFollowsLoaded` / `subscribeFollows` / `followsSnapshot` / `toggleFollow` / `resetFollows` — **one kind:3 fetch app-wide** (a feed of 20 cards does 1 fetch, not 20), with **serialized toggles** (a promise `chain`) so two quick follows can't each republish from stale state and drop each other. `components/follow-button.tsx` holds the `useFollows()` hook + `<FollowButton>`; the show-notes buttons use the same singleton, so a follow made in a card and in the notes stay in sync. The singleton is store-free (no zustand import) to avoid a cycle with `lib/store`.
...
Proxy `app/api/transcript/route.ts` clones `/api/chapters` but returns the body as **text** (SRT/VTT) or JSON with the upstream Content-Type passed through — plus the `?type=` hint, load-bearing because hosts serve SRT as `application/octet-stream`.
...
- **Funding:** `<podcast:funding url>message</podcast:funding>` (channel-level, `parseFunding`) → `Podcast.funding` (also read from PI's `funding` in `buildPodcast`). Rendered as a `[↗ SUPPORT]` link in the `EpisodeList` header cluster and next to BOOST on the episode page.
