...
- **Share UI:** `<ShareNostrPicker>` (`components/boost-modal/share-nostr-picker.tsx`), shared by `BoostModal` + `BoostAllModal`. Signed in → three radios (own key / site key / don't post, persisted as `bmb:share_nostr` + `bmb:share_nostr_as`); signed out → the original two-state checkbox (site key or nothing).
