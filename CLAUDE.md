...
- **The kind:30003 favorites list is SHARED with other apps (`d:podcast:favorites`), so the same rule binds harder: read → merge → publish, always, and never on a degraded read.** It is a replaceable event with many writers and no partial update, so a blind publish deletes whatever StableKraft (or any third app) added — silently, on someone else's device. There is deliberately no exported "publish my favorites"; `syncFavorites` is the only writer and it reads first. The merge diffs against `storage.favSynced` (the baseline this device last agreed with the relay on), NOT against the local set, and it never interprets an identifier — kinds this app can't render ride through untouched. → [`docs/pc20-favorites.md`](docs/pc20-favorites.md)
