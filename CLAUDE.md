...
**Anonymity applies to the payment, not just the Nostr note.** `sender_id` is the user's pubkey, and recipient aggregators resolve it to their profile (avatar + name). So when the share picker's **"Anonymous"** is active (`shareNostr && shareAs === 'site'`), `sender_id` is **omitted** from every boostagram — single plus boost-all per-track/host/summary legs (`sender_id: anonymous ? undefined : identity?.pubkey`). Previously "Anonymous" only site-signed the note while the payment still shipped the pubkey, leaking the PFP despite the promise. "My feed"/"Don't post" still identify the sender; signed-out already carried none.
